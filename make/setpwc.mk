...
#SETPWC_CONFFILES=$(OPTWARE_PREFIX)etc/setpwc.conf $(OPTWARE_PREFIX)etc/init.d/SXXsetpwc
...
#
# Binaries should be installed into $(SETPWC_IPK_DIR)$(OPTWARE_PREFIX)sbin or $(SETPWC_IPK_DIR)$(OPTWARE_PREFIX)bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(SETPWC_IPK_DIR)$(OPTWARE_PREFIX){lib,include}
# Configuration files should be installed in $(SETPWC_IPK_DIR)$(OPTWARE_PREFIX)etc/setpwc/...
# Documentation files should be installed in $(SETPWC_IPK_DIR)$(OPTWARE_PREFIX)doc/setpwc/...
# Daemon startup scripts should be installed in $(SETPWC_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/S??setpwc
#
# You may need to patch your application to make it use these locations.
#
$(SETPWC_IPK): $(SETPWC_BUILD_DIR)/.built
	rm -rf $(SETPWC_IPK_DIR) $(BUILD_DIR)/setpwc_*_$(TARGET_ARCH).ipk
	install -d $(SETPWC_IPK_DIR)$(OPTWARE_PREFIX)bin
	install -m 755 $(SETPWC_BUILD_DIR)/setpwc $(SETPWC_IPK_DIR)$(OPTWARE_PREFIX)bin
	$(STRIP_COMMAND) $(SETPWC_IPK_DIR)$(OPTWARE_PREFIX)bin/setpwc
	install -d $(SETPWC_IPK_DIR)$(OPTWARE_PREFIX)man/man1
	install -m 644 $(SETPWC_BUILD_DIR)/setpwc.1 $(SETPWC_IPK_DIR)$(OPTWARE_PREFIX)man/man1
