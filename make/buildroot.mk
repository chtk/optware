...
# BUILDROOT_CONFFILES=$(OPTWARE_PREFIX)etc/buildroot.conf $(OPTWARE_PREFIX)etc/init.d/SXXbuildroot
...
ifneq ($(OPTWARE_TARGET), ts101)
	sed -i.orig -e '/^+/s|/lib/|$(OPTWARE_PREFIX)lib/|g' $(BUILDROOT_BUILD_DIR)/toolchain/gcc/$(BUILDROOT_GCC)/100-uclibc-conf.patch
	sed -i.orig -e '/^+/s|/lib/|$(OPTWARE_PREFIX)lib/|g' $(BUILDROOT_BUILD_DIR)/toolchain/binutils/$(BUILDROOT_BINUTILS)/100-uclibc-conf.patch
#	sed -i.orig -e '/^+/s|/lib/|$(OPTWARE_PREFIX)lib/|g' $(BUILDROOT_BUILD_DIR)/toolchain/binutils/$(BUILDROOT_BINUTILS)/110-uclibc-libtool-conf.patch
	sed -i.orig.0 -e 's|(TARGET_DIR)/lib|(TARGET_DIR)$(OPTWARE_PREFIX)lib|g' $(BUILDROOT_TOOLS_MK)
	sed -i.orig.1 -e 's|(TARGET_DIR)/usr|(TARGET_DIR)/opt|g' $(BUILDROOT_TOOLS_MK)
	sed -i.orig.2 -e 's|=/usr|=/opt|g;s|=\\"/lib|=\\"$(OPTWARE_PREFIX)lib|g;s|=\\"/usr|=\\"/opt|g' $(BUILDROOT_TOOLS_MK)
...
#
# Binaries should be installed into $(BUILDROOT_IPK_DIR)$(OPTWARE_PREFIX)sbin or $(BUILDROOT_IPK_DIR)$(OPTWARE_PREFIX)bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(BUILDROOT_IPK_DIR)$(OPTWARE_PREFIX){lib,include}
# Configuration files should be installed in $(BUILDROOT_IPK_DIR)$(OPTWARE_PREFIX)etc/buildroot/...
# Documentation files should be installed in $(BUILDROOT_IPK_DIR)$(OPTWARE_PREFIX)doc/buildroot/...
# Daemon startup scripts should be installed in $(BUILDROOT_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/S??buildroot
#
...
#	tar -xv -C $(BUILDROOT_IPK_DIR) -f $(BUILDROOT_BUILD_DIR)/rootfs.$(TARGET_ARCH).tar ./opt
	cp -fa $(BUILDROOT_BUILD_DIR)/build_$(TARGET_ARCH)/root$(OPTWARE_PREFIX) $(BUILDROOT_IPK_DIR)
#	Remove files provided by uclibc-opt
	rm -f $(patsubst %, $(BUILDROOT_IPK_DIR)$(OPTWARE_PREFIX)lib/%*so*, $(UCLIBC-OPT_LIBS))
	rm -f $(BUILDROOT_IPK_DIR)$(OPTWARE_PREFIX)sbin/ldconfig
#	install -m 755 $(BUILDROOT_BUILD_DIR)/build_$(TARGET_ARCH)/root/usr/bin/ccache $(BUILDROOT_IPK_DIR)$(OPTWARE_PREFIX)bin
	install -m 755 $(BUILDROOT_BUILD_DIR)/build_$(TARGET_ARCH)/root/usr/bin/gdb $(BUILDROOT_IPK_DIR)$(OPTWARE_PREFIX)bin
