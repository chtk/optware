...
VPNC_CONFFILES=$(OPTWARE_PREFIX)etc/vpnc/default.conf $(OPTWARE_PREFIX)etc/vpnc/vpnc-script
...
#
# Binaries should be installed into $(VPNC_IPK_DIR)$(OPTWARE_PREFIX)sbin or $(VPNC_IPK_DIR)$(OPTWARE_PREFIX)bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(VPNC_IPK_DIR)$(OPTWARE_PREFIX){lib,include}
# Configuration files should be installed in $(VPNC_IPK_DIR)$(OPTWARE_PREFIX)etc/vpnc/...
# Documentation files should be installed in $(VPNC_IPK_DIR)$(OPTWARE_PREFIX)doc/vpnc/...
# Daemon startup scripts should be installed in $(VPNC_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/S??vpnc
#
...
$(VPNC_IPK): $(VPNC_BUILD_DIR)/.built
	...
	$(STRIP_COMMAND) $(VPNC_IPK_DIR)$(OPTWARE_PREFIX)sbin/vpnc
	$(STRIP_COMMAND) $(VPNC_IPK_DIR)$(OPTWARE_PREFIX)bin/cisco-decrypt
	sed -i -e 's|/var/|/opt&|' $(VPNC_IPK_DIR)$(OPTWARE_PREFIX)etc/vpnc/vpnc-script
	install -d $(VPNC_IPK_DIR)$(OPTWARE_PREFIX)man/man8
	install -m 644 $(VPNC_SOURCE_DIR)/vpnc.8 $(VPNC_IPK_DIR)$(OPTWARE_PREFIX)man/man8
