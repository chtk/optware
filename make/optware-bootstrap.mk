...
$(OPTWARE-BOOTSTRAP_XSH): $(OPTWARE-BOOTSTRAP_BUILD_DIR)/.built $(OPTWARE-BOOTSTRAP_IPKS_DONE)
	...
	install -d -m 755 \
		$(OPTWARE-BOOTSTRAP_IPK_DIR)$(OPTWARE_PREFIX)etc \
		$(OPTWARE-BOOTSTRAP_IPK_DIR)$(OPTWARE_PREFIX)lib \
		$(OPTWARE-BOOTSTRAP_IPK_DIR)$(OPTWARE_PREFIX)var
ifneq ($(OPTWARE-BOOTSTRAP_LIBS),)
	cp -rip $(OPTWARE-BOOTSTRAP_LIBS) $(OPTWARE-BOOTSTRAP_IPK_DIR)$(OPTWARE_PREFIX)lib
endif
	install -d $(OPTWARE-BOOTSTRAP_IPK_DIR)$(OPTWARE_PREFIX)var/lib
	install -d $(OPTWARE-BOOTSTRAP_IPK_DIR)/etc/init.d
	install -d -m 1755 $(OPTWARE-BOOTSTRAP_IPK_DIR)$(OPTWARE_PREFIX)tmp
	install -m 755 $(IPKG-OPT_SOURCE_DIR)/rc.optware $(OPTWARE-BOOTSTRAP_IPK_DIR)$(OPTWARE_PREFIX)etc/
ifneq (, $(filter fsg3 fsg3v4, $(OPTWARE_TARGET)))
	...
	install -d $(OPTWARE-BOOTSTRAP_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d
	if [ -e $(OPTWARE-BOOTSTRAP_SOURCE_DIR)/$(OPTWARE_TARGET)/S00optware ] ; then \
		install -m 755 $(OPTWARE-BOOTSTRAP_SOURCE_DIR)/$(OPTWARE_TARGET)/S00optware \
			$(OPTWARE-BOOTSTRAP_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/ ; \
	fi
