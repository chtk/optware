...
#PLAN9PORT_CONFFILES=$(OPTWARE_PREFIX)etc/plan9port.conf $(OPTWARE_PREFIX)etc/init.d/SXXplan9port
...
PLAN9PORT_PATCHES+=$(PLAN9PORT_SOURCE_DIR)/no-futimes.patch
endif
ifeq ($(OPTWARE_TARGET), $(filter nslu2, $(OPTWARE_TARGET)))
PLAN9PORT_PATCHES+=$(PLAN9PORT_SOURCE_DIR)/no-posix_fadvise64.patch
endif
...
#	$(MAKE) -C $(PLAN9PORT_BUILD_DIR) DESTDIR=$(PLAN9PORT_IPK_DIR) install-strip
	...
	cp -rp $(PLAN9PORT_BUILD_DIR) $(PLAN9PORT_IPK_DIR)$(OPTWARE_PREFIX)plan9
	...
	grep '^[ 	]*echo' $(PLAN9PORT_IPK_DIR)$(OPTWARE_PREFIX)plan9/INSTALL | tail -4 \
		>> $(PLAN9PORT_IPK_DIR)/CONTROL/postinst
