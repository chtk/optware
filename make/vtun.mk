...
VTUN_CONFFILES=$(OPTWARE_PREFIX)etc/vtund.conf
...
#
# Binaries should be installed into $(VTUN_IPK_DIR)$(OPTWARE_PREFIX)sbin or $(VTUN_IPK_DIR)$(OPTWARE_PREFIX)bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(VTUN_IPK_DIR)$(OPTWARE_PREFIX){lib,include}
# Configuration files should be installed in $(VTUN_IPK_DIR)$(OPTWARE_PREFIX)etc/vtun/...
# Documentation files should be installed in $(VTUN_IPK_DIR)$(OPTWARE_PREFIX)doc/vtun/...
# Daemon startup scripts should be installed in $(VTUN_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/S??vtun
#
...
$(VTUN_IPK): $(VTUN_BUILD_DIR)/.built
	...
	$(STRIP_COMMAND) $(VTUN_IPK_DIR)$(OPTWARE_PREFIX)sbin/vtund
#	install -d $(VTUN_IPK_DIR)$(OPTWARE_PREFIX)etc/
#	install -m 644 $(VTUN_SOURCE_DIR)/vtun.conf $(VTUN_IPK_DIR)$(OPTWARE_PREFIX)etc/vtun.conf
#	install -d $(VTUN_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d
#	install -m 755 $(VTUN_SOURCE_DIR)/rc.vtun $(VTUN_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/SXXvtun
#	sed -i -e '/^#!/aOPTWARE_TARGET=${OPTWARE_TARGET}' $(VTUN_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/SXXvtun
