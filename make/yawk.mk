...
YAWK_CONFFILES=$(OPTWARE_PREFIX)etc/yawk/yawk.conf $(OPTWARE_PREFIX)etc/yawk/wiki-wiki.conf
...
#	sed -i.orig \
	    ...
	    -e 's|/usr/local/bin/|$$(DESTDIR)$(OPTWARE_PREFIX)bin/|' \
	    -e 's|/usr/httpd/|$$(DESTDIR)$(OPTWARE_PREFIX)share/www/|' \
	    $(YAWK_BUILD_DIR)/makefile
	sed -i \
	    -e 's|#! */usr/bin/gawk|#!$(OPTWARE_PREFIX)bin/gawk|' \
	    `grep -Il '#! */usr/bin/gawk' $(YAWK_BUILD_DIR)/*`
	sed -i \
	    -e '/configdir[ 	]*\/etc\/yawk/s|/etc/yawk|$(OPTWARE_PREFIX)etc/yawk|' \
	    -e '/bindir[ 	]*\/usr\/lib\/yawk/s|/usr/lib/yawk|$(OPTWARE_PREFIX)lib/yawk|' \
	    $(YAWK_BUILD_DIR)/yawk.conf
	sed -i \
	    -e '/configdir *=/s|"[^"]*"|"$(OPTWARE_PREFIX)etc/yawk"|' \
	    -e '/bindir *=/s|"[^"]*"|"$(OPTWARE_PREFIX)lib/yawk"|' \
	    -e '/yawkconf *=/s|"[^"]*"|"$(OPTWARE_PREFIX)etc/yawk/yawk.conf"|' \
...
#
# Binaries should be installed into $(YAWK_IPK_DIR)$(OPTWARE_PREFIX)sbin or $(YAWK_IPK_DIR)$(OPTWARE_PREFIX)bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(YAWK_IPK_DIR)$(OPTWARE_PREFIX){lib,include}
# Configuration files should be installed in $(YAWK_IPK_DIR)$(OPTWARE_PREFIX)etc/yawk/...
# Documentation files should be installed in $(YAWK_IPK_DIR)$(OPTWARE_PREFIX)doc/yawk/...
# Daemon startup scripts should be installed in $(YAWK_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/S??yawk
#
...
$(YAWK_IPK): $(YAWK_BUILD_DIR)/.built
	...
	install -d \
		$(YAWK_IPK_DIR)$(OPTWARE_PREFIX)etc/yawk \
		$(YAWK_IPK_DIR)$(OPTWARE_PREFIX)share/www/cgi-bin \
		$(YAWK_IPK_DIR)$(OPTWARE_PREFIX)lib/yawk \
		;
	...
	install $(YAWK_BUILD_DIR)/wiki-parser \
		...
		$(YAWK_IPK_DIR)$(OPTWARE_PREFIX)lib/yawk/
	install -d $(YAWK_IPK_DIR)$(OPTWARE_PREFIX)share/yawk-wikispace/wiki-wiki
	echo "dir	$(OPTWARE_PREFIX)share/yawk-wikispace/wiki-wiki" \
		> $(YAWK_IPK_DIR)$(OPTWARE_PREFIX)etc/yawk/wiki-wiki.conf
	chmod -R 777 $(YAWK_IPK_DIR)$(OPTWARE_PREFIX)share/yawk-wikispace
