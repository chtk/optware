...
#PSSH_CONFFILES=$(OPTWARE_PREFIX)etc/pssh.conf $(OPTWARE_PREFIX)etc/init.d/SXXpssh
...
#	cat $(PSSH_PATCHES) | patch -d $(BUILD_DIR)/$(PSSH_DIR) -p1
	...
	sed -i -e '1s|#!.*|#!$(OPTWARE_PREFIX)bin/python2.5|' $(@D)/2.5/bin/p*
	(cd $(@D)/2.5; \
	    ( \
		echo "[build_ext]"; \
...
#
# Binaries should be installed into $(PSSH_IPK_DIR)$(OPTWARE_PREFIX)sbin or $(PSSH_IPK_DIR)$(OPTWARE_PREFIX)bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(PSSH_IPK_DIR)$(OPTWARE_PREFIX){lib,include}
# Configuration files should be installed in $(PSSH_IPK_DIR)$(OPTWARE_PREFIX)etc/pssh/...
# Documentation files should be installed in $(PSSH_IPK_DIR)$(OPTWARE_PREFIX)doc/pssh/...
# Daemon startup scripts should be installed in $(PSSH_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/S??pssh
#
