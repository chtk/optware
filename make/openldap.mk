...
#OPENLDAP_CONFFILES=$(OPTWARE_PREFIX)etc/openldap.conf $(OPTWARE_PREFIX)etc/init.d/SXXopenldap
...
#
# Binaries should be installed into $(OPENLDAP_IPK_DIR)$(OPTWARE_PREFIX)sbin or $(OPENLDAP_IPK_DIR)$(OPTWARE_PREFIX)bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(OPENLDAP_IPK_DIR)$(OPTWARE_PREFIX){lib,include}
# Configuration files should be installed in $(OPENLDAP_IPK_DIR)$(OPTWARE_PREFIX)etc/openldap/...
# Documentation files should be installed in $(OPENLDAP_IPK_DIR)$(OPTWARE_PREFIX)doc/openldap/...
# Daemon startup scripts should be installed in $(OPENLDAP_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/S??openldap
#
...
$(OPENLDAP_IPK): $(OPENLDAP_BUILD_DIR)/.built
	...
	install -d $(OPENLDAP_IPK_DIR)$(OPTWARE_PREFIX)etc/
#	install -m 755 $(OPENLDAP_SOURCE_DIR)/openldap.conf $(OPENLDAP_IPK_DIR)$(OPTWARE_PREFIX)etc/openldap.conf
	install -d $(OPENLDAP_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d
	install -m 755 $(OPENLDAP_SOURCE_DIR)/rc.openldap $(OPENLDAP_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/S58slapd
	...
	rm -f $(OPENLDAP_IPK_DIR)$(OPTWARE_PREFIX)lib/*.a
	rm -f $(OPENLDAP_IPK_DIR)$(OPTWARE_PREFIX)lib/*.la
	$(STRIP_COMMAND) $(OPENLDAP_IPK_DIR)$(OPTWARE_PREFIX)lib/*.so
	$(STRIP_COMMAND) $(OPENLDAP_IPK_DIR)$(OPTWARE_PREFIX)bin/*
	$(STRIP_COMMAND) $(OPENLDAP_IPK_DIR)$(OPTWARE_PREFIX)libexec/*
	install -d $(OPENLDAP_LIBS_IPK_DIR)/opt
	mv $(OPENLDAP_IPK_DIR)$(OPTWARE_PREFIX)include  $(OPENLDAP_LIBS_IPK_DIR)/opt
	mv $(OPENLDAP_IPK_DIR)$(OPTWARE_PREFIX)lib  $(OPENLDAP_LIBS_IPK_DIR)/opt
