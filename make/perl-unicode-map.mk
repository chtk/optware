...
$(PERL-UNICODE-MAP_IPK): $(PERL-UNICODE-MAP_BUILD_DIR)/.built
	...
	perl -pi -e 's|$(PERL_HOSTPERL)|$(OPTWARE_PREFIX)bin/perl|g' $(PERL-UNICODE-MAP_IPK_DIR)/*
	find $(PERL-UNICODE-MAP_IPK_DIR)$(OPTWARE_PREFIX)-name 'perllocal.pod' -exec rm -f {} \;
	(cd $(PERL-UNICODE-MAP_IPK_DIR)$(OPTWARE_PREFIX)lib/perl5 ; \
		find . -name '*.so' -exec chmod +w {} \; ; \
		find . -name '*.so' -exec $(STRIP_COMMAND) {} \; ; \
		find . -name '*.so' -exec chmod -w {} \; ; \
	)
	find $(PERL-UNICODE-MAP_IPK_DIR)$(OPTWARE_PREFIX)-type d -exec chmod go+rx {} \;
