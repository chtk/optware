...
SAMBA2_CONFFILES=$(OPTWARE_PREFIX)etc/init.d/S80samba \
		$(OPTWARE_PREFIX)etc/samba/smb.conf \
		$(OPTWARE_PREFIX)etc/xinetd.d/swat
...
#
# Binaries should be installed into $(SAMBA2_IPK_DIR)$(OPTWARE_PREFIX)sbin or $(SAMBA2_IPK_DIR)$(OPTWARE_PREFIX)bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(SAMBA2_IPK_DIR)$(OPTWARE_PREFIX){lib,include}
# Configuration files should be installed in $(SAMBA2_IPK_DIR)$(OPTWARE_PREFIX)etc/samba2/...
# Documentation files should be installed in $(SAMBA2_IPK_DIR)$(OPTWARE_PREFIX)doc/samba2/...
# Daemon startup scripts should be installed in $(SAMBA2_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/S??samba2
#
# You may need to patch your application to make it use these locations.
#
$(SAMBA2_IPK): $(SAMBA2_BUILD_DIR)/.built
	rm -rf $(SAMBA2_IPK_DIR) $(BUILD_DIR)/samba2_*_$(TARGET_ARCH).ipk
	install -d $(SAMBA2_IPK_DIR)$(OPTWARE_PREFIX)share/
	...
	chmod +s $(SAMBA2_IPK_DIR)$(OPTWARE_PREFIX)bin/smbmount $(SAMBA2_IPK_DIR)$(OPTWARE_PREFIX)bin/smbumount
	$(STRIP_COMMAND) `ls $(SAMBA2_IPK_DIR)$(OPTWARE_PREFIX)sbin/* | egrep -v 'mount.smbfs'`
	$(STRIP_COMMAND) `ls $(SAMBA2_IPK_DIR)$(OPTWARE_PREFIX)bin/* | egrep -v 'findsmb|smbtar'`
	install -d $(SAMBA2_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d
	install -m 755 $(SAMBA2_SOURCE_DIR)/rc.samba $(SAMBA2_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/S80samba
	install -d $(SAMBA2_IPK_DIR)$(OPTWARE_PREFIX)etc/samba
	install -d $(SAMBA2_IPK_DIR)$(OPTWARE_PREFIX)etc/xinetd.d
	install -m 644 $(SAMBA2_SOURCE_DIR)/swat $(SAMBA2_IPK_DIR)$(OPTWARE_PREFIX)etc/xinetd.d
	install -m 644 $(SAMBA2_SOURCE_DIR)/smb.conf $(SAMBA2_IPK_DIR)$(OPTWARE_PREFIX)etc/samba/
	install -d $(SAMBA2_IPK_DIR)$(OPTWARE_PREFIX)var/log/samba
	install -d $(SAMBA2_IPK_DIR)$(OPTWARE_PREFIX)var/spool/samba
