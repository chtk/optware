...
#LIBICONV_CONFFILES=$(OPTWARE_PREFIX)etc/libiconv.conf $(OPTWARE_PREFIX)etc/init.d/SXXlibiconv
...
#
# Binaries should be installed into $(LIBICONV_IPK_DIR)$(OPTWARE_PREFIX)sbin or $(LIBICONV_IPK_DIR)$(OPTWARE_PREFIX)bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(LIBICONV_IPK_DIR)$(OPTWARE_PREFIX){lib,include}
# Configuration files should be installed in $(LIBICONV_IPK_DIR)$(OPTWARE_PREFIX)etc/libiconv/...
# Documentation files should be installed in $(LIBICONV_IPK_DIR)$(OPTWARE_PREFIX)doc/libiconv/...
# Daemon startup scripts should be installed in $(LIBICONV_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/S??libiconv
#
...
$(LIBICONV_IPK): $(LIBICONV_BUILD_DIR)/.built
	...
	$(STRIP_COMMAND) $(LIBICONV_IPK_DIR)$(OPTWARE_PREFIX)bin/iconv
	$(STRIP_COMMAND) $(LIBICONV_IPK_DIR)$(OPTWARE_PREFIX)lib/libcharset.so.[0-9]*.[0-9]*.[0-9]*
	$(STRIP_COMMAND) $(LIBICONV_IPK_DIR)$(OPTWARE_PREFIX)lib/libiconv.so.[0-9]*.[0-9]*.[0-9]*
	$(STRIP_COMMAND) $(LIBICONV_IPK_DIR)$(OPTWARE_PREFIX)lib/preloadable_libiconv.so
#	install -d $(LIBICONV_IPK_DIR)$(OPTWARE_PREFIX)etc/
#	install -m 644 $(LIBICONV_SOURCE_DIR)/libiconv.conf $(LIBICONV_IPK_DIR)$(OPTWARE_PREFIX)etc/libiconv.conf
#	install -d $(LIBICONV_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d
#	install -m 755 $(LIBICONV_SOURCE_DIR)/rc.libiconv $(LIBICONV_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/SXXlibiconv
#	sed -i -e '/^#!/aOPTWARE_TARGET=${OPTWARE_TARGET}' $(LIBICONV_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/SXXlibiconv
