...
ERL_EJABBERD_DESCRIPTION=A Jabber/XMPP server written in Erlang/OTP
ERL_EJABBERD_SECTION=net
ERL_EJABBERD_PRIORITY=optional
ERL_EJABBERD_DEPENDS=erlang, expat
ifeq (libiconv,$(filter libiconv, $(PACKAGES)))
ERL_EJABBERD_DEPENDS+=, libiconv
endif
...
#
# Binaries should be installed into $(ERL_EJABBERD_IPK_DIR)$(OPTWARE_PREFIX)sbin or $(ERL_EJABBERD_IPK_DIR)$(OPTWARE_PREFIX)bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(ERL_EJABBERD_IPK_DIR)$(OPTWARE_PREFIX){lib,include}
# Configuration files should be installed in $(ERL_EJABBERD_IPK_DIR)$(OPTWARE_PREFIX)etc/erl-ejabberd/...
# Documentation files should be installed in $(ERL_EJABBERD_IPK_DIR)$(OPTWARE_PREFIX)doc/erl-ejabberd/...
# Daemon startup scripts should be installed in $(ERL_EJABBERD_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/S??erl-ejabberd
#
...
#	install -m 755 $(ERL_EJABBERD_SOURCE_DIR)/rc.erl-ejabberd $(ERL_EJABBERD_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/SXXerl-ejabberd
	(cd $(ERL_EJABBERD_IPK_DIR)$(OPTWARE_PREFIX) ; \
