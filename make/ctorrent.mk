...
#
# Binaries should be installed into $(CTORRENT_IPK_DIR)$(OPTWARE_PREFIX)sbin or $(CTORRENT_IPK_DIR)$(OPTWARE_PREFIX)bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(CTORRENT_IPK_DIR)$(OPTWARE_PREFIX){lib,include}
# Configuration files should be installed in $(CTORRENT_IPK_DIR)$(OPTWARE_PREFIX)etc/ctorrent/...
# Documentation files should be installed in $(CTORRENT_IPK_DIR)$(OPTWARE_PREFIX)doc/ctorrent/...
# Daemon startup scripts should be installed in $(CTORRENT_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/S??ctorrent
#
# You may need to patch your application to make it use these locations.
#
$(CTORRENT_IPK): $(CTORRENT_BUILD_DIR)/.built
	rm -rf $(CTORRENT_IPK_DIR) $(CTORRENT_IPK)
	install -d $(CTORRENT_IPK_DIR)$(OPTWARE_PREFIX)bin
	$(STRIP_COMMAND) $(CTORRENT_BUILD_DIR)/ctorrent -o $(CTORRENT_IPK_DIR)$(OPTWARE_PREFIX)bin/ctorrent
#	install -d $(CTORRENT_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d
#	install -m 755 $(CTORRENT_SOURCE_DIR)/rc.ctorrent $(CTORRENT_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/SXXctorrent
