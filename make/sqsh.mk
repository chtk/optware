...
SQSH_CONFFILES=$(OPTWARE_PREFIX)etc/sqshrc
...
#
# Binaries should be installed into $(SQSH_IPK_DIR)$(OPTWARE_PREFIX)sbin or $(SQSH_IPK_DIR)$(OPTWARE_PREFIX)bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(SQSH_IPK_DIR)$(OPTWARE_PREFIX){lib,include}
# Configuration files should be installed in $(SQSH_IPK_DIR)$(OPTWARE_PREFIX)etc/sqsh/...
# Documentation files should be installed in $(SQSH_IPK_DIR)$(OPTWARE_PREFIX)doc/sqsh/...
# Daemon startup scripts should be installed in $(SQSH_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/S??sqsh
#
# You may need to patch your application to make it use these locations.
#
$(SQSH_IPK): $(SQSH_BUILD_DIR)/.built
	rm -rf $(SQSH_IPK_DIR) $(BUILD_DIR)/sqsh_*_$(TARGET_ARCH).ipk
	$(MAKE) -C $(SQSH_BUILD_DIR) prefix=$(SQSH_IPK_DIR)$(OPTWARE_PREFIX)install install.man
	$(STRIP_COMMAND) $(SQSH_IPK_DIR)$(OPTWARE_PREFIX)bin/sqsh
#	install -d $(SQSH_IPK_DIR)$(OPTWARE_PREFIX)etc/
#	install -m 644 $(SQSH_SOURCE_DIR)/sqsh.conf $(SQSH_IPK_DIR)$(OPTWARE_PREFIX)etc/sqsh.conf
#	install -d $(SQSH_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d
#	install -m 755 $(SQSH_SOURCE_DIR)/rc.sqsh $(SQSH_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/SXXsqsh
