...
$(PSUTILS_BUILD_DIR)/.configured: $(DL_DIR)/$(PSUTILS_SOURCE) $(PSUTILS_PATCHES)
	...
	(cd $(PSUTILS_BUILD_DIR); \
		...
		INCLUDEDIR=$(OPTWARE_PREFIX)share/psutils \
...
#
# Binaries should be installed into $(PSUTILS_IPK_DIR)$(OPTWARE_PREFIX)sbin or $(PSUTILS_IPK_DIR)$(OPTWARE_PREFIX)bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(PSUTILS_IPK_DIR)$(OPTWARE_PREFIX){lib,include}
# Configuration files should be installed in $(PSUTILS_IPK_DIR)$(OPTWARE_PREFIX)etc/psutils/...
# Documentation files should be installed in $(PSUTILS_IPK_DIR)$(OPTWARE_PREFIX)doc/psutils/...
# Daemon startup scripts should be installed in $(PSUTILS_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/S??psutils
#
# You may need to patch your application to make it use these locations.
#
$(PSUTILS_IPK): $(PSUTILS_BUILD_DIR)/.built
	rm -rf $(PSUTILS_IPK_DIR) $(BUILD_DIR)/psutils_*_$(TARGET_ARCH).ipk
	mkdir -p $(PSUTILS_IPK_DIR)$(OPTWARE_PREFIX)bin $(PSUTILS_IPK_DIR)$(OPTWARE_PREFIX)share/man/man1 \
		$(PSUTILS_IPK_DIR)$(OPTWARE_PREFIX)share/psutils
	BINDIR=$(PSUTILS_IPK_DIR)$(OPTWARE_PREFIX)bin \
		INCLUDEDIR=$(PSUTILS_IPK_DIR)$(OPTWARE_PREFIX)share/psutils \
		MANDIR=$(PSUTILS_IPK_DIR)$(OPTWARE_PREFIX)share/man/man1/ \
		$(MAKE) -C $(PSUTILS_BUILD_DIR) -e -f $(PSUTILS_BUILD_DIR)/Makefile.unix install
	$(STRIP_COMMAND) $(PSUTILS_IPK_DIR)$(OPTWARE_PREFIX)bin/psbook  \
		$(PSUTILS_IPK_DIR)$(OPTWARE_PREFIX)bin/psselect \
		$(PSUTILS_IPK_DIR)$(OPTWARE_PREFIX)bin/pstops \
		$(PSUTILS_IPK_DIR)$(OPTWARE_PREFIX)bin/epsffit \
		$(PSUTILS_IPK_DIR)$(OPTWARE_PREFIX)bin/psnup \
		$(PSUTILS_IPK_DIR)$(OPTWARE_PREFIX)bin/psresize 

	#install -m 755 $(PSUTILS_SOURCE_DIR)/rc.psutils $(PSUTILS_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/SXXpsutils
