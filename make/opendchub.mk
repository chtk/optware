...
#OPENDCHUB_CONFFILES=$(OPTWARE_PREFIX)etc/opendchub.conf $(OPTWARE_PREFIX)etc/init.d/SXXopendchub
...
#
# Binaries should be installed into $(OPENDCHUB_IPK_DIR)$(OPTWARE_PREFIX)sbin or $(OPENDCHUB_IPK_DIR)$(OPTWARE_PREFIX)bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(OPENDCHUB_IPK_DIR)$(OPTWARE_PREFIX){lib,include}
# Configuration files should be installed in $(OPENDCHUB_IPK_DIR)$(OPTWARE_PREFIX)etc/opendchub/...
# Documentation files should be installed in $(OPENDCHUB_IPK_DIR)$(OPTWARE_PREFIX)doc/opendchub/...
# Daemon startup scripts should be installed in $(OPENDCHUB_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/S??opendchub
#
...
$(OPENDCHUB_IPK): $(OPENDCHUB_BUILD_DIR)/.built
	...
	$(STRIP_COMMAND) $(OPENDCHUB_IPK_DIR)$(OPTWARE_PREFIX)*bin/*
	install -d $(OPENDCHUB_IPK_DIR)$(OPTWARE_PREFIX)share/doc/opendchub
	install $(OPENDCHUB_BUILD_DIR)/[ACRN]* \
		$(OPENDCHUB_BUILD_DIR)/Documentation/* \
		$(OPENDCHUB_BUILD_DIR)/Samplescripts/* \
		$(OPENDCHUB_IPK_DIR)$(OPTWARE_PREFIX)share/doc/opendchub/
#	install -d $(OPENDCHUB_IPK_DIR)$(OPTWARE_PREFIX)etc/
#	install -m 644 $(OPENDCHUB_SOURCE_DIR)/opendchub.conf $(OPENDCHUB_IPK_DIR)$(OPTWARE_PREFIX)etc/opendchub.conf
#	install -d $(OPENDCHUB_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d
#	install -m 755 $(OPENDCHUB_SOURCE_DIR)/rc.opendchub $(OPENDCHUB_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/SXXopendchub
#	sed -i -e '/^#!/aOPTWARE_TARGET=${OPTWARE_TARGET}' $(OPENDCHUB_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/SXXopendchub
