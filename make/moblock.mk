...
#MOBLOCK_CONFFILES=$(OPTWARE_PREFIX)etc/moblock.conf $(OPTWARE_PREFIX)etc/init.d/SXXmoblock
...
#
# Binaries should be installed into $(MOBLOCK_IPK_DIR)$(OPTWARE_PREFIX)sbin or $(MOBLOCK_IPK_DIR)$(OPTWARE_PREFIX)bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(MOBLOCK_IPK_DIR)$(OPTWARE_PREFIX){lib,include}
# Configuration files should be installed in $(MOBLOCK_IPK_DIR)$(OPTWARE_PREFIX)etc/moblock/...
# Documentation files should be installed in $(MOBLOCK_IPK_DIR)$(OPTWARE_PREFIX)doc/moblock/...
# Daemon startup scripts should be installed in $(MOBLOCK_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/S??moblock
#
# You may need to patch your application to make it use these locations.
#
$(MOBLOCK_IPK): $(MOBLOCK_BUILD_DIR)/.built
	rm -rf $(MOBLOCK_IPK_DIR) $(BUILD_DIR)/moblock_*_$(TARGET_ARCH).ipk
	install -d $(MOBLOCK_IPK_DIR)$(OPTWARE_PREFIX)bin
	$(MAKE) -C $(MOBLOCK_BUILD_DIR) DESTDIR=$(MOBLOCK_IPK_DIR) install
	install -d $(MOBLOCK_IPK_DIR)$(OPTWARE_PREFIX)share/doc/moblock
	install -m 644 $(MOBLOCK_BUILD_DIR)/README $(MOBLOCK_IPK_DIR)$(OPTWARE_PREFIX)share/doc/moblock
	install -m 644 $(MOBLOCK_BUILD_DIR)/MoBlock-nfq.sh $(MOBLOCK_IPK_DIR)$(OPTWARE_PREFIX)bin
#	install -m 644 $(MOBLOCK_SOURCE_DIR)/moblock.conf $(MOBLOCK_IPK_DIR)$(OPTWARE_PREFIX)etc/moblock.conf
#	install -d $(MOBLOCK_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d
#	install -m 755 $(MOBLOCK_SOURCE_DIR)/rc.moblock $(MOBLOCK_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/SXXmoblock
#	sed -i -e '/^#!/aOPTWARE_TARGET=${OPTWARE_TARGET}' $(MOBLOCK_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/SXXmoblock
