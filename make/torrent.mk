...
TORRENT_CONFFILES=$(OPTWARE_PREFIX)etc/torrent.conf $(OPTWARE_PREFIX)etc/init.d/S80busybox_httpd
...
#
# Binaries should be installed into $(TORRENT_IPK_DIR)$(OPTWARE_PREFIX)sbin or $(TORRENT_IPK_DIR)$(OPTWARE_PREFIX)bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(TORRENT_IPK_DIR)$(OPTWARE_PREFIX){lib,include}
# Configuration files should be installed in $(TORRENT_IPK_DIR)$(OPTWARE_PREFIX)etc/torrent/...
# Documentation files should be installed in $(TORRENT_IPK_DIR)$(OPTWARE_PREFIX)doc/torrent/...
# Daemon startup scripts should be installed in $(TORRENT_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/S??torrent
#
...
$(TORRENT_IPK): $(TORRENT_BUILD_DIR)/.configured
	...
	install -d $(TORRENT_IPK_DIR)$(OPTWARE_PREFIX)bin
	install -d $(TORRENT_IPK_DIR)$(OPTWARE_PREFIX)sbin
	install -d $(TORRENT_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d
	install -d $(TORRENT_IPK_DIR)$(OPTWARE_PREFIX)share/www/cgi-bin
	install -m 700 $(SOURCE_DIR)/torrent/torrent_watchdog $(TORRENT_IPK_DIR)$(OPTWARE_PREFIX)sbin
	install -m 755 $(SOURCE_DIR)/torrent/torrent_admin $(TORRENT_IPK_DIR)$(OPTWARE_PREFIX)sbin
	install -m 755 $(SOURCE_DIR)/torrent/btcheck-target $(TORRENT_IPK_DIR)$(OPTWARE_PREFIX)bin
	install -m 755 $(SOURCE_DIR)/torrent/torrent.cgi $(TORRENT_IPK_DIR)$(OPTWARE_PREFIX)share/www/cgi-bin
	install -m 644 $(SOURCE_DIR)/torrent/torrent.conf $(TORRENT_IPK_DIR)$(OPTWARE_PREFIX)etc
	install -m 755 $(SOURCE_DIR)/torrent/S80busybox_httpd $(TORRENT_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d
