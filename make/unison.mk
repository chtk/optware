...
#UNISON_CONFFILES=$(OPTWARE_PREFIX)etc/unison.conf $(OPTWARE_PREFIX)etc/init.d/SXXunison
...
$(UNISON_BUILD_DIR)/.configured: $(DL_DIR)/$(UNISON_SOURCE) $(UNISON_PATCHES)
	...
	sed -i -e 's|-cclib -lutil|& -cclib -Wl,-rpath,$(OPTWARE_PREFIX)lib|' $(@D)/Makefile.OCaml
	touch $@
...
#
# Binaries should be installed into $(UNISON_IPK_DIR)$(OPTWARE_PREFIX)sbin or $(UNISON_IPK_DIR)$(OPTWARE_PREFIX)bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(UNISON_IPK_DIR)$(OPTWARE_PREFIX){lib,include}
# Configuration files should be installed in $(UNISON_IPK_DIR)$(OPTWARE_PREFIX)etc/unison/...
# Documentation files should be installed in $(UNISON_IPK_DIR)$(OPTWARE_PREFIX)doc/unison/...
# Daemon startup scripts should be installed in $(UNISON_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/S??unison
#
# You may need to patch your application to make it use these locations.
#
$(UNISON_IPK): $(UNISON_BUILD_DIR)/.built
	rm -rf $(UNISON_IPK_DIR) $(BUILD_DIR)/unison_*_$(TARGET_ARCH).ipk
	install -d $(UNISON_IPK_DIR)$(OPTWARE_PREFIX)bin
	install -m 755 $(UNISON_BUILD_DIR)/unison $(UNISON_IPK_DIR)$(OPTWARE_PREFIX)bin
