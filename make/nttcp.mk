...
#
NTTCP_CPPFLAGS=
NTTCP_LDFLAGS=-Wl,-rpath,$(OPTWARE_PREFIX)lib
...
#
# Binaries should be installed into $(NTTCP_IPK_DIR)$(OPTWARE_PREFIX)sbin or $(NTTCP_IPK_DIR)$(OPTWARE_PREFIX)bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(NTTCP_IPK_DIR)$(OPTWARE_PREFIX){lib,include}
# Configuration files should be installed in $(NTTCP_IPK_DIR)$(OPTWARE_PREFIX)etc/nttcp/...
# Documentation files should be installed in $(NTTCP_IPK_DIR)$(OPTWARE_PREFIX)doc/nttcp/...
# Daemon startup scripts should be installed in $(NTTCP_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/S??nttcp
#
# You may need to patch your application to make it use these locations.
#
$(NTTCP_IPK): $(NTTCP_BUILD_DIR)/.built
	rm -rf $(NTTCP_IPK_DIR) $(BUILD_DIR)/nttcp_*_$(TARGET_ARCH).ipk
	install -d $(NTTCP_IPK_DIR)$(OPTWARE_PREFIX)bin
	install $(NTTCP_BUILD_DIR)/nttcp $(NTTCP_IPK_DIR)$(OPTWARE_PREFIX)bin/nttcp
	$(TARGET_STRIP) $(NTTCP_IPK_DIR)$(OPTWARE_PREFIX)bin/nttcp
	install -d $(NTTCP_IPK_DIR)$(OPTWARE_PREFIX)man/man1
	install -m 644 $(NTTCP_BUILD_DIR)/nttcp.1 $(NTTCP_IPK_DIR)$(OPTWARE_PREFIX)man/man1/nttcp.1
