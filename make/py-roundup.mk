...
#PY-ROUNDUP_CONFFILES=$(OPTWARE_PREFIX)etc/py-roundup.conf $(OPTWARE_PREFIX)etc/init.d/SXXpy-roundup
...
PY26-ROUNDUP_IPK_DIR=$(BUILD_DIR)/py26-roundup-$(PY-ROUNDUP_VERSION)-ipk
PY26-ROUNDUP_IPK=$(BUILD_DIR)/py26-roundup_$(PY-ROUNDUP_VERSION)-$(PY-ROUNDUP_IPK_VERSION)_$(TARGET_ARCH).ipk
...
#
$(PY-ROUNDUP_BUILD_DIR)/.configured: $(DL_DIR)/$(PY-ROUNDUP_SOURCE) $(PY-ROUNDUP_PATCHES) make/py-roundup.mk
...
$(PY26-ROUNDUP_IPK_DIR)/CONTROL/control:
	...
	@echo "Source: $(PY-ROUNDUP_SITE)/$(PY-ROUNDUP_SOURCE)" >>$@
	@echo "Description: $(PY-ROUNDUP_DESCRIPTION)" >>$@
	@echo "Depends: $(PY26-ROUNDUP_DEPENDS)" >>$@
	@echo "Conflicts: $(PY-ROUNDUP_CONFLICTS)" >>$@

#
# This builds the IPK file.
#
# Binaries should be installed into $(PY-ROUNDUP_IPK_DIR)$(OPTWARE_PREFIX)sbin or $(PY-ROUNDUP_IPK_DIR)$(OPTWARE_PREFIX)bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(PY-ROUNDUP_IPK_DIR)$(OPTWARE_PREFIX){lib,include}
# Configuration files should be installed in $(PY-ROUNDUP_IPK_DIR)$(OPTWARE_PREFIX)etc/py-roundup/...
# Documentation files should be installed in $(PY-ROUNDUP_IPK_DIR)$(OPTWARE_PREFIX)doc/py-roundup/...
# Daemon startup scripts should be installed in $(PY-ROUNDUP_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/S??py-roundup
#
...
$(PY25-ROUNDUP_IPK): $(PY-ROUNDUP_BUILD_DIR)/.built
	...
	install -d $(PY25-ROUNDUP_IPK_DIR)$(OPTWARE_PREFIX)man $(PY25-ROUNDUP_IPK_DIR)$(OPTWARE_PREFIX)share
	(cd $(PY-ROUNDUP_BUILD_DIR)/2.5; \
	 CC='$(TARGET_CC)' LDSHARED='$(TARGET_CC) -shared' \
	    $(HOST_STAGING_PREFIX)/bin/python2.5 setup.py install \
	    --root=$(PY25-ROUNDUP_IPK_DIR) --prefix=/opt)
	sed -i -e '1s|^#!.*|#! $(OPTWARE_PREFIX)bin/python2.5|' $(PY25-ROUNDUP_IPK_DIR)$(OPTWARE_PREFIX)bin/*
#	$(STRIP_COMMAND) `find $(PY25-ROUNDUP_IPK_DIR)$(OPTWARE_PREFIX)lib/python2.5/site-packages -name '*.so'`
	rm -rf $(PY25-ROUNDUP_IPK_DIR)$(OPTWARE_PREFIX)man $(PY25-ROUNDUP_IPK_DIR)$(OPTWARE_PREFIX)share
...
$(PY26-ROUNDUP_IPK) $(PY-ROUNDUP-COMMON_IPK): $(PY-ROUNDUP_BUILD_DIR)/.built
	...
	install -d $(PY26-ROUNDUP_IPK_DIR)$(OPTWARE_PREFIX)man $(PY26-ROUNDUP_IPK_DIR)$(OPTWARE_PREFIX)share
	(cd $(PY-ROUNDUP_BUILD_DIR)/2.6; \
	 CC='$(TARGET_CC)' LDSHARED='$(TARGET_CC) -shared' \
	    $(HOST_STAGING_PREFIX)/bin/python2.6 setup.py install \
	    --root=$(PY26-ROUNDUP_IPK_DIR) --prefix=/opt)
	sed -i -e '1s|^#!.*|#! $(OPTWARE_PREFIX)bin/python2.6|' $(PY26-ROUNDUP_IPK_DIR)$(OPTWARE_PREFIX)bin/*
	for f in $(PY26-ROUNDUP_IPK_DIR)$(OPTWARE_PREFIX)bin/*; \
		do mv $$f `echo $$f | sed 's|$$|-2.6|'`; done
#	$(STRIP_COMMAND) `find $(PY26-ROUNDUP_IPK_DIR)$(OPTWARE_PREFIX)lib/python2.6/site-packages -name '*.so'`
	mkdir -p $(PY-ROUNDUP-COMMON_IPK_DIR)/opt
	mv $(PY26-ROUNDUP_IPK_DIR)$(OPTWARE_PREFIX)share $(PY26-ROUNDUP_IPK_DIR)$(OPTWARE_PREFIX)man $(PY-ROUNDUP-COMMON_IPK_DIR)$(OPTWARE_PREFIX)
	...
	cd $(BUILD_DIR); $(IPKG_BUILD) $(PY26-ROUNDUP_IPK_DIR)

#
# This is called from the top level makefile to create the IPK file.
#
py-roundup-ipk: $(PY25-ROUNDUP_IPK) $(PY26-ROUNDUP_IPK) $(PY-ROUNDUP-COMMON_IPK)
...
py-roundup-dirclean:
	...
	rm -rf $(PY26-ROUNDUP_IPK_DIR) $(PY26-ROUNDUP_IPK)

#
# Some sanity check for the package.
#
py-roundup-check: $(PY25-ROUNDUP_IPK) $(PY26-ROUNDUP_IPK) $(PY-ROUNDUP-COMMON_IPK)
