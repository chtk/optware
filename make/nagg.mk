...
NAGG_CONFFILES=$(OPTWARE_PREFIX)lib/nagg/nagg.conf $(OPTWARE_PREFIX)lib/nagg/nagg.css \
	$(OPTWARE_PREFIX)lib/nagg/indextemplate.html $(OPTWARE_PREFIX)lib/nagg/slidetemplate.html
...
#	$(MAKE) <bar>-stage <baz>-stage
	...
	(cd $(NAGG_BUILD_DIR); \
		...
		sed -i -e '/^libdir=/s|.*|libdir=$(OPTWARE_PREFIX)lib/nagg|' \
			-e 's|/bin/bash|$(OPTWARE_PREFIX)bin/bash|' nagg \
	)
	touch $@
...
#
# Binaries should be installed into $(NAGG_IPK_DIR)$(OPTWARE_PREFIX)sbin or $(NAGG_IPK_DIR)$(OPTWARE_PREFIX)bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(NAGG_IPK_DIR)$(OPTWARE_PREFIX){lib,include}
# Configuration files should be installed in $(NAGG_IPK_DIR)$(OPTWARE_PREFIX)etc/nagg/...
# Documentation files should be installed in $(NAGG_IPK_DIR)$(OPTWARE_PREFIX)doc/nagg/...
# Daemon startup scripts should be installed in $(NAGG_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/S??nagg
#
# You may need to patch your application to make it use these locations.
#
$(NAGG_IPK): $(NAGG_BUILD_DIR)/.built
	rm -rf $(NAGG_IPK_DIR) $(BUILD_DIR)/nagg_*_$(TARGET_ARCH).ipk
	$(MAKE) -C $(NAGG_BUILD_DIR) DESTDIR=$(NAGG_IPK_DIR)$(OPTWARE_PREFIX)install
#	install -d $(NAGG_IPK_DIR)$(OPTWARE_PREFIX)etc/
#	install -m 644 $(NAGG_SOURCE_DIR)/nagg.conf $(NAGG_IPK_DIR)$(OPTWARE_PREFIX)etc/nagg.conf
#	install -d $(NAGG_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d
#	install -m 755 $(NAGG_SOURCE_DIR)/rc.nagg $(NAGG_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/SXXnagg
#	sed -i -e '/^#!/aOPTWARE_TARGET=${OPTWARE_TARGET}' $(NAGG_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/SXXnagg
