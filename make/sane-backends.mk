...
SANE_BACKENDS_CONFFILES=$(OPTWARE_PREFIX)etc/sane.d/saned.conf $(OPTWARE_PREFIX)etc/init.d/S01sane-backends
...
SANE_BACKENDS_LDFLAGS=-Wl,-rpath=$(OPTWARE_PREFIX)lib/sane -ldl -lpthread -ltiff -lz
...
#
# Binaries should be installed into $(SANE_BACKENDS_IPK_DIR)$(OPTWARE_PREFIX)sbin or $(SANE_BACKENDS_IPK_DIR)$(OPTWARE_PREFIX)bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(SANE_BACKENDS_IPK_DIR)$(OPTWARE_PREFIX){lib,include}
# Configuration files should be installed in $(SANE_BACKENDS_IPK_DIR)$(OPTWARE_PREFIX)etc/sane-backends/...
# Documentation files should be installed in $(SANE_BACKENDS_IPK_DIR)$(OPTWARE_PREFIX)doc/sane-backends/...
# Daemon startup scripts should be installed in $(SANE_BACKENDS_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/S??sane-backends
#
...
$(SANE_BACKENDS_IPK): $(SANE_BACKENDS_BUILD_DIR)/.built
	...
	rm -rf $(SANE_BACKENDS_IPK_DIR)$(OPTWARE_PREFIX)lib/libsane.la
	$(STRIP_COMMAND) $(SANE_BACKENDS_IPK_DIR)$(OPTWARE_PREFIX)bin/gamma4scanimage
	$(STRIP_COMMAND) $(SANE_BACKENDS_IPK_DIR)$(OPTWARE_PREFIX)bin/sane-find-scanner
	$(STRIP_COMMAND) $(SANE_BACKENDS_IPK_DIR)$(OPTWARE_PREFIX)bin/scanimage
	$(STRIP_COMMAND) $(SANE_BACKENDS_IPK_DIR)$(OPTWARE_PREFIX)sbin/saned
	$(STRIP_COMMAND) $(SANE_BACKENDS_IPK_DIR)$(OPTWARE_PREFIX)lib/sane/*.so.*
	$(STRIP_COMMAND) $(SANE_BACKENDS_IPK_DIR)$(OPTWARE_PREFIX)lib/*.so.*
	rm -rf $(SANE_BACKENDS_IPK_DIR)$(OPTWARE_PREFIX)lib/sane/*.la
	install -d $(SANE_BACKENDS_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d
	install -m 755 $(SANE_BACKENDS_SOURCE_DIR)/rc.sane-backends $(SANE_BACKENDS_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/S01sane-backends
