...
#PY-DUPLICITY_CONFFILES=$(OPTWARE_PREFIX)etc/py-duplicity.conf $(OPTWARE_PREFIX)etc/init.d/SXXpy-duplicity
...
#
# Binaries should be installed into $(PY-DUPLICITY_IPK_DIR)$(OPTWARE_PREFIX)sbin or $(PY-DUPLICITY_IPK_DIR)$(OPTWARE_PREFIX)bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(PY-DUPLICITY_IPK_DIR)$(OPTWARE_PREFIX){lib,include}
# Configuration files should be installed in $(PY-DUPLICITY_IPK_DIR)$(OPTWARE_PREFIX)etc/py-duplicity/...
# Documentation files should be installed in $(PY-DUPLICITY_IPK_DIR)$(OPTWARE_PREFIX)doc/py-duplicity/...
# Daemon startup scripts should be installed in $(PY-DUPLICITY_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/S??py-duplicity
#
...
$(PY25-DUPLICITY_IPK) $(PY26-DUPLICITY_IPK) $(PY-DUPLICITY-DOC_IPK): $(PY-DUPLICITY_BUILD_DIR)/.built
	...
	$(STRIP_COMMAND) $(PY25-DUPLICITY_IPK_DIR)$(OPTWARE_PREFIX)lib/python2.5/site-packages/duplicity/*.so
	for f in $(PY25-DUPLICITY_IPK_DIR)$(OPTWARE_PREFIX)*bin/*; \
		do mv $$f `echo $$f | sed 's|$$|-py2.5|'`; done
	$(MAKE) $(PY25-DUPLICITY_IPK_DIR)/CONTROL/control
	rm -rf $(PY25-DUPLICITY_IPK_DIR)$(OPTWARE_PREFIX)share
	...
	$(STRIP_COMMAND) $(PY26-DUPLICITY_IPK_DIR)$(OPTWARE_PREFIX)lib/python2.6/site-packages/duplicity/*.so
	...
	mv $(PY26-DUPLICITY_IPK_DIR)$(OPTWARE_PREFIX)share $(PY-DUPLICITY-DOC_IPK_DIR)/opt
