...
GNUPLOT_DEPENDS=expat, libgd, libstdc++, lua, ncurses, readline, zlib
GNUPLOT_SUGGESTS=
GNUPLOT_CONFLICTS=
...
#GNUPLOT_CONFFILES=$(OPTWARE_PREFIX)etc/gnuplot.conf $(OPTWARE_PREFIX)etc/init.d/SXXgnuplot
...
$(GNUPLOT_BUILD_DIR)/.configured: $(DL_DIR)/$(GNUPLOT_SOURCE) $(GNUPLOT_PATCHES) make/gnuplot.mk
	$(MAKE) expat-stage libpng-stage libgd-stage lua-stage ncurses-stage readline-stage zlib-stage
ifeq (pango, $(filter pango, $(PACKAGES)))
	$(MAKE) pango-stage
endif
...
$(GNUPLOT_BUILD_DIR)/.built: $(GNUPLOT_BUILD_DIR)/.configured
	rm -f $@
ifneq (, $(filter cs05q1armel cs05q3armel ds101 fsg3v4 gumstix1151 mbwe-bluering syno-e500, $(OPTWARE_TARGET)))
# no optimization
...
#
# Binaries should be installed into $(GNUPLOT_IPK_DIR)$(OPTWARE_PREFIX)sbin or $(GNUPLOT_IPK_DIR)$(OPTWARE_PREFIX)bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(GNUPLOT_IPK_DIR)$(OPTWARE_PREFIX){lib,include}
# Configuration files should be installed in $(GNUPLOT_IPK_DIR)$(OPTWARE_PREFIX)etc/gnuplot/...
# Documentation files should be installed in $(GNUPLOT_IPK_DIR)$(OPTWARE_PREFIX)doc/gnuplot/...
# Daemon startup scripts should be installed in $(GNUPLOT_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/S??gnuplot
#
...
#	install -d $(GNUPLOT_IPK_DIR)$(OPTWARE_PREFIX)etc/
#	install -m 644 $(GNUPLOT_SOURCE_DIR)/gnuplot.conf $(GNUPLOT_IPK_DIR)$(OPTWARE_PREFIX)etc/gnuplot.conf
#	install -d $(GNUPLOT_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d
#	install -m 755 $(GNUPLOT_SOURCE_DIR)/rc.gnuplot $(GNUPLOT_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/SXXgnuplot
