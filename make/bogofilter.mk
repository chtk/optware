...
#	install -m 755 $(BOGOFILTER_SOURCE_DIR)/prerm $(BOGOFILTER_IPK_DIR)/CONTROL/prerm
	sed -i -e "s,/opt/,$(OPTWARE_PREFIX)/,g" \
		$(BOGOFILTER_IPK_DIR)$(OPTWARE_PREFIX)/sbin/postfix-bogofilter.sh \
		$(BOGOFILTER_IPK_DIR)$(OPTWARE_PREFIX)/doc/bogofilter/master.cf.patch \
		$(BOGOFILTER_IPK_DIR)$(OPTWARE_PREFIX)/etc/bogofilter.conf \
		$(BOGOFILTER_IPK_DIR)/CONTROL/postinst
