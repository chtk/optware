...
THTTPD_CONFFILES=$(OPTWARE_PREFIX)etc/init.d/S80thttpd $(OPTWARE_PREFIX)etc/thttpd.conf
...
#
# Binaries should be installed into $(THTTPD_IPK_DIR)$(OPTWARE_PREFIX)sbin or $(THTTPD_IPK_DIR)$(OPTWARE_PREFIX)bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(THTTPD_IPK_DIR)$(OPTWARE_PREFIX){lib,include}
# Configuration files should be installed in $(THTTPD_IPK_DIR)$(OPTWARE_PREFIX)etc/thttpd/...
# Documentation files should be installed in $(THTTPD_IPK_DIR)$(OPTWARE_PREFIX)doc/thttpd/...
# Daemon startup scripts should be installed in $(THTTPD_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/S??thttpd
#
...
$(THTTPD_IPK): $(THTTPD_BUILD_DIR)/.built
	...
	chmod +w $(THTTPD_IPK_DIR)$(OPTWARE_PREFIX)sbin/thttpd && \
	$(STRIP_COMMAND) $(THTTPD_IPK_DIR)$(OPTWARE_PREFIX)sbin/thttpd && \
	chmod -w $(THTTPD_IPK_DIR)$(OPTWARE_PREFIX)sbin/thttpd && \
	$(STRIP_COMMAND) $(THTTPD_IPK_DIR)$(OPTWARE_PREFIX)sbin/makeweb
	$(STRIP_COMMAND) $(THTTPD_IPK_DIR)$(OPTWARE_PREFIX)sbin/htpasswd
	$(STRIP_COMMAND) $(THTTPD_IPK_DIR)$(OPTWARE_PREFIX)share/www/cgi-bin/*
	install -d $(THTTPD_IPK_DIR)$(OPTWARE_PREFIX)etc/
	install -m 644 $(THTTPD_SOURCE_DIR)/thttpd.conf $(THTTPD_IPK_DIR)$(OPTWARE_PREFIX)etc/thttpd.conf
	install -d $(THTTPD_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d
	install -m 755 $(THTTPD_SOURCE_DIR)/rc.thttpd $(THTTPD_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/S80thttpd
