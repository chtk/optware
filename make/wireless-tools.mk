...
#WIRELESS-TOOLS_CONFFILES=$(OPTWARE_PREFIX)etc/wireless-tools.conf $(OPTWARE_PREFIX)etc/init.d/SXXwireless-tools
...
$(WIRELESS-TOOLS_BUILD_DIR)/.built: $(WIRELESS-TOOLS_BUILD_DIR)/.configured
	...
	$(MAKE) -C $(WIRELESS-TOOLS_BUILD_DIR) CC=$(TARGET_CC) PREFIX=$(OPTWARE_PREFIX)LDFLAGS=-Wl,-rpath=$(OPTWARE_PREFIX)lib
...
#
# Binaries should be installed into $(WIRELESS-TOOLS_IPK_DIR)$(OPTWARE_PREFIX)sbin or $(WIRELESS-TOOLS_IPK_DIR)$(OPTWARE_PREFIX)bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(WIRELESS-TOOLS_IPK_DIR)$(OPTWARE_PREFIX){lib,include}
# Configuration files should be installed in $(WIRELESS-TOOLS_IPK_DIR)$(OPTWARE_PREFIX)etc/wireless-tools/...
# Documentation files should be installed in $(WIRELESS-TOOLS_IPK_DIR)$(OPTWARE_PREFIX)doc/wireless-tools/...
# Daemon startup scripts should be installed in $(WIRELESS-TOOLS_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/S??wireless-tools
#
...
$(WIRELESS-TOOLS_IPK): $(WIRELESS-TOOLS_BUILD_DIR)/.built
	...
	$(MAKE) -C $(WIRELESS-TOOLS_BUILD_DIR) PREFIX=$(WIRELESS-TOOLS_IPK_DIR)$(OPTWARE_PREFIX)install
	$(STRIP_COMMAND) $(WIRELESS-TOOLS_IPK_DIR)$(OPTWARE_PREFIX)bin/*
	$(STRIP_COMMAND) $(WIRELESS-TOOLS_IPK_DIR)$(OPTWARE_PREFIX)sbin/*
	$(STRIP_COMMAND) $(WIRELESS-TOOLS_IPK_DIR)$(OPTWARE_PREFIX)lib/*
	#install -d $(WIRELESS-TOOLS_IPK_DIR)$(OPTWARE_PREFIX)etc/
	#install -m 644 $(WIRELESS-TOOLS_SOURCE_DIR)/wireless-tools.conf $(WIRELESS-TOOLS_IPK_DIR)$(OPTWARE_PREFIX)etc/wireless-tools.conf
	#install -d $(WIRELESS-TOOLS_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d
	#install -m 755 $(WIRELESS-TOOLS_SOURCE_DIR)/rc.wireless-tools $(WIRELESS-TOOLS_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/SXXwireless-tools
