...
APCUPSD_CONFFILES=$(OPTWARE_PREFIX)etc/apcupsd/apcupsd.conf
#$(OPTWARE_PREFIX)etc/init.d/SXXapcupsd
...
#	$(MAKE) tcpwrappers-stage
	...
	sed -i -e 's|prefix=NONE|prefix=/opt|; s|/usr/share/hal/|$(OPTWARE_PREFIX)share/hal/|' $(@D)/configure
	...
	(cd $(@D); \
		...
		--sysconfdir=$(OPTWARE_PREFIX)etc/apcupsd \
		--mandir=$(OPTWARE_PREFIX)share/man \
		--with-nologin=$(OPTWARE_PREFIX)etc/apcupsd \
		--with-pid-dir=$(OPTWARE_PREFIX)var/run \
		--with-log-dir=$(OPTWARE_PREFIX)var/log \
		--with-lock-dir=$(OPTWARE_PREFIX)var/lock \
		--enable-usb \
		--enable-cgi \
		--with-cgi-bin=$(OPTWARE_PREFIX)share/www/cgi-bin \
		--without-libwrap \
		--without-x \
		--disable-nls \
		--disable-static \
...
#
# Binaries should be installed into $(APCUPSD_IPK_DIR)$(OPTWARE_PREFIX)sbin or $(APCUPSD_IPK_DIR)$(OPTWARE_PREFIX)bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(APCUPSD_IPK_DIR)$(OPTWARE_PREFIX){lib,include}
# Configuration files should be installed in $(APCUPSD_IPK_DIR)$(OPTWARE_PREFIX)etc/apcupsd/...
# Documentation files should be installed in $(APCUPSD_IPK_DIR)$(OPTWARE_PREFIX)doc/apcupsd/...
# Daemon startup scripts should be installed in $(APCUPSD_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/S??apcupsd
#
...
$(APCUPSD_IPK) $(APCUPSD-CGI_IPK): $(APCUPSD_BUILD_DIR)/.built
	...
	$(STRIP_COMMAND) $(APCUPSD_IPK_DIR)$(OPTWARE_PREFIX)sbin/* $(APCUPSD_IPK_DIR)$(OPTWARE_PREFIX)share/www/cgi-bin/*
	install -d $(APCUPSD-CGI_IPK_DIR)$(OPTWARE_PREFIX)share $(APCUPSD-CGI_IPK_DIR)$(OPTWARE_PREFIX)etc/apcupsd
	mv $(APCUPSD_IPK_DIR)$(OPTWARE_PREFIX)share/www $(APCUPSD-CGI_IPK_DIR)$(OPTWARE_PREFIX)share/
	mv $(APCUPSD_IPK_DIR)$(OPTWARE_PREFIX)etc/apcupsd/*.css $(APCUPSD-CGI_IPK_DIR)$(OPTWARE_PREFIX)etc/apcupsd/
