...
#
# Binaries should be installed into $(NETHACK_IPK_DIR)$(OPTWARE_PREFIX)sbin or $(NETHACK_IPK_DIR)$(OPTWARE_PREFIX)bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(NETHACK_IPK_DIR)$(OPTWARE_PREFIX){lib,include}
# Configuration files should be installed in $(NETHACK_IPK_DIR)$(OPTWARE_PREFIX)etc/nethack/...
# Documentation files should be installed in $(NETHACK_IPK_DIR)$(OPTWARE_PREFIX)doc/nethack/...
# Daemon startup scripts should be installed in $(NETHACK_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/S??nethack
#
# You may need to patch your application to make it use these locations.
#
$(NETHACK_IPK): $(NETHACK_BUILD_DIR)/.built
	rm -rf $(NETHACK_IPK_DIR) $(BUILD_DIR)/nethack_*_$(TARGET_ARCH).ipk
	install -d $(NETHACK_IPK_DIR)$(OPTWARE_PREFIX)bin
	install -m 755 $(NETHACK_BUILD_DIR)/install/nethack $(NETHACK_IPK_DIR)$(OPTWARE_PREFIX)bin/
	install -d $(NETHACK_IPK_DIR)$(OPTWARE_PREFIX)share/nethackdir/
	cp -r $(NETHACK_BUILD_DIR)/install/nethackdir/* $(NETHACK_IPK_DIR)$(OPTWARE_PREFIX)share/nethackdir/
	$(STRIP_COMMAND) \
		$(NETHACK_IPK_DIR)$(OPTWARE_PREFIX)share/nethackdir/nethack \
		$(NETHACK_IPK_DIR)$(OPTWARE_PREFIX)share/nethackdir/recover \
		$(NETHACK_IPK_DIR)$(OPTWARE_PREFIX)share/nethackdir/util/*
