...
#LSOF_CONFFILES=$(OPTWARE_PREFIX)etc/lsof.conf $(OPTWARE_PREFIX)etc/init.d/SXXlsof
...
#
# Binaries should be installed into $(LSOF_IPK_DIR)$(OPTWARE_PREFIX)sbin or $(LSOF_IPK_DIR)$(OPTWARE_PREFIX)bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(LSOF_IPK_DIR)$(OPTWARE_PREFIX){lib,include}
# Configuration files should be installed in $(LSOF_IPK_DIR)$(OPTWARE_PREFIX)etc/lsof/...
# Documentation files should be installed in $(LSOF_IPK_DIR)$(OPTWARE_PREFIX)doc/lsof/...
# Daemon startup scripts should be installed in $(LSOF_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/S??lsof
#
...
#	$(MAKE) -C $(LSOF_BUILD_DIR) DESTDIR=$(LSOF_IPK_DIR) install-strip
	install -d $(LSOF_IPK_DIR)$(OPTWARE_PREFIX)sbin
	$(STRIP_COMMAND) $(LSOF_BUILD_DIR)/lsof -o $(LSOF_IPK_DIR)$(OPTWARE_PREFIX)sbin/lsof
	install -d $(LSOF_IPK_DIR)$(OPTWARE_PREFIX)share/man/man8
	install $(LSOF_BUILD_DIR)/lsof.8 $(LSOF_IPK_DIR)$(OPTWARE_PREFIX)share/man/man8
