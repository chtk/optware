...
#GDCHART_CONFFILES=$(OPTWARE_PREFIX)etc/gdchart.conf $(OPTWARE_PREFIX)etc/init.d/SXXgdchart
...
#
# Binaries should be installed into $(GDCHART_IPK_DIR)$(OPTWARE_PREFIX)sbin or $(GDCHART_IPK_DIR)$(OPTWARE_PREFIX)bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(GDCHART_IPK_DIR)$(OPTWARE_PREFIX){lib,include}
# Configuration files should be installed in $(GDCHART_IPK_DIR)$(OPTWARE_PREFIX)etc/gdchart/...
# Documentation files should be installed in $(GDCHART_IPK_DIR)$(OPTWARE_PREFIX)doc/gdchart/...
# Daemon startup scripts should be installed in $(GDCHART_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/S??gdchart
#
# You may need to patch your application to make it use these locations.
#
$(GDCHART_IPK): $(GDCHART_BUILD_DIR)/.built
	rm -rf $(GDCHART_IPK_DIR) $(BUILD_DIR)/gdchart_*_$(TARGET_ARCH).ipk
	install -d $(GDCHART_IPK_DIR)$(OPTWARE_PREFIX)include $(GDCHART_IPK_DIR)$(OPTWARE_PREFIX)lib
	$(MAKE) -C $(GDCHART_BUILD_DIR) \
		PREFIX_INC=$(GDCHART_IPK_DIR)$(OPTWARE_PREFIX)include \
		PREFIX_LIB=$(GDCHART_IPK_DIR)$(OPTWARE_PREFIX)lib \
		GD_INCL=$(STAGING_INCLUDE_DIR)/ \
		GD_LD=$(STAGING_LIB_DIR)/ \
		install
