...
#TNFTP_CONFFILES=$(OPTWARE_PREFIX)etc/tnftp.conf $(OPTWARE_PREFIX)etc/init.d/SXXtnftp
...
#
# Binaries should be installed into $(TNFTP_IPK_DIR)$(OPTWARE_PREFIX)sbin or $(TNFTP_IPK_DIR)$(OPTWARE_PREFIX)bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(TNFTP_IPK_DIR)$(OPTWARE_PREFIX){lib,include}
# Configuration files should be installed in $(TNFTP_IPK_DIR)$(OPTWARE_PREFIX)etc/tnftp/...
# Documentation files should be installed in $(TNFTP_IPK_DIR)$(OPTWARE_PREFIX)doc/tnftp/...
# Daemon startup scripts should be installed in $(TNFTP_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/S??tnftp
#
...
$(TNFTP_IPK): $(TNFTP_BUILD_DIR)/.built
	...
	$(MAKE) -C $(TNFTP_BUILD_DIR) install \
		...
		prefix=$(TNFTP_IPK_DIR)$(OPTWARE_PREFIX) \
		mandircat1=$(TNFTP_IPK_DIR)$(OPTWARE_PREFIX)/share/man/man1 
	chmod +w $(TNFTP_IPK_DIR)$(OPTWARE_PREFIX)/bin/tnftp && \
	$(STRIP_COMMAND) $(TNFTP_IPK_DIR)$(OPTWARE_PREFIX)/bin/tnftp && \
	chmod -w $(TNFTP_IPK_DIR)$(OPTWARE_PREFIX)/bin/tnftp
#	install -d $(TNFTP_IPK_DIR)$(OPTWARE_PREFIX)etc/
#	install -m 644 $(TNFTP_SOURCE_DIR)/tnftp.conf $(TNFTP_IPK_DIR)$(OPTWARE_PREFIX)etc/tnftp.conf
#	install -d $(TNFTP_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d
#	install -m 755 $(TNFTP_SOURCE_DIR)/rc.tnftp $(TNFTP_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/SXXtnftp
#	sed -i -e '/^#!/aOPTWARE_TARGET=${OPTWARE_TARGET}' $(XINETD_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/SXXtnftp
