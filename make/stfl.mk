...
#STFL_CONFFILES=$(OPTWARE_PREFIX)etc/stfl.conf $(OPTWARE_PREFIX)etc/init.d/SXXstfl
...
$(STFL_BUILD_DIR)/.staged: $(STFL_BUILD_DIR)/.built
	...
	$(MAKE) -C $(STFL_BUILD_DIR) DESTDIR=$(STAGING_DIR) prefix=$(OPTWARE_PREFIX)install
	touch $@
...
#
# Binaries should be installed into $(STFL_IPK_DIR)$(OPTWARE_PREFIX)sbin or $(STFL_IPK_DIR)$(OPTWARE_PREFIX)bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(STFL_IPK_DIR)$(OPTWARE_PREFIX){lib,include}
# Configuration files should be installed in $(STFL_IPK_DIR)$(OPTWARE_PREFIX)etc/stfl/...
# Documentation files should be installed in $(STFL_IPK_DIR)$(OPTWARE_PREFIX)doc/stfl/...
# Daemon startup scripts should be installed in $(STFL_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/S??stfl
#
# You may need to patch your application to make it use these locations.
#
$(STFL_IPK): $(STFL_BUILD_DIR)/.built
	rm -rf $(STFL_IPK_DIR) $(BUILD_DIR)/stfl_*_$(TARGET_ARCH).ipk
	$(MAKE) -C $(STFL_BUILD_DIR) DESTDIR=$(STFL_IPK_DIR) prefix=$(OPTWARE_PREFIX)install
#	install -d $(STFL_IPK_DIR)$(OPTWARE_PREFIX)etc/
#	install -m 644 $(STFL_SOURCE_DIR)/stfl.conf $(STFL_IPK_DIR)$(OPTWARE_PREFIX)etc/stfl.conf
#	install -d $(STFL_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d
#	install -m 755 $(STFL_SOURCE_DIR)/rc.stfl $(STFL_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/SXXstfl
#	sed -i -e '/^#!/aOPTWARE_TARGET=${OPTWARE_TARGET}' $(STFL_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/SXXstfl
