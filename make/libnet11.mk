...
#LIBNET11_CONFFILES=$(OPTWARE_PREFIX)etc/libnet11.conf $(OPTWARE_PREFIX)etc/init.d/SXXlibnet11
...
$(LIBNET11_BUILD_DIR)/.staged: $(LIBNET11_BUILD_DIR)/.built
	...
	$(MAKE) -C $(LIBNET11_BUILD_DIR) install \
		DESTDIR=$(STAGING_DIR) \
		libdir="$(OPTWARE_PREFIX)lib/libnet11" \
		includedir="$(OPTWARE_PREFIX)include/libnet11" \
		;
	touch $@
...
#
# Binaries should be installed into $(LIBNET11_IPK_DIR)$(OPTWARE_PREFIX)sbin or $(LIBNET11_IPK_DIR)$(OPTWARE_PREFIX)bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(LIBNET11_IPK_DIR)$(OPTWARE_PREFIX){lib,include}
# Configuration files should be installed in $(LIBNET11_IPK_DIR)$(OPTWARE_PREFIX)etc/libnet11/...
# Documentation files should be installed in $(LIBNET11_IPK_DIR)$(OPTWARE_PREFIX)doc/libnet11/...
# Daemon startup scripts should be installed in $(LIBNET11_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/S??libnet11
#
