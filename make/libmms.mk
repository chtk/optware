...
#LIBMMS_CONFFILES=$(OPTWARE_PREFIX)etc/libmms.conf $(OPTWARE_PREFIX)etc/init.d/SXXlibmms
...
#
# Binaries should be installed into $(LIBMMS_IPK_DIR)$(OPTWARE_PREFIX)sbin or $(LIBMMS_IPK_DIR)$(OPTWARE_PREFIX)bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(LIBMMS_IPK_DIR)$(OPTWARE_PREFIX){lib,include}
# Configuration files should be installed in $(LIBMMS_IPK_DIR)$(OPTWARE_PREFIX)etc/libmms/...
# Documentation files should be installed in $(LIBMMS_IPK_DIR)$(OPTWARE_PREFIX)doc/libmms/...
# Daemon startup scripts should be installed in $(LIBMMS_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/S??libmms
#
...
#	install -d $(LIBMMS_IPK_DIR)$(OPTWARE_PREFIX)etc/
#	install -m 644 $(LIBMMS_SOURCE_DIR)/libmms.conf $(LIBMMS_IPK_DIR)$(OPTWARE_PREFIX)etc/libmms.conf
#	install -d $(LIBMMS_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d
#	install -m 755 $(LIBMMS_SOURCE_DIR)/rc.libmms $(LIBMMS_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/SXXlibmms
#	sed -i -e '/^#!/aOPTWARE_TARGET=${OPTWARE_TARGET}' $(LIBMMS_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/SXXlibmms
