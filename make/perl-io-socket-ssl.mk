...
$(PERL-IO-SOCKET-SSL_IPK): $(PERL-IO-SOCKET-SSL_BUILD_DIR)/.built
	...
	find $(PERL-IO-SOCKET-SSL_IPK_DIR)$(OPTWARE_PREFIX)-name 'perllocal.pod' -exec rm -f {} \;
	(cd $(PERL-IO-SOCKET-SSL_IPK_DIR)$(OPTWARE_PREFIX)lib/perl5 ; \
		find . -name '*.so' -exec chmod +w {} \; ; \
		find . -name '*.so' -exec $(STRIP_COMMAND) {} \; ; \
		find . -name '*.so' -exec chmod -w {} \; ; \
	)
	find $(PERL-IO-SOCKET-SSL_IPK_DIR)$(OPTWARE_PREFIX)-type d -exec chmod go+rx {} \;
