...
USBUTILS_CONFFILES=$(OPTWARE_PREFIX)share/misc/usb.ids
...
$(USBUTILS_BUILD_DIR)/.configured: $(DL_DIR)/$(USBUTILS_SOURCE) $(USBUTILS_PATCHES) make/usbutils.mk
	...
	sed -i 's|DEST=|&$(OPTWARE_PREFIX)share/misc/|' $(@D)/update-usbids.sh
	(cd $(@D); \
		...
		--disable-nls \
		--datadir=$(OPTWARE_PREFIX)share/misc \
	)
	touch $@
...
#
# Binaries should be installed into $(USBUTILS_IPK_DIR)$(OPTWARE_PREFIX)sbin or $(USBUTILS_IPK_DIR)$(OPTWARE_PREFIX)bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(USBUTILS_IPK_DIR)$(OPTWARE_PREFIX){lib,include}
# Configuration files should be installed in $(USBUTILS_IPK_DIR)$(OPTWARE_PREFIX)etc/usbutils/...
# Documentation files should be installed in $(USBUTILS_IPK_DIR)$(OPTWARE_PREFIX)doc/usbutils/...
# Daemon startup scripts should be installed in $(USBUTILS_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/S??usbutils
#
...
$(USBUTILS_IPK): $(USBUTILS_BUILD_DIR)/.built
	...
	install -m 755 $(USBUTILS_BUILD_DIR)/update-usbids.sh $(USBUTILS_IPK_DIR)$(OPTWARE_PREFIX)sbin/
	# don't want these as they conflict with real libusb
	rm -rf $(USBUTILS_IPK_DIR)$(OPTWARE_PREFIX)lib $(USBUTILS_IPK_DIR)$(OPTWARE_PREFIX)include
#	install -d $(USBUTILS_IPK_DIR)$(OPTWARE_PREFIX)etc/
#	install -m 644 $(USBUTILS_SOURCE_DIR)/usbutils.conf $(USBUTILS_IPK_DIR)$(OPTWARE_PREFIX)etc/usbutils.conf
#	install -d $(USBUTILS_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d
#	install -m 755 $(USBUTILS_SOURCE_DIR)/rc.usbutils $(USBUTILS_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/SXXusbutils
