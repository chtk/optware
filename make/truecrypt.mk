...
#TRUECRYPT_CONFFILES=$(OPTWARE_PREFIX)etc/truecrypt.conf $(OPTWARE_PREFIX)etc/init.d/SXXtruecrypt
...
TRUECRYPT_MODDIR=$(TRUECRYPT_IPK_DIR)$(OPTWARE_PREFIX)lib/modules/2.6.12.6-arm1/extra
...
$(TRUECRYPT_BUILD_DIR)/.configured: $(DL_DIR)/$(TRUECRYPT_SOURCE) $(TRUECRYPT_PATCHES) make/truecrypt.mk
	...
	sed -i	-e '/setenv.*PATH/s|"/|"$(OPTWARE_PREFIX)sbin:$(OPTWARE_PREFIX)bin:/|' $(@D)/Linux/Cli/Cli.c
	touch $@
...
#
# Binaries should be installed into $(TRUECRYPT_IPK_DIR)$(OPTWARE_PREFIX)sbin or $(TRUECRYPT_IPK_DIR)$(OPTWARE_PREFIX)bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(TRUECRYPT_IPK_DIR)$(OPTWARE_PREFIX){lib,include}
# Configuration files should be installed in $(TRUECRYPT_IPK_DIR)$(OPTWARE_PREFIX)etc/truecrypt/...
# Documentation files should be installed in $(TRUECRYPT_IPK_DIR)$(OPTWARE_PREFIX)doc/truecrypt/...
# Daemon startup scripts should be installed in $(TRUECRYPT_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/S??truecrypt
#
...
$(TRUECRYPT_IPK): $(TRUECRYPT_BUILD_DIR)/.built
	...
	install -d $(TRUECRYPT_IPK_DIR)$(OPTWARE_PREFIX)bin
	$(STRIP_COMMAND) $(TRUECRYPT_BUILD_DIR)/Linux/Cli/truecrypt \
		-o $(TRUECRYPT_IPK_DIR)$(OPTWARE_PREFIX)bin/truecrypt
	install -d $(TRUECRYPT_IPK_DIR)$(OPTWARE_PREFIX)share/man/man1
	install $(TRUECRYPT_BUILD_DIR)/Linux/Cli/Man/truecrypt.1 $(TRUECRYPT_IPK_DIR)$(OPTWARE_PREFIX)share/man/man1/
	install -d $(TRUECRYPT_IPK_DIR)$(OPTWARE_PREFIX)share/truecrypt/doc
	install -m 644 \
		$(TRUECRYPT_BUILD_DIR)/License.txt \
		$(TRUECRYPT_IPK_DIR)$(OPTWARE_PREFIX)share/truecrypt/doc/
	install -m 644 \
		"$(TRUECRYPT_BUILD_DIR)/Release/Setup Files/TrueCrypt User Guide.pdf" \
		$(TRUECRYPT_IPK_DIR)$(OPTWARE_PREFIX)share/truecrypt/doc/TrueCrypt-User-Guide.pdf
