...
#PY-AMARA_CONFFILES=$(OPTWARE_PREFIX)etc/py-amara.conf $(OPTWARE_PREFIX)etc/init.d/SXXpy-amara
...
#
# Binaries should be installed into $(PY-AMARA_IPK_DIR)$(OPTWARE_PREFIX)sbin or $(PY-AMARA_IPK_DIR)$(OPTWARE_PREFIX)bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(PY-AMARA_IPK_DIR)$(OPTWARE_PREFIX){lib,include}
# Configuration files should be installed in $(PY-AMARA_IPK_DIR)$(OPTWARE_PREFIX)etc/py-amara/...
# Documentation files should be installed in $(PY-AMARA_IPK_DIR)$(OPTWARE_PREFIX)doc/py-amara/...
# Daemon startup scripts should be installed in $(PY-AMARA_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/S??py-amara
#
...
$(PY24-AMARA_IPK): $(PY-AMARA_BUILD_DIR)/.built
	...
	for f in $(PY24-AMARA_IPK_DIR)$(OPTWARE_PREFIX)bin/*; \
		do mv $$f `echo $$f | sed 's|$$|-2.4|'`; done
#	$(STRIP_COMMAND) `find $(PY24-AMARA_IPK_DIR)$(OPTWARE_PREFIX)lib/ -name '*.so'`
...
#	$(STRIP_COMMAND) `find $(PY25-AMARA_IPK_DIR)$(OPTWARE_PREFIX)lib/ -name '*.so'`
