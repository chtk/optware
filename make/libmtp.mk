...
#LIBMTP_CONFFILES=$(OPTWARE_PREFIX)etc/libmtp.conf $(OPTWARE_PREFIX)etc/init.d/SXXlibmtp
...
#
# Binaries should be installed into $(LIBMTP_IPK_DIR)$(OPTWARE_PREFIX)sbin or $(LIBMTP_IPK_DIR)$(OPTWARE_PREFIX)bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(LIBMTP_IPK_DIR)$(OPTWARE_PREFIX){lib,include}
# Configuration files should be installed in $(LIBMTP_IPK_DIR)$(OPTWARE_PREFIX)etc/libmtp/...
# Documentation files should be installed in $(LIBMTP_IPK_DIR)$(OPTWARE_PREFIX)doc/libmtp/...
# Daemon startup scripts should be installed in $(LIBMTP_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/S??libmtp
#
...
#	install -d $(LIBMTP_IPK_DIR)$(OPTWARE_PREFIX)etc/
#	install -m 644 $(LIBMTP_SOURCE_DIR)/libmtp.conf $(LIBMTP_IPK_DIR)$(OPTWARE_PREFIX)etc/libmtp.conf
#	install -d $(LIBMTP_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d
#	install -m 755 $(LIBMTP_SOURCE_DIR)/rc.libmtp $(LIBMTP_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/SXXlibmtp
#	sed -i -e '/^#!/aOPTWARE_TARGET=${OPTWARE_TARGET}' $(LIBMTP_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/SXXlibmtp
