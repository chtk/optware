...
$(PERL-NET-CIDR-LITE_IPK): $(PERL-NET-CIDR-LITE_BUILD_DIR)/.built
	...
	find $(PERL-NET-CIDR-LITE_IPK_DIR)$(OPTWARE_PREFIX)-name 'perllocal.pod' -exec rm -f {} \;
	(cd $(PERL-NET-CIDR-LITE_IPK_DIR)$(OPTWARE_PREFIX)lib/perl5 ; \
		find . -name '*.so' -exec chmod +w {} \; ; \
		find . -name '*.so' -exec $(STRIP_COMMAND) {} \; ; \
		find . -name '*.so' -exec chmod -w {} \; ; \
	)
	find $(PERL-NET-CIDR-LITE_IPK_DIR)$(OPTWARE_PREFIX)-type d -exec chmod go+rx {} \;
