...
UEMACS_CONFFILES=$(OPTWARE_PREFIX)etc/.uemacsrc
...
#
# Binaries should be installed into $(UEMACS_IPK_DIR)$(OPTWARE_PREFIX)sbin or $(UEMACS_IPK_DIR)$(OPTWARE_PREFIX)bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(UEMACS_IPK_DIR)$(OPTWARE_PREFIX){lib,include}
# Configuration files should be installed in $(UEMACS_IPK_DIR)$(OPTWARE_PREFIX)etc/uemacs/...
# Documentation files should be installed in $(UEMACS_IPK_DIR)$(OPTWARE_PREFIX)doc/uemacs/...
# Daemon startup scripts should be installed in $(UEMACS_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/S??uemacs
#
# You may need to patch your application to make it use these locations.
#
$(UEMACS_IPK): $(UEMACS_BUILD_DIR)/.built
	rm -rf $(UEMACS_IPK_DIR) $(BUILD_DIR)/uemacs_*_$(TARGET_ARCH).ipk
	install -d $(UEMACS_IPK_DIR)$(OPTWARE_PREFIX)bin $(UEMACS_IPK_DIR)$(OPTWARE_PREFIX)etc
	$(MAKE) -C $(UEMACS_BUILD_DIR) \
		BINDIR=$(UEMACS_IPK_DIR)$(OPTWARE_PREFIX)bin \
		LIBDIR=$(UEMACS_IPK_DIR)$(OPTWARE_PREFIX)etc \
		install
	cd $(UEMACS_IPK_DIR)$(OPTWARE_PREFIX)bin; ln -s em uemacs
	mv $(UEMACS_IPK_DIR)$(OPTWARE_PREFIX)etc/.emacsrc $(UEMACS_IPK_DIR)$(OPTWARE_PREFIX)etc/.uemacsrc
	mv $(UEMACS_IPK_DIR)$(OPTWARE_PREFIX)etc/emacs.hlp $(UEMACS_IPK_DIR)$(OPTWARE_PREFIX)etc/uemacs.hlp
#	install -d $(UEMACS_IPK_DIR)$(OPTWARE_PREFIX)etc/
#	install -m 644 $(UEMACS_SOURCE_DIR)/uemacs.conf $(UEMACS_IPK_DIR)$(OPTWARE_PREFIX)etc/uemacs.conf
#	install -d $(UEMACS_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d
#	install -m 755 $(UEMACS_SOURCE_DIR)/rc.uemacs $(UEMACS_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/SXXuemacs
#	sed -i -e '/^#!/aOPTWARE_TARGET=${OPTWARE_TARGET}' $(UEMACS_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/SXXuemacs
