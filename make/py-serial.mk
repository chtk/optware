...
PY26-SERIAL_DEPENDS=python26, py-serial-common
PY-SERIAL_CONFLICTS=
...
#PY-SERIAL_CONFFILES=$(OPTWARE_PREFIX)etc/py-serial.conf $(OPTWARE_PREFIX)etc/init.d/SXXpy-serial
...
PY26-SERIAL_IPK_DIR=$(BUILD_DIR)/py26-serial-$(PY-SERIAL_VERSION)-ipk
PY26-SERIAL_IPK=$(BUILD_DIR)/py26-serial_$(PY-SERIAL_VERSION)-$(PY-SERIAL_IPK_VERSION)_$(TARGET_ARCH).ipk
...
#
$(PY-SERIAL_BUILD_DIR)/.configured: $(DL_DIR)/$(PY-SERIAL_SOURCE) $(PY-SERIAL_PATCHES) make/py-serial.mk
	$(MAKE) python-stage
	rm -rf $(@D)
	mkdir -p $(@D)
	# 2.5
...
$(PY26-SERIAL_IPK_DIR)/CONTROL/control:
	...
	@echo "Source: $(PY-SERIAL_SITE)/$(PY-SERIAL_SOURCE)" >>$@
	@echo "Description: $(PY-SERIAL_DESCRIPTION)" >>$@
	@echo "Depends: $(PY26-SERIAL_DEPENDS)" >>$@
	@echo "Conflicts: $(PY-SERIAL_CONFLICTS)" >>$@

#
# This builds the IPK file.
#
# Binaries should be installed into $(PY-SERIAL_IPK_DIR)$(OPTWARE_PREFIX)sbin or $(PY-SERIAL_IPK_DIR)$(OPTWARE_PREFIX)bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(PY-SERIAL_IPK_DIR)$(OPTWARE_PREFIX){lib,include}
# Configuration files should be installed in $(PY-SERIAL_IPK_DIR)$(OPTWARE_PREFIX)etc/py-serial/...
# Documentation files should be installed in $(PY-SERIAL_IPK_DIR)$(OPTWARE_PREFIX)doc/py-serial/...
# Daemon startup scripts should be installed in $(PY-SERIAL_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/S??py-serial
#
# You may need to patch your application to make it use these locations.
#
$(PY25-SERIAL_IPK): $(PY-SERIAL_BUILD_DIR)/.built
...
$(PY26-SERIAL_IPK) $(PY-SERIAL-COMMON_IPK): $(PY-SERIAL_BUILD_DIR)/.built
	rm -rf $(PY26-SERIAL_IPK_DIR) $(BUILD_DIR)/py26-serial_*_$(TARGET_ARCH).ipk
	...
	install -d $(PY-SERIAL-COMMON_IPK_DIR)$(OPTWARE_PREFIX)share/doc/py-serial/examples
	install $(PY-SERIAL_BUILD_DIR)/2.6/[CLR]*.txt $(PY-SERIAL-COMMON_IPK_DIR)$(OPTWARE_PREFIX)share/doc/py-serial/
	install $(PY-SERIAL_BUILD_DIR)/2.6/examples/* $(PY-SERIAL-COMMON_IPK_DIR)$(OPTWARE_PREFIX)share/doc/py-serial/examples/
	mv $(PY26-SERIAL_IPK_DIR)$(OPTWARE_PREFIX)bin/miniterm.py $(PY26-SERIAL_IPK_DIR)$(OPTWARE_PREFIX)bin/miniterm.py26
...
#
py-serial-ipk: $(PY25-SERIAL_IPK) $(PY26-SERIAL_IPK) $(PY-SERIAL-COMMON_IPK)
...
py-serial-dirclean:
	...
	rm -rf $(PY26-SERIAL_IPK_DIR) $(PY26-SERIAL_IPK)

#
# Some sanity check for the package.
#
py-serial-check: $(PY25-SERIAL_IPK) $(PY26-SERIAL_IPK)
