...
NET_SNMP_CONFFILES=$(OPTWARE_PREFIX)etc/snmpd.conf $(OPTWARE_PREFIX)etc/init.d/S70net-snmp
...
$(NET_SNMP_BUILD_DIR)/.configured: $(DL_DIR)/$(NET_SNMP_SOURCE) $(NET_SNMP_PATCHES)
	...
	(cd $(@D); \
		...
		--with-logfile=$(OPTWARE_PREFIX)var/log/snmpd.log \
		--with-persistent-directory=$(OPTWARE_PREFIX)var/net-snmp \
	)
...
#
# Binaries should be installed into $(NET_SNMP_IPK_DIR)$(OPTWARE_PREFIX)sbin or $(NET_SNMP_IPK_DIR)$(OPTWARE_PREFIX)bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(NET_SNMP_IPK_DIR)$(OPTWARE_PREFIX){lib,include}
# Configuration files should be installed in $(NET_SNMP_IPK_DIR)$(OPTWARE_PREFIX)etc/net-snmp/...
# Documentation files should be installed in $(NET_SNMP_IPK_DIR)$(OPTWARE_PREFIX)doc/net-snmp/...
# Daemon startup scripts should be installed in $(NET_SNMP_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/S??net-snmp
#
...
$(NET_SNMP_IPK): $(NET_SNMP_BUILD_DIR)/.built
	...
	for F in $(NET_SNMP_IPK_DIR)$(OPTWARE_PREFIX)bin/* ; do (file $$F |fgrep -vq ELF) || $(STRIP_COMMAND) $$F ; done
	$(STRIP_COMMAND) $(NET_SNMP_IPK_DIR)$(OPTWARE_PREFIX)sbin/*
	$(STRIP_COMMAND) $(NET_SNMP_IPK_DIR)$(OPTWARE_PREFIX)lib/*.so
	install -d $(NET_SNMP_IPK_DIR)$(OPTWARE_PREFIX)etc/
	install -m 644 $(NET_SNMP_SOURCE_DIR)/snmpd.conf $(NET_SNMP_IPK_DIR)$(OPTWARE_PREFIX)etc/snmpd.conf
	install -d $(NET_SNMP_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d
	install -m 755 $(NET_SNMP_SOURCE_DIR)/rc.net-snmp $(NET_SNMP_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/S70net-snmp
