...
#
TESSERACT-OCR_IPK_VERSION=3

#
# TESSERACT-OCR_CONFFILES should be a list of user-editable files
#TESSERACT-OCR_CONFFILES=$(OPTWARE_PREFIX)etc/tesseract-ocr.conf $(OPTWARE_PREFIX)etc/init.d/SXXtesseract-ocr
...
#
# Binaries should be installed into $(TESSERACT-OCR_IPK_DIR)$(OPTWARE_PREFIX)sbin or $(TESSERACT-OCR_IPK_DIR)$(OPTWARE_PREFIX)bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(TESSERACT-OCR_IPK_DIR)$(OPTWARE_PREFIX){lib,include}
# Configuration files should be installed in $(TESSERACT-OCR_IPK_DIR)$(OPTWARE_PREFIX)etc/tesseract-ocr/...
# Documentation files should be installed in $(TESSERACT-OCR_IPK_DIR)$(OPTWARE_PREFIX)doc/tesseract-ocr/...
# Daemon startup scripts should be installed in $(TESSERACT-OCR_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/S??tesseract-ocr
#
...
$(TESSERACT-OCR_IPK) $(TESSERACT-OCR-DEV_IPK): $(TESSERACT-OCR_BUILD_DIR)/.built
	...
	$(STRIP_COMMAND) $(TESSERACT-OCR_IPK_DIR)$(OPTWARE_PREFIX)bin/*
	rm -f $(TESSERACT-OCR_IPK_DIR)$(OPTWARE_PREFIX)share/tessdata/???.*
	...
	mv $(TESSERACT-OCR_IPK_DIR)$(OPTWARE_PREFIX)include $(TESSERACT-OCR_IPK_DIR)$(OPTWARE_PREFIX)lib $(TESSERACT-OCR-DEV_IPK_DIR)$(OPTWARE_PREFIX)
...
$(TESSERACT-OCR_BUILD_DIR)/.lang-ipks-done: $(TESSERACT-OCR-LANG_TARBALLS) $(TESSERACT-OCR_BUILD_DIR)/.built
	...
	for l in $(TESSERACT-OCR_LANGS_200); do \
		install -d $(TESSERACT-OCR_IPK_DIR)-langs/$$l$(OPTWARE_PREFIX)share; \
		tar -C $(TESSERACT-OCR_IPK_DIR)-langs/$$l$(OPTWARE_PREFIX)share -xzvf $(DL_DIR)/tesseract-2.00.$$l.tar.gz; \
		chmod 644 $(TESSERACT-OCR_IPK_DIR)-langs/$$l$(OPTWARE_PREFIX)share/tessdata/*; \
		cd $(BUILD_DIR); $(IPKG_BUILD) $(TESSERACT-OCR_IPK_DIR)-langs/$$l; \
	done
	for l in $(TESSERACT-OCR_LANGS_201); do \
		install -d $(TESSERACT-OCR_IPK_DIR)-langs/$$l$(OPTWARE_PREFIX)share; \
		tar -C $(TESSERACT-OCR_IPK_DIR)-langs/$$l$(OPTWARE_PREFIX)share -xzvf $(DL_DIR)/tesseract-2.01.$$l.tar.gz; \
		chmod 644 $(TESSERACT-OCR_IPK_DIR)-langs/$$l$(OPTWARE_PREFIX)share/tessdata/*; \
		cd $(BUILD_DIR); $(IPKG_BUILD) $(TESSERACT-OCR_IPK_DIR)-langs/$$l; \
	done
	touch $@
