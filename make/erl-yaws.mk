...
ERL-YAWS_CONFFILES=$(OPTWARE_PREFIX)etc/yaws/yaws.conf $(OPTWARE_PREFIX)etc/yaws/yaws-cert.pem $(OPTWARE_PREFIX)etc/yaws/yaws-key.pem
...
#
# Binaries should be installed into $(ERL-YAWS_IPK_DIR)$(OPTWARE_PREFIX)sbin or $(ERL-YAWS_IPK_DIR)$(OPTWARE_PREFIX)bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(ERL-YAWS_IPK_DIR)$(OPTWARE_PREFIX){lib,include}
# Configuration files should be installed in $(ERL-YAWS_IPK_DIR)$(OPTWARE_PREFIX)etc/erl-yaws/...
# Documentation files should be installed in $(ERL-YAWS_IPK_DIR)$(OPTWARE_PREFIX)doc/erl-yaws/...
# Daemon startup scripts should be installed in $(ERL-YAWS_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/S??erl-yaws
#
...
$(ERL-YAWS_IPK): $(ERL-YAWS_BUILD_DIR)/.built
	...
	$(STRIP_COMMAND) $(ERL-YAWS_IPK_DIR)$(OPTWARE_PREFIX)lib/yaws/priv/lib/*.so
	mv $(ERL-YAWS_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/yaws $(ERL-YAWS_IPK_DIR)$(OPTWARE_PREFIX)share/doc/$(ERL-YAWS_DIR)/sample-init.d-yaws
	sed -i \
	    -e 's|^erl=.*|erl="$(OPTWARE_PREFIX)lib/erlang/bin/erl"|' \
	    -e 's|^run_erl=.*|run_erl="$(OPTWARE_PREFIX)lib/erlang/bin/run_erl"|' \
	    -e 's|^to_erl=.*|to_erl="$(OPTWARE_PREFIX)lib/erlang/bin/to_erl"|' \
	    $(ERL-YAWS_IPK_DIR)$(OPTWARE_PREFIX)bin/yaws
	sed -i \
	    -e '/^<server localhost/,$$s/^/#/' \
	    -e 's/<server.*>/<server localhost>/' \
	    $(ERL-YAWS_IPK_DIR)$(OPTWARE_PREFIX)etc/yaws/yaws.conf
