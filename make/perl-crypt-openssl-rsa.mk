...
$(PERL-CRYPT-OPENSSL-RSA_IPK): $(PERL-CRYPT-OPENSSL-RSA_BUILD_DIR)/.built
	...
	find $(PERL-CRYPT-OPENSSL-RSA_IPK_DIR)$(OPTWARE_PREFIX)-name 'perllocal.pod' -exec rm -f {} \;
	(cd $(PERL-CRYPT-OPENSSL-RSA_IPK_DIR)$(OPTWARE_PREFIX)lib/perl5 ; \
		find . -name '*.so' -exec chmod +w {} \; ; \
		find . -name '*.so' -exec $(STRIP_COMMAND) {} \; ; \
		find . -name '*.so' -exec chmod -w {} \; ; \
	)
	find $(PERL-CRYPT-OPENSSL-RSA_IPK_DIR)$(OPTWARE_PREFIX)-type d -exec chmod go+rx {} \;
