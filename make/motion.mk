...
MOTION_CONFFILES=$(OPTWARE_PREFIX)etc/motion.conf $(OPTWARE_PREFIX)etc/init.d/S99motion
...
MOTION_LDFLAGS="-Wl,-rpath,$(OPTWARE_PREFIX)lib/mysql" -L$(STAGING_PREFIX)/lib/mysql
endif
...
#
# Binaries should be installed into $(MOTION_IPK_DIR)$(OPTWARE_PREFIX)sbin or $(MOTION_IPK_DIR)$(OPTWARE_PREFIX)bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(MOTION_IPK_DIR)$(OPTWARE_PREFIX){lib,include}
# Configuration files should be installed in $(MOTION_IPK_DIR)$(OPTWARE_PREFIX)etc/motion/...
# Documentation files should be installed in $(MOTION_IPK_DIR)$(OPTWARE_PREFIX)doc/motion/...
#
...
$(MOTION_IPK): $(MOTION_BUILD_DIR)/.built
	...
	$(STRIP_COMMAND) $(MOTION_IPK_DIR)$(OPTWARE_PREFIX)bin/motion
	install -d $(MOTION_IPK_DIR)$(OPTWARE_PREFIX)etc/
	install -m 644 $(MOTION_SOURCE_DIR)/motion.conf $(MOTION_IPK_DIR)$(OPTWARE_PREFIX)etc/motion.conf
	install -d $(MOTION_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d
	install -m 755 $(MOTION_SOURCE_DIR)/rc.motion $(MOTION_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/S99motion
