...
#SLSC_CONFFILES=$(OPTWARE_PREFIX)etc/slsc.conf $(OPTWARE_PREFIX)etc/init.d/SXXslsc
...
$(SLSC_BUILD_DIR)/.built: $(SLSC_BUILD_DIR)/.configured
	...
	$(MAKE) -C $(SLSC_BUILD_DIR) \
		SLSC_ROOT=$(OPTWARE_PREFIX)lib/slsc \
		SLSC_BIN=$(OPTWARE_PREFIX)bin \
		;
	touch $@
...
#
# Binaries should be installed into $(SLSC_IPK_DIR)$(OPTWARE_PREFIX)sbin or $(SLSC_IPK_DIR)$(OPTWARE_PREFIX)bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(SLSC_IPK_DIR)$(OPTWARE_PREFIX){lib,include}
# Configuration files should be installed in $(SLSC_IPK_DIR)$(OPTWARE_PREFIX)etc/slsc/...
# Documentation files should be installed in $(SLSC_IPK_DIR)$(OPTWARE_PREFIX)doc/slsc/...
# Daemon startup scripts should be installed in $(SLSC_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/S??slsc
#
# You may need to patch your application to make it use these locations.
#
$(SLSC_IPK): $(SLSC_BUILD_DIR)/.built
	rm -rf $(SLSC_IPK_DIR) $(BUILD_DIR)/slsc_*_$(TARGET_ARCH).ipk
	install -d $(SLSC_IPK_DIR)$(OPTWARE_PREFIX)lib
	$(MAKE) -C $(SLSC_BUILD_DIR) DESTDIR=$(SLSC_IPK_DIR) install \
		SLSC_ROOT=$(SLSC_IPK_DIR)$(OPTWARE_PREFIX)lib/slsc \
		SLSC_BIN=$(SLSC_IPK_DIR)$(OPTWARE_PREFIX)bin \
		;
	$(STRIP_COMMAND) $(SLSC_IPK_DIR)$(OPTWARE_PREFIX)bin/slsc $(SLSC_IPK_DIR)$(OPTWARE_PREFIX)lib/slsc/vprint
