...
# NTFSPROGS_CONFFILES=$(OPTWARE_PREFIX)etc/ntfsprogs.conf $(OPTWARE_PREFIX)etc/init.d/SXXntfsprogs
...
#
# Binaries should be installed into $(NTFSPROGS_IPK_DIR)$(OPTWARE_PREFIX)sbin or $(NTFSPROGS_IPK_DIR)$(OPTWARE_PREFIX)bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(NTFSPROGS_IPK_DIR)$(OPTWARE_PREFIX){lib,include}
# Configuration files should be installed in $(NTFSPROGS_IPK_DIR)$(OPTWARE_PREFIX)etc/ntfsprogs/...
# Documentation files should be installed in $(NTFSPROGS_IPK_DIR)$(OPTWARE_PREFIX)doc/ntfsprogs/...
# Daemon startup scripts should be installed in $(NTFSPROGS_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/S??ntfsprogs
#
...
#	install -d $(NTFSPROGS_IPK_DIR)$(OPTWARE_PREFIX)etc/
#	install -m 644 $(NTFSPROGS_SOURCE_DIR)/ntfsprogs.conf $(NTFSPROGS_IPK_DIR)$(OPTWARE_PREFIX)etc/ntfsprogs.conf
#	install -d $(NTFSPROGS_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d
#	install -m 755 $(NTFSPROGS_SOURCE_DIR)/rc.ntfsprogs $(NTFSPROGS_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/SXXntfsprogs
...
#	echo $(NTFSPROGS_CONFFILES) | sed -e 's/ /\n/g' > $(NTFSPROGS_IPK_DIR)/CONTROL/conffiles
	rm -f $(NTFSPROGS_IPK_DIR)$(OPTWARE_PREFIX)lib/*.la
	rm -rf $(NTFSPROGS_IPK_DIR)/sbin
	rm -rf $(NTFSPROGS_IPK_DIR)$(OPTWARE_PREFIX)include
