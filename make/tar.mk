...
#
# Binaries should be installed into $(TAR_IPK_DIR)$(OPTWARE_PREFIX)sbin or $(TAR_IPK_DIR)$(OPTWARE_PREFIX)bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(TAR_IPK_DIR)$(OPTWARE_PREFIX){lib,include}
# Configuration files should be installed in $(TAR_IPK_DIR)$(OPTWARE_PREFIX)etc/tar/...
# Documentation files should be installed in $(TAR_IPK_DIR)$(OPTWARE_PREFIX)doc/tar/...
# Daemon startup scripts should be installed in $(TAR_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/S??tar
#
...
$(TAR_IPK): $(TAR_BUILD_DIR)/.built
	...
	mv $(TAR_IPK_DIR)$(OPTWARE_PREFIX)bin/tar $(TAR_IPK_DIR)$(OPTWARE_PREFIX)bin/gnutar
	mv $(TAR_IPK_DIR)$(OPTWARE_PREFIX)libexec/rmt $(TAR_IPK_DIR)$(OPTWARE_PREFIX)libexec/rmt-tar
	$(MAKE) $(TAR_IPK_DIR)/CONTROL/control
	(echo "#!/bin/sh"; \
	 echo "update-alternatives --install $(OPTWARE_PREFIX)bin/tar tar $(OPTWARE_PREFIX)bin/gnutar 80"; \
	) > $(TAR_IPK_DIR)/CONTROL/postinst
	(echo "#!/bin/sh"; \
	 echo "update-alternatives --remove tar $(OPTWARE_PREFIX)bin/gnutar"; \
