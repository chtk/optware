...
#PY-PGSQL_CONFFILES=$(OPTWARE_PREFIX)etc/py-pgsql.conf $(OPTWARE_PREFIX)etc/init.d/SXXpy-pgsql
...
#
# Binaries should be installed into $(PY-PGSQL_IPK_DIR)$(OPTWARE_PREFIX)sbin or $(PY-PGSQL_IPK_DIR)$(OPTWARE_PREFIX)bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(PY-PGSQL_IPK_DIR)$(OPTWARE_PREFIX){lib,include}
# Configuration files should be installed in $(PY-PGSQL_IPK_DIR)$(OPTWARE_PREFIX)etc/py-pgsql/...
# Documentation files should be installed in $(PY-PGSQL_IPK_DIR)$(OPTWARE_PREFIX)doc/py-pgsql/...
# Daemon startup scripts should be installed in $(PY-PGSQL_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/S??py-pgsql
#
...
$(PY24-PGSQL_IPK): $(PY-PGSQL_BUILD_DIR)/.built
	...
	$(STRIP_COMMAND) `find $(PY24-PGSQL_IPK_DIR)$(OPTWARE_PREFIX)lib -name '*.so'`
...
$(PY25-PGSQL_IPK): $(PY-PGSQL_BUILD_DIR)/.built
	...
	$(STRIP_COMMAND) `find $(PY25-PGSQL_IPK_DIR)$(OPTWARE_PREFIX)lib -name '*.so'`
