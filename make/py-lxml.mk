...
#PY-LXML_CONFFILES=$(OPTWARE_PREFIX)etc/py-lxml.conf $(OPTWARE_PREFIX)etc/init.d/SXXpy-lxml
...
#
# Binaries should be installed into $(PY-LXML_IPK_DIR)$(OPTWARE_PREFIX)sbin or $(PY-LXML_IPK_DIR)$(OPTWARE_PREFIX)bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(PY-LXML_IPK_DIR)$(OPTWARE_PREFIX){lib,include}
# Configuration files should be installed in $(PY-LXML_IPK_DIR)$(OPTWARE_PREFIX)etc/py-lxml/...
# Documentation files should be installed in $(PY-LXML_IPK_DIR)$(OPTWARE_PREFIX)doc/py-lxml/...
# Daemon startup scripts should be installed in $(PY-LXML_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/S??py-lxml
#
...
$(PY25-LXML_IPK): $(PY-LXML_BUILD_DIR)/.built
	...
	(cd $(PY-LXML_BUILD_DIR)/2.5; \
		...
		install --root=$(PY25-LXML_IPK_DIR) --prefix=$(OPTWARE_PREFIX)\
		--with-xslt-config=$(STAGING_PREFIX)/bin/xslt-config)
	$(STRIP_COMMAND) `find $(PY25-LXML_IPK_DIR)$(OPTWARE_PREFIX)lib/ -name '*.so'`
...
$(PY26-LXML_IPK): $(PY-LXML_BUILD_DIR)/.built
	...
	(cd $(PY-LXML_BUILD_DIR)/2.6; \
		...
		install --root=$(PY26-LXML_IPK_DIR) --prefix=$(OPTWARE_PREFIX)\
		--with-xslt-config=$(STAGING_PREFIX)/bin/xslt-config)
	$(STRIP_COMMAND) `find $(PY26-LXML_IPK_DIR)$(OPTWARE_PREFIX)lib/ -name '*.so'`
