...
# MZSCHEME_CONFFILES=$(OPTWARE_PREFIX)etc/mzscheme.conf $(OPTWARE_PREFIX)etc/init.d/SXXmzscheme
...
#
# Binaries should be installed into $(MZSCHEME_IPK_DIR)$(OPTWARE_PREFIX)sbin or $(MZSCHEME_IPK_DIR)$(OPTWARE_PREFIX)bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(MZSCHEME_IPK_DIR)$(OPTWARE_PREFIX){lib,include}
# Configuration files should be installed in $(MZSCHEME_IPK_DIR)$(OPTWARE_PREFIX)etc/mzscheme/...
# Documentation files should be installed in $(MZSCHEME_IPK_DIR)$(OPTWARE_PREFIX)doc/mzscheme/...
# Daemon startup scripts should be installed in $(MZSCHEME_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/S??mzscheme
#
# You may need to patch your application to make it use these locations.
#
$(MZSCHEME_IPK): $(MZSCHEME_BUILD_DIR)/.built
	rm -rf $(MZSCHEME_IPK_DIR) $(BUILD_DIR)/mzscheme_*_$(TARGET_ARCH).ipk
	umask 022; PLT_EXTENSION_LIB_PATHS=$(STAGING_DIR)$(OPTWARE_PREFIX)\
            $(MAKE) -C $(MZSCHEME_BUILD_DIR)/src prefix=$(MZSCHEME_IPK_DIR)$(OPTWARE_PREFIX)lib/plt install
	sed -i \
	    -e '/^CC=/s:^.*$$:CC=$(OPTWARE_PREFIX)bin/gcc:' \
	    -e 's:$(STAGING_DIR)::' \
	    $(MZSCHEME_IPK_DIR)$(OPTWARE_PREFIX)lib/plt/lib/buildinfo
	sed -i \
	    -e 's:$(MZSCHEME_IPK_DIR)::' \
            `ls $(MZSCHEME_IPK_DIR)$(OPTWARE_PREFIX)lib/plt/bin/*  | grep -v plt/bin/mzscheme`
	$(STRIP_COMMAND) $(MZSCHEME_IPK_DIR)$(OPTWARE_PREFIX)lib/plt/bin/mzscheme
	$(STRIP_COMMAND) `find $(MZSCHEME_IPK_DIR)$(OPTWARE_PREFIX)lib/plt -name '*.so'`
	install -d $(MZSCHEME_IPK_DIR)$(OPTWARE_PREFIX)bin/
	cd $(MZSCHEME_IPK_DIR)$(OPTWARE_PREFIX)bin/; \
            for f in ../lib/plt/bin/*; do ln -s $$f .; done
	# a hack to work around POSIX tar 100 character limitation
	cd $(MZSCHEME_IPK_DIR)$(OPTWARE_PREFIX)lib/plt; \
	    mv collects/web-server/default-web-root .
	cd $(MZSCHEME_IPK_DIR)$(OPTWARE_PREFIX)lib/plt/collects/web-server; \
            ln -s ../../default-web-root .
	umask 111; touch $(MZSCHEME_IPK_DIR)$(OPTWARE_PREFIX)lib/plt/collects/web-server/default-web-root/log
