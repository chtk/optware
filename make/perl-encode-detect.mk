...
ifeq ($(OPTWARE_TARGET), $(filter dns323 ds101g fsg3 nslu2, $(OPTWARE_TARGET)))
PERL-ENCODE-DETECT_PATCHES=$(PERL-ENCODE-DETECT_SOURCE_DIR)/Detect-xs.patch
endif
...
$(PERL-ENCODE-DETECT_IPK): $(PERL-ENCODE-DETECT_BUILD_DIR)/.built
	...
	(cd $(PERL-ENCODE-DETECT_BUILD_DIR); \
		PERL5LIB="$(STAGING_LIB_DIR)/perl5/site_perl" \
		./Build --prefix $(PERL-ENCODE-DETECT_IPK_DIR)$(OPTWARE_PREFIX)install \
	)
	find $(PERL-ENCODE-DETECT_IPK_DIR)$(OPTWARE_PREFIX)-name 'perllocal.pod' -exec rm -f {} \;
	(cd $(PERL-ENCODE-DETECT_IPK_DIR)$(OPTWARE_PREFIX)lib/perl5 ; \
		find . -name '*.so' -exec chmod +w {} \; ; \
		find . -name '*.so' -exec $(STRIP_COMMAND) {} \; ; \
		find . -name '*.so' -exec chmod -w {} \; ; \
	)
	find $(PERL-ENCODE-DETECT_IPK_DIR)$(OPTWARE_PREFIX)-type d -exec chmod go+rx {} \;
