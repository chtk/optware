...
#NMON_CONFFILES=$(OPTWARE_PREFIX)etc/nmon.conf $(OPTWARE_PREFIX)etc/init.d/SXXnmon
...
#
# Binaries should be installed into $(NMON_IPK_DIR)$(OPTWARE_PREFIX)sbin or $(NMON_IPK_DIR)$(OPTWARE_PREFIX)bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(NMON_IPK_DIR)$(OPTWARE_PREFIX){lib,include}
# Configuration files should be installed in $(NMON_IPK_DIR)$(OPTWARE_PREFIX)etc/nmon/...
# Documentation files should be installed in $(NMON_IPK_DIR)$(OPTWARE_PREFIX)doc/nmon/...
# Daemon startup scripts should be installed in $(NMON_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/S??nmon
#
# You may need to patch your application to make it use these locations.
#
$(NMON_IPK): $(NMON_BUILD_DIR)/.built
	rm -rf $(NMON_IPK_DIR) $(BUILD_DIR)/nmon_*_$(TARGET_ARCH).ipk
	install -d $(NMON_IPK_DIR)$(OPTWARE_PREFIX)bin
	install -m 755 $(NMON_BUILD_DIR)/nmon $(NMON_IPK_DIR)$(OPTWARE_PREFIX)bin
	$(STRIP_COMMAND) $(NMON_IPK_DIR)$(OPTWARE_PREFIX)bin/nmon
