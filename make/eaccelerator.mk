...
EACCELERATOR_CONFFILES=$(OPTWARE_PREFIX)etc/php.d/eaccelerator.ini
...
make/eaccelerator.mk make/php.mk
	...
	(cd $(@D); \
		...
		--with-php-config=$(STAGING_DIR)$(OPTWARE_PREFIX)bin/php-config \
		--with-eaccelerator-userid='"nobody"' \
	)
	sed -i -e '/^CPPFLAGS/s|-I$(OPTWARE_PREFIX)include/php ||' $(@D)/Makefile
	touch $@
...
#
# Binaries should be installed into $(EACCELERATOR_IPK_DIR)$(OPTWARE_PREFIX)sbin or $(EACCELERATOR_IPK_DIR)$(OPTWARE_PREFIX)bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(EACCELERATOR_IPK_DIR)$(OPTWARE_PREFIX){lib,include}
# Configuration files should be installed in $(EACCELERATOR_IPK_DIR)$(OPTWARE_PREFIX)etc/eaccelerator/...
# Documentation files should be installed in $(EACCELERATOR_IPK_DIR)$(OPTWARE_PREFIX)doc/eaccelerator/...
# Daemon startup scripts should be installed in $(EACCELERATOR_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/S??eaccelerator
#
...
$(EACCELERATOR_IPK): $(EACCELERATOR_BUILD_DIR)/.built
	...
	$(STRIP_COMMAND) $(EACCELERATOR_IPK_DIR)$(OPTWARE_PREFIX)lib/php/extensions/eaccelerator.so
	install -d $(EACCELERATOR_IPK_DIR)$(OPTWARE_PREFIX)tmp/eaccelerator
	install -d $(EACCELERATOR_IPK_DIR)$(OPTWARE_PREFIX)etc/php.d
	install -m 644 $(EACCELERATOR_SOURCE_DIR)/eaccelerator.ini $(EACCELERATOR_IPK_DIR)$(OPTWARE_PREFIX)etc/php.d/eaccelerator.ini
