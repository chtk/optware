...
#
# Binaries should be installed into $(UNZIP_IPK_DIR)$(OPTWARE_PREFIX)sbin or $(UNZIP_IPK_DIR)$(OPTWARE_PREFIX)bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(UNZIP_IPK_DIR)$(OPTWARE_PREFIX){lib,include}
# Configuration files should be installed in $(UNZIP_IPK_DIR)$(OPTWARE_PREFIX)etc/unzip/...
# Documentation files should be installed in $(UNZIP_IPK_DIR)$(OPTWARE_PREFIX)doc/unzip/...
# Daemon startup scripts should be installed in $(UNZIP_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/S??unzip
#
# You may need to patch your application to make it use these locations.
#
$(UNZIP_IPK): $(UNZIP_BUILD_DIR)/.built
	rm -rf $(UNZIP_IPK_DIR) $(BUILD_DIR)/unzip_*_$(TARGET_ARCH).ipk
	$(MAKE) -C $(UNZIP_BUILD_DIR) -f unix/Makefile prefix=$(UNZIP_IPK_DIR)$(OPTWARE_PREFIX)install
	mv $(UNZIP_IPK_DIR)$(OPTWARE_PREFIX)bin/unzip $(UNZIP_IPK_DIR)$(OPTWARE_PREFIX)bin/unzip-unzip
	$(MAKE) $(UNZIP_IPK_DIR)/CONTROL/control
	(echo "#!/bin/sh"; \
	 echo "update-alternatives --install $(OPTWARE_PREFIX)bin/unzip unzip $(OPTWARE_PREFIX)bin/unzip-unzip 80"; \
	) > $(UNZIP_IPK_DIR)/CONTROL/postinst
	(echo "#!/bin/sh"; \
	 echo "update-alternatives --remove unzip $(OPTWARE_PREFIX)bin/unzip-unzip"; \
