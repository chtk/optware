...
$(PERL-MIME-BASE64_BUILD_DIR)/.built: $(PERL-MIME-BASE64_BUILD_DIR)/.configured
	...
	$(MAKE) -C $(PERL-MIME-BASE64_BUILD_DIR) $(PERL_INC) all
...
$(PERL-MIME-BASE64_IPK): $(PERL-MIME-BASE64_BUILD_DIR)/.built
	...
	find $(PERL-MIME-BASE64_IPK_DIR)$(OPTWARE_PREFIX)-name 'perllocal.pod' -exec rm -f {} \;
	(cd $(PERL-MIME-BASE64_IPK_DIR)$(OPTWARE_PREFIX)lib/perl5 ; \
		find . -name '*.so' -exec chmod +w {} \; ; \
		find . -name '*.so' -exec $(STRIP_COMMAND) {} \; ; \
		find . -name '*.so' -exec chmod -w {} \; ; \
	)
	find $(PERL-MIME-BASE64_IPK_DIR)$(OPTWARE_PREFIX)-type d -exec chmod go+rx {} \;
