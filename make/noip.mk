...
$(NOIP_BUILD_DIR)/.built: $(NOIP_BUILD_DIR)/.configured
	rm -f $@
	$(MAKE) -C $(NOIP_BUILD_DIR) PREFIX=$(OPTWARE_PREFIX)$(TARGET_CONFIGURE_OPTS)
	touch $@
...
#
# Binaries should be installed into $(NOIP_IPK_DIR)$(OPTWARE_PREFIX)sbin or $(NOIP_IPK_DIR)$(OPTWARE_PREFIX)bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(NOIP_IPK_DIR)$(OPTWARE_PREFIX){lib,include}
# Configuration files should be installed in $(NOIP_IPK_DIR)$(OPTWARE_PREFIX)etc/noip/...
# Documentation files should be installed in $(NOIP_IPK_DIR)$(OPTWARE_PREFIX)doc/noip/...
# Daemon startup scripts should be installed in $(NOIP_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/S??noip
#
# You may need to patch your application to make it use these locations.
#
$(NOIP_IPK): $(NOIP_BUILD_DIR)/.built
	rm -rf $(NOIP_IPK_DIR) $(BUILD_DIR)/noip_*_$(TARGET_ARCH).ipk
	install -d $(NOIP_IPK_DIR)$(OPTWARE_PREFIX)bin
	install -m 755 $(NOIP_BUILD_DIR)/noip2 $(NOIP_IPK_DIR)$(OPTWARE_PREFIX)bin
	install -d $(NOIP_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d
	$(STRIP_COMMAND) $(NOIP_IPK_DIR)$(OPTWARE_PREFIX)bin/noip2
#	install -m 755 $(NOIP_SOURCE_DIR)/rc.noip $(NOIP_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/SXXnoip
