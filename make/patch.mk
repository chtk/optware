...
#
# Binaries should be installed into $(PATCH_IPK_DIR)$(OPTWARE_PREFIX)sbin or $(PATCH_IPK_DIR)$(OPTWARE_PREFIX)bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(PATCH_IPK_DIR)$(OPTWARE_PREFIX){lib,include}
# Configuration files should be installed in $(PATCH_IPK_DIR)$(OPTWARE_PREFIX)etc/patch/...
# Documentation files should be installed in $(PATCH_IPK_DIR)$(OPTWARE_PREFIX)doc/patch/...
# Daemon startup scripts should be installed in $(PATCH_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/S??patch
#
# You may need to patch your application to make it use these locations.
#
$(PATCH_IPK): $(PATCH_BUILD_DIR)/.built
	rm -rf $(PATCH_IPK_DIR) $(BUILD_DIR)/patch_*_$(TARGET_ARCH).ipk
	$(MAKE) -C $(PATCH_BUILD_DIR) prefix=$(PATCH_IPK_DIR)$(OPTWARE_PREFIX)install
	$(STRIP_COMMAND) $(PATCH_IPK_DIR)$(OPTWARE_PREFIX)bin/patch
	mv $(PATCH_IPK_DIR)$(OPTWARE_PREFIX)bin/patch $(PATCH_IPK_DIR)$(OPTWARE_PREFIX)bin/patch-patch
	$(MAKE) $(PATCH_IPK_DIR)/CONTROL/control
	(echo "#!/bin/sh"; \
	 echo "update-alternatives --install $(OPTWARE_PREFIX)bin/patch patch $(OPTWARE_PREFIX)bin/patch-patch 80"; \
	) > $(PATCH_IPK_DIR)/CONTROL/postinst
	(echo "#!/bin/sh"; \
	 echo "update-alternatives --remove patch $(OPTWARE_PREFIX)bin/patch-patch"; \
