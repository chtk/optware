...
OPENVPN_DEPENDS=openssl, lzo, net-tools, psmisc
...
OPENVPN_CONFFILES=$(OPTWARE_PREFIX)etc/openvpn/openvpn.conf $(OPTWARE_PREFIX)etc/openvpn/openvpn.up \
$(OPTWARE_PREFIX)etc/init.d/S20openvpn $(OPTWARE_PREFIX)etc/xinetd.d/openvpn
...
#
# Binaries should be installed into $(OPENVPN_IPK_DIR)$(OPTWARE_PREFIX)sbin or $(OPENVPN_IPK_DIR)$(OPTWARE_PREFIX)bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(OPENVPN_IPK_DIR)$(OPTWARE_PREFIX){lib,include}
# Configuration files should be installed in $(OPENVPN_IPK_DIR)$(OPTWARE_PREFIX)etc/openvpn/...
# Documentation files should be installed in $(OPENVPN_IPK_DIR)$(OPTWARE_PREFIX)doc/openvpn/...
# Daemon startup scripts should be installed in $(OPENVPN_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/S??openvpn
#
# You may need to patch your application to make it use these locations.
#
$(OPENVPN_IPK): $(OPENVPN_BUILD_DIR)/.built
	rm -rf $(OPENVPN_IPK_DIR) $(BUILD_DIR)/openvpn_*_$(TARGET_ARCH).ipk
	# Install server to $(OPTWARE_PREFIX)sbin
	install -d $(OPENVPN_IPK_DIR)$(OPTWARE_PREFIX)sbin
	$(STRIP_COMMAND) $(OPENVPN_BUILD_DIR)/openvpn -o $(OPENVPN_IPK_DIR)$(OPTWARE_PREFIX)sbin/openvpn

	# xinetd startup file
	install -d $(OPENVPN_IPK_DIR)$(OPTWARE_PREFIX)etc/xinetd.d
	install -m 755 $(OPENVPN_SOURCE_DIR)/openvpn.xinetd $(OPENVPN_IPK_DIR)$(OPTWARE_PREFIX)etc/xinetd.d/openvpn

	# init.d startup file
	install -d $(OPENVPN_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d
	install -m 755 $(OPENVPN_SOURCE_DIR)/S20openvpn $(OPENVPN_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d

	# openvpn config files
	install -d $(OPENVPN_IPK_DIR)$(OPTWARE_PREFIX)etc/openvpn
	install -m 644 $(OPENVPN_SOURCE_DIR)/openvpn.conf $(OPENVPN_IPK_DIR)$(OPTWARE_PREFIX)etc/openvpn
	install -m 755 $(OPENVPN_SOURCE_DIR)/openvpn.up $(OPENVPN_IPK_DIR)$(OPTWARE_PREFIX)etc/openvpn

	# openvpn loopback test 
	install -d $(OPENVPN_IPK_DIR)$(OPTWARE_PREFIX)etc/openvpn/sample-config-files
	install -m 644 $(OPENVPN_BUILD_DIR)/sample-config-files/* $(OPENVPN_IPK_DIR)$(OPTWARE_PREFIX)etc/openvpn/sample-config-files

	# openvpn sample keys
	install -d $(OPENVPN_IPK_DIR)$(OPTWARE_PREFIX)etc/openvpn/sample-keys
	install -m 644 $(OPENVPN_BUILD_DIR)/sample-keys/* $(OPENVPN_IPK_DIR)$(OPTWARE_PREFIX)etc/openvpn/sample-keys

	# openvpn sample scripts
	install -d $(OPENVPN_IPK_DIR)$(OPTWARE_PREFIX)etc/openvpn/sample-scripts
	install -m 644 $(OPENVPN_BUILD_DIR)/sample-scripts/* $(OPENVPN_IPK_DIR)$(OPTWARE_PREFIX)etc/openvpn/sample-scripts

	# Install man pages
	install -d $(OPENVPN_IPK_DIR)$(OPTWARE_PREFIX)man/man8
	install -m 644 $(OPENVPN_BUILD_DIR)/openvpn.8 $(OPENVPN_IPK_DIR)$(OPTWARE_PREFIX)man/man8

	# Create log directory
	install -d $(OPENVPN_IPK_DIR)$(OPTWARE_PREFIX)var/log/openvpn
