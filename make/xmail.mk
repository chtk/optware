...
#    everything it installed. e.g $(OPTWARE_PREFIX)var/MailRoot
...
#XMAIL_CONFFILES=$(OPTWARE_PREFIX)etc/xmail.conf $(OPTWARE_PREFIX)etc/init.d/S70xmail
...
#
# Binaries should be installed into $(XMAIL_IPK_DIR)$(OPTWARE_PREFIX)sbin or $(XMAIL_IPK_DIR)$(OPTWARE_PREFIX)bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(XMAIL_IPK_DIR)$(OPTWARE_PREFIX){lib,include}
# Configuration files should be installed in $(XMAIL_IPK_DIR)$(OPTWARE_PREFIX)etc/xmail/...
# Documentation files should be installed in $(XMAIL_IPK_DIR)$(OPTWARE_PREFIX)doc/xmail/...
# Daemon startup scripts should be installed in $(XMAIL_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/S??xmail
#
...
$(XMAIL_IPK): $(XMAIL_BUILD_DIR)/.built
	...
	install -d $(XMAIL_IPK_DIR)$(OPTWARE_PREFIX)var/MailRoot
	cp -R $(XMAIL_BUILD_DIR)/MailRoot/* $(XMAIL_IPK_DIR)$(OPTWARE_PREFIX)var/MailRoot
	#chown root $(XMAIL_IPK_DIR)$(OPTWARE_PREFIX)var/MailRoot
	#chgrp root $(XMAIL_IPK_DIR)$(OPTWARE_PREFIX)var/MailRoot
	install -m 755 $(XMAIL_BUILD_DIR)/xmail $(XMAIL_IPK_DIR)$(OPTWARE_PREFIX)var/MailRoot
	chmod 700 $(XMAIL_IPK_DIR)$(OPTWARE_PREFIX)var/MailRoot
	# The binaries ($(OPTWARE_PREFIX)bin)
	install -d $(XMAIL_IPK_DIR)$(OPTWARE_PREFIX)bin
	install -m 755 $(XMAIL_BUILD_DIR)/bin/CtrlClnt $(XMAIL_IPK_DIR)$(OPTWARE_PREFIX)bin
	install -m 700 $(XMAIL_BUILD_DIR)/bin/MkUsers $(XMAIL_IPK_DIR)$(OPTWARE_PREFIX)bin
	install -m 700 $(XMAIL_BUILD_DIR)/bin/sendmail $(XMAIL_IPK_DIR)$(OPTWARE_PREFIX)bin
	install -m 700 $(XMAIL_BUILD_DIR)/bin/XMail $(XMAIL_IPK_DIR)$(OPTWARE_PREFIX)bin
	install -m 700 $(XMAIL_BUILD_DIR)/bin/XMCrypt $(XMAIL_IPK_DIR)$(OPTWARE_PREFIX)bin
	# The docs ($(OPTWARE_PREFIX)doc)
	install -d $(XMAIL_IPK_DIR)$(OPTWARE_PREFIX)doc/xmail
	install -m 755 $(XMAIL_BUILD_DIR)/docs/Readme.txt $(XMAIL_IPK_DIR)$(OPTWARE_PREFIX)doc/xmail
	install -m 755 $(XMAIL_BUILD_DIR)/docs/Readme.html $(XMAIL_IPK_DIR)$(OPTWARE_PREFIX)doc/xmail
	# rc  ($(OPTWARE_PREFIX)etc/init.d)
	# This is handled by the postinst script
	# Rest of the stuff
	$(MAKE) $(XMAIL_IPK_DIR)/CONTROL/control
	install -d $(XMAIL_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d
	...
	(cd $(XMAIL_BUILD_DIR)/MailRoot && \
	 ...
	 sed 's|^\.|$(OPTWARE_PREFIX)var/MailRoot|') > $(XMAIL_IPK_DIR)/CONTROL/conffiles
