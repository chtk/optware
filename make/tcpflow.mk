...
#TCPFLOW_CONFFILES=$(OPTWARE_PREFIX)etc/tcpflow.conf $(OPTWARE_PREFIX)etc/init.d/SXXtcpflow
...
#
# Binaries should be installed into $(TCPFLOW_IPK_DIR)$(OPTWARE_PREFIX)sbin or $(TCPFLOW_IPK_DIR)$(OPTWARE_PREFIX)bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(TCPFLOW_IPK_DIR)$(OPTWARE_PREFIX){lib,include}
# Configuration files should be installed in $(TCPFLOW_IPK_DIR)$(OPTWARE_PREFIX)etc/tcpflow/...
# Documentation files should be installed in $(TCPFLOW_IPK_DIR)$(OPTWARE_PREFIX)doc/tcpflow/...
# Daemon startup scripts should be installed in $(TCPFLOW_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/S??tcpflow
#
...
$(TCPFLOW_IPK): $(TCPFLOW_BUILD_DIR)/.built
	...
	$(STRIP_COMMAND) $(TCPFLOW_IPK_DIR)$(OPTWARE_PREFIX)bin/tcpflow
#	install -d $(TCPFLOW_IPK_DIR)$(OPTWARE_PREFIX)etc/
#	install -m 644 $(TCPFLOW_SOURCE_DIR)/tcpflow.conf $(TCPFLOW_IPK_DIR)$(OPTWARE_PREFIX)etc/tcpflow.conf
#	install -d $(TCPFLOW_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d
#	install -m 755 $(TCPFLOW_SOURCE_DIR)/rc.tcpflow $(TCPFLOW_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/SXXtcpflow
#	sed -i -e '/^#!/aOPTWARE_TARGET=${OPTWARE_TARGET}' $(TCPFLOW_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/SXXtcpflow
