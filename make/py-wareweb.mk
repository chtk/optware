...
#PY-WAREWEB_CONFFILES=$(OPTWARE_PREFIX)etc/py-wareweb.conf $(OPTWARE_PREFIX)etc/init.d/SXXpy-wareweb
...
ifeq ($(PY-WAREWEB_SVN_REV),)
	...
	(cd $(PY-WAREWEB_BUILD_DIR); \
	    (echo "[build_scripts]"; echo "executable=$(OPTWARE_PREFIX)bin/python") >> setup.cfg \
	)
...
#
# Binaries should be installed into $(PY-WAREWEB_IPK_DIR)$(OPTWARE_PREFIX)sbin or $(PY-WAREWEB_IPK_DIR)$(OPTWARE_PREFIX)bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(PY-WAREWEB_IPK_DIR)$(OPTWARE_PREFIX){lib,include}
# Configuration files should be installed in $(PY-WAREWEB_IPK_DIR)$(OPTWARE_PREFIX)etc/py-wareweb/...
# Documentation files should be installed in $(PY-WAREWEB_IPK_DIR)$(OPTWARE_PREFIX)doc/py-wareweb/...
# Daemon startup scripts should be installed in $(PY-WAREWEB_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/S??py-wareweb
#
...
$(PY-WAREWEB_IPK): $(PY-WAREWEB_BUILD_DIR)/.built
	...
	PYTHONPATH=$(STAGING_LIB_DIR)/python2.4/site-packages \
		...
		--root=$(PY-WAREWEB_IPK_DIR) --prefix=$(OPTWARE_PREFIX)--single-version-externally-managed)
