...
STUPID-FTPD_CONFFILES=$(OPTWARE_PREFIX)etc/stupid-ftpd.conf
#$(OPTWARE_PREFIX)etc/init.d/SXXstupid-ftpd
...
#	$(MAKE) <bar>-stage <baz>-stage
	...
	(cd $(@D); \
		sed -i -e "/^CC/c \\" -e "CC=$(TARGET_CC)" -e "/^CFLAGS/c \\" \
			...
			Makefile ; \
		sed -i -e 's|/etc/stupid-ftpd/|$(OPTWARE_PREFIX)etc/|' ftpdconfig.c stupid-ftpd.conf; \
...
#
# Binaries should be installed into $(STUPID-FTPD_IPK_DIR)$(OPTWARE_PREFIX)sbin or $(STUPID-FTPD_IPK_DIR)$(OPTWARE_PREFIX)bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(STUPID-FTPD_IPK_DIR)$(OPTWARE_PREFIX){lib,include}
# Configuration files should be installed in $(STUPID-FTPD_IPK_DIR)$(OPTWARE_PREFIX)etc/stupid-ftpd/...
# Documentation files should be installed in $(STUPID-FTPD_IPK_DIR)$(OPTWARE_PREFIX)doc/stupid-ftpd/...
# Daemon startup scripts should be installed in $(STUPID-FTPD_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/S??stupid-ftpd
#
...
$(STUPID-FTPD_IPK): $(STUPID-FTPD_BUILD_DIR)/.built
	...
	install -d $(STUPID-FTPD_IPK_DIR)$(OPTWARE_PREFIX)sbin/
	install -m 755 $(STUPID-FTPD_BUILD_DIR)/stupid-ftpd.Linux6 \
		$(STUPID-FTPD_IPK_DIR)$(OPTWARE_PREFIX)sbin/stupid-ftpd
	$(STRIP_COMMAND) $(STUPID-FTPD_IPK_DIR)$(OPTWARE_PREFIX)sbin/stupid-ftpd
	install -d $(STUPID-FTPD_IPK_DIR)$(OPTWARE_PREFIX)etc/
	install -m 644 $(STUPID-FTPD_BUILD_DIR)/stupid-ftpd.conf \
		$(STUPID-FTPD_IPK_DIR)$(OPTWARE_PREFIX)etc/stupid-ftpd.conf
#	install -d $(STUPID-FTPD_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d
#	install -m 755 $(STUPID-FTPD_SOURCE_DIR)/rc.stupid-ftpd $(STUPID-FTPD_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/SXXstupid-ftpd
#	sed -i -e '/^#!/aOPTWARE_TARGET=${OPTWARE_TARGET}' $(STUPID-FTPD_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/SXXstupid-ftpd
