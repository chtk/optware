...
#NETCAT_CONFFILES=$(OPTWARE_PREFIX)etc/netcat.conf $(OPTWARE_PREFIX)etc/init.d/SXXnetcat
...
#	$(MAKE) <bar>-stage <baz>-stage
	...
	(cd $(@D); \
		...
		sed -i -e 's|/usr/share|$(OPTWARE_PREFIX)share|g' debian/nc.1; \
	)
	touch $@
...
#
# Binaries should be installed into $(NETCAT_IPK_DIR)$(OPTWARE_PREFIX)sbin or $(NETCAT_IPK_DIR)$(OPTWARE_PREFIX)bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(NETCAT_IPK_DIR)$(OPTWARE_PREFIX){lib,include}
# Configuration files should be installed in $(NETCAT_IPK_DIR)$(OPTWARE_PREFIX)etc/netcat/...
# Documentation files should be installed in $(NETCAT_IPK_DIR)$(OPTWARE_PREFIX)doc/netcat/...
# Daemon startup scripts should be installed in $(NETCAT_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/S??netcat
#
# You may need to patch your application to make it use these locations.
#
$(NETCAT_IPK): $(NETCAT_BUILD_DIR)/.built
	rm -rf $(NETCAT_IPK_DIR) $(BUILD_DIR)/netcat_*_$(TARGET_ARCH).ipk
	install -d $(NETCAT_IPK_DIR)$(OPTWARE_PREFIX)bin
	install $(NETCAT_BUILD_DIR)/nc $(NETCAT_IPK_DIR)$(OPTWARE_PREFIX)bin/netcat-nc
	install -d $(NETCAT_IPK_DIR)$(OPTWARE_PREFIX)man/man1
	install $(NETCAT_BUILD_DIR)/debian/nc.1 $(NETCAT_IPK_DIR)$(OPTWARE_PREFIX)man/man1
	install -d $(NETCAT_IPK_DIR)$(OPTWARE_PREFIX)share/doc/netcat
	gzip -c $(NETCAT_BUILD_DIR)/README > $(NETCAT_IPK_DIR)$(OPTWARE_PREFIX)share/doc/netcat/README.gz
	$(STRIP_COMMAND) $(NETCAT_IPK_DIR)$(OPTWARE_PREFIX)bin/netcat-nc
	$(MAKE) $(NETCAT_IPK_DIR)/CONTROL/control
	(echo "#!/bin/sh"; \
	 echo "update-alternatives --install $(OPTWARE_PREFIX)bin/nc nc $(OPTWARE_PREFIX)bin/netcat-nc 80"; \
	) > $(NETCAT_IPK_DIR)/CONTROL/postinst
	(echo "#!/bin/sh"; \
	 echo "update-alternatives --remove nc $(OPTWARE_PREFIX)bin/netcat-nc"; \
