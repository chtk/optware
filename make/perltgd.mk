...
PERLTGD_DEPENDS=perl, cron, which
PERLTGD_SUGGESTS=ftpd-topfield, puppy, wget, wput
PERLTGD_CONFLICTS=
...
PERLTGD_CONFFILES= \
	$(OPTWARE_PREFIX)etc/perltgd/perltgd.settings \
	$(OPTWARE_PREFIX)etc/perltgd/xmltv2tgd.settings \
	$(OPTWARE_PREFIX)etc/perltgd/append.timers \
	$(OPTWARE_PREFIX)etc/perltgd/favourites.ini \
	$(OPTWARE_PREFIX)etc/perltgd/overrun.shows \
	$(OPTWARE_PREFIX)etc/perltgd/shows.repeat \
	$(OPTWARE_PREFIX)etc/cron.d/perltgd
...
#
# Binaries should be installed into $(PERLTGD_IPK_DIR)$(OPTWARE_PREFIX)sbin or $(PERLTGD_IPK_DIR)$(OPTWARE_PREFIX)bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(PERLTGD_IPK_DIR)$(OPTWARE_PREFIX){lib,include}
# Configuration files should be installed in $(PERLTGD_IPK_DIR)$(OPTWARE_PREFIX)etc/perltgd/...
# Documentation files should be installed in $(PERLTGD_IPK_DIR)$(OPTWARE_PREFIX)doc/perltgd/...
# Daemon startup scripts should be installed in $(PERLTGD_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/S??perltgd
#
# You may need to patch your application to make it use these locations.
#
$(PERLTGD_IPK): $(PERLTGD_BUILD_DIR)/.built
	rm -rf $(PERLTGD_IPK_DIR) $(BUILD_DIR)/perltgd_*_$(TARGET_ARCH).ipk
	install -d $(PERLTGD_IPK_DIR)$(OPTWARE_PREFIX)bin
	install -m 755 $(PERLTGD_BUILD_DIR)/perltgdcli.pl $(PERLTGD_IPK_DIR)$(OPTWARE_PREFIX)bin/perltgd
	install -m 755 $(PERLTGD_BUILD_DIR)/xmltv2tgd.pl $(PERLTGD_IPK_DIR)$(OPTWARE_PREFIX)bin/xmltv2tgd
	install -d $(PERLTGD_IPK_DIR)$(OPTWARE_PREFIX)etc/perltgd
	install -m 644 $(PERLTGD_SOURCE_DIR)/perltgd.settings $(PERLTGD_IPK_DIR)$(OPTWARE_PREFIX)etc/perltgd/
	install -m 644 $(PERLTGD_SOURCE_DIR)/xmltv2tgd.settings $(PERLTGD_IPK_DIR)$(OPTWARE_PREFIX)etc/perltgd/
	install -m 644 $(PERLTGD_BUILD_DIR)/perlTGDslug/append.timers $(PERLTGD_IPK_DIR)$(OPTWARE_PREFIX)etc/perltgd/
	install -m 644 $(PERLTGD_BUILD_DIR)/perlTGDslug/favourites.ini $(PERLTGD_IPK_DIR)$(OPTWARE_PREFIX)etc/perltgd/
	install -m 644 $(PERLTGD_BUILD_DIR)/perlTGDslug/overrun.shows $(PERLTGD_IPK_DIR)$(OPTWARE_PREFIX)etc/perltgd/
	install -m 644 $(PERLTGD_BUILD_DIR)/perlTGDslug/shows.repeat $(PERLTGD_IPK_DIR)$(OPTWARE_PREFIX)etc/perltgd/
	install -d $(PERLTGD_IPK_DIR)$(OPTWARE_PREFIX)etc/cron.d
	install -m 600 $(PERLTGD_SOURCE_DIR)/cron.perltgd $(PERLTGD_IPK_DIR)$(OPTWARE_PREFIX)etc/cron.d/perltgd
