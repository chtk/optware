...
#MODULE_INIT_TOOLS_CONFFILES=$(OPTWARE_PREFIX)etc/module-init-tools.conf $(OPTWARE_PREFIX)etc/init.d/SXXmodule-init-tools
...
MODULE_INIT_TOOLS_CONFIGURE_OPTIONS=--with-moddir=$(OPTWARE_PREFIX)lib/modules
endif
...
#
# Binaries should be installed into $(MODULE_INIT_TOOLS_IPK_DIR)$(OPTWARE_PREFIX)sbin or $(MODULE_INIT_TOOLS_IPK_DIR)$(OPTWARE_PREFIX)bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(MODULE_INIT_TOOLS_IPK_DIR)$(OPTWARE_PREFIX){lib,include}
# Configuration files should be installed in $(MODULE_INIT_TOOLS_IPK_DIR)$(OPTWARE_PREFIX)etc/module-init-tools/...
# Documentation files should be installed in $(MODULE_INIT_TOOLS_IPK_DIR)$(OPTWARE_PREFIX)doc/module-init-tools/...
# Daemon startup scripts should be installed in $(MODULE_INIT_TOOLS_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/S??module-init-tools
#
...
$(MODULE_INIT_TOOLS_IPK): $(MODULE_INIT_TOOLS_BUILD_DIR)/.built
	...
	cd $(MODULE_INIT_TOOLS_IPK_DIR)$(OPTWARE_PREFIX)sbin; \
	for f in depmod insmod modprobe rmmod; do \
	    mv $$f module-init-tools-$$f; \
	    echo "update-alternatives --install $(OPTWARE_PREFIX)sbin/$$f $$f $(OPTWARE_PREFIX)sbin/module-init-tools-$$f 80" \
		>> $(MODULE_INIT_TOOLS_IPK_DIR)/CONTROL/postinst; \
	    echo "update-alternatives --remove $$f $(OPTWARE_PREFIX)sbin/module-init-tools-$$f" \
