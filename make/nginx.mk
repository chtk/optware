...
NGINX_CONFFILES=\
	$(OPTWARE_PREFIX)etc/default/nginx \
	$(OPTWARE_PREFIX)etc/nginx/fastcgi_params \
	$(OPTWARE_PREFIX)etc/nginx/mime.types \
	$(OPTWARE_PREFIX)etc/nginx/nginx.conf \
	$(OPTWARE_PREFIX)share/nginx/html/index.html \
	$(OPTWARE_PREFIX)share/nginx/html/50x.html
...
#		--build=$(GNU_HOST_NAME) \
	...
	(cd $(@D); \
	    ...
	    $(NGINX_CONFIGURE_ENV) \
	    ./configure \
		--prefix=$(OPTWARE_PREFIX)share/nginx \
		--sbin-path=$(OPTWARE_PREFIX)sbin/nginx \
		--conf-path=$(OPTWARE_PREFIX)etc/nginx/nginx.conf \
		--error-log-path=$(OPTWARE_PREFIX)var/nginx/log/error.log \
		--pid-path=$(OPTWARE_PREFIX)var/nginx/run/nginx.pid \
		--lock-path=$(OPTWARE_PREFIX)var/nginx/run/nginx.lock \
		--http-log-path=$(OPTWARE_PREFIX)var/nginx/log/access.log \
		--http-client-body-temp-path=$(OPTWARE_PREFIX)var/nginx/tmp/client_body_temp \
		--http-proxy-temp-path=$(OPTWARE_PREFIX)var/nginx/tmp/proxy_temp \
		--http-fastcgi-temp-path=$(OPTWARE_PREFIX)var/nginx/tmp/fastcgi_temp \
                --with-cc=$(TARGET_CC) \
                --with-cpp=$(TARGET_CPP) \
	...
	sed -i.orig \
                ...
                -e '/^CFLAGS/{s| -Werror||;s|-I$(OPTWARE_PREFIX)include||;}' \
                $(@D)/objs/Makefile
...
#
# Binaries should be installed into $(NGINX_IPK_DIR)$(OPTWARE_PREFIX)sbin or $(NGINX_IPK_DIR)$(OPTWARE_PREFIX)bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(NGINX_IPK_DIR)$(OPTWARE_PREFIX){lib,include}
# Configuration files should be installed in $(NGINX_IPK_DIR)$(OPTWARE_PREFIX)etc/nginx/...
# Documentation files should be installed in $(NGINX_IPK_DIR)$(OPTWARE_PREFIX)doc/nginx/...
# Daemon startup scripts should be installed in $(NGINX_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/S??nginx
#
...
$(NGINX_IPK): $(NGINX_BUILD_DIR)/.built
	...
	sed -i -e "/^[ 	]*listen/s|listen.*80;|listen\t8082;|" $(NGINX_IPK_DIR)$(OPTWARE_PREFIX)etc/nginx/nginx.conf
	$(STRIP_COMMAND) $(NGINX_IPK_DIR)$(OPTWARE_PREFIX)sbin/nginx
	install -d $(NGINX_IPK_DIR)$(OPTWARE_PREFIX)var/nginx/tmp
	install -d $(NGINX_IPK_DIR)$(OPTWARE_PREFIX)share/www
	ln -s $(OPTWARE_PREFIX)share/nginx/html $(NGINX_IPK_DIR)$(OPTWARE_PREFIX)share/www/nginx
	install -d $(NGINX_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d
	install -m 755 $(NGINX_SOURCE_DIR)/rc.nginx $(NGINX_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/S80nginx
	install -d $(NGINX_IPK_DIR)$(OPTWARE_PREFIX)etc/default
	install -m 755 $(NGINX_SOURCE_DIR)/default $(NGINX_IPK_DIR)$(OPTWARE_PREFIX)etc/default/nginx
