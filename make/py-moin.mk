...
PY26-MOIN_DEPENDS=python26, coreutils, sed, tar
...
#PY-MOIN_CONFFILES=$(OPTWARE_PREFIX)etc/py-moin.conf $(OPTWARE_PREFIX)etc/init.d/SXXpy-moin
...
PY26-MOIN_IPK_DIR=$(BUILD_DIR)/py26-moin-$(PY-MOIN_VERSION)-ipk
PY26-MOIN_IPK=$(BUILD_DIR)/py26-moin_$(PY-MOIN_VERSION)-$(PY-MOIN_IPK_VERSION)_$(TARGET_ARCH).ipk
...
$(PY-MOIN_BUILD_DIR)/.configured: $(DL_DIR)/$(PY-MOIN_SOURCE) $(PY-MOIN_PATCHES)
	...
	sed -i -e 's|/usr/lib/python/|$(OPTWARE_PREFIX)lib/python2.6/|' $(@D)/2.6/setup.py
	(echo "[build_scripts]"; \
         echo "executable=$(OPTWARE_PREFIX)bin/python2.6") >> $(@D)/2.6/setup.cfg
	# 2.5
	rm -rf $(BUILD_DIR)/$(PY-MOIN_DIR)
	$(PY-MOIN_UNZIP) $(DL_DIR)/$(PY-MOIN_SOURCE) | tar -C $(BUILD_DIR) -xvf -
	cat $(PY-MOIN_PATCHES) | patch -d $(BUILD_DIR)/$(PY-MOIN_DIR) -p1
	mv $(BUILD_DIR)/$(PY-MOIN_DIR) $(@D)/2.5
	sed -i -e 's|/usr/lib/python/|$(OPTWARE_PREFIX)lib/python2.5/|' $(@D)/2.5/setup.py
	(echo "[build_scripts]"; \
         echo "executable=$(OPTWARE_PREFIX)bin/python2.5") >> $(@D)/2.5/setup.cfg
	touch $@
...
$(PY26-MOIN_IPK_DIR)/CONTROL/control:
	...
	@echo "Source: $(PY-MOIN_SITE)/$(PY-MOIN_SOURCE)" >>$@
	@echo "Description: $(PY-MOIN_DESCRIPTION)" >>$@
	@echo "Depends: py-moin-common, $(PY26-MOIN_DEPENDS)" >>$@
	@echo "Conflicts: $(PY-MOIN_CONFLICTS)" >>$@
...
#
# Binaries should be installed into $(PY-MOIN_IPK_DIR)$(OPTWARE_PREFIX)sbin or $(PY-MOIN_IPK_DIR)$(OPTWARE_PREFIX)bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(PY-MOIN_IPK_DIR)$(OPTWARE_PREFIX){lib,include}
# Configuration files should be installed in $(PY-MOIN_IPK_DIR)$(OPTWARE_PREFIX)etc/py-moin/...
# Documentation files should be installed in $(PY-MOIN_IPK_DIR)$(OPTWARE_PREFIX)doc/py-moin/...
# Daemon startup scripts should be installed in $(PY-MOIN_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/S??py-moin
#
# You may need to patch your application to make it use these locations.
#
$(PY26-MOIN_IPK): $(PY-MOIN_BUILD_DIR)/.built
	rm -rf $(BUILD_DIR)/py24-moin_*_$(TARGET_ARCH).ipk
	...
	for f in $(PY26-MOIN_IPK_DIR)$(OPTWARE_PREFIX)bin/*; \
		do mv $$f `echo $$f | sed 's|$$|-2.6|'`; done
	rm -rf $(PY26-MOIN_IPK_DIR)$(OPTWARE_PREFIX)share/
	sed -e 's|python2.[4-9]|python2.6|' $(PY-MOIN_SOURCE_DIR)/createinstance.sh \
		> $(PY26-MOIN_IPK_DIR)$(OPTWARE_PREFIX)bin/py26-moin-createinstance.sh
	chmod 755 $(PY26-MOIN_IPK_DIR)$(OPTWARE_PREFIX)bin/py26-moin-createinstance.sh
...
$(PY25-MOIN_IPK) $(PY-MOIN-COMMON_IPK): $(PY-MOIN_BUILD_DIR)/.built
	...
	cd $(PY25-MOIN_IPK_DIR)$(OPTWARE_PREFIX)share/moin; \
	    tar --remove-files -cvzf underlay.tar.gz underlay; \
	    rm -rf underlay
	sed -e 's|python2.[4-9]|python2.5|' $(PY-MOIN_SOURCE_DIR)/createinstance.sh \
		> $(PY25-MOIN_IPK_DIR)$(OPTWARE_PREFIX)bin/py25-moin-createinstance.sh
	chmod 755 $(PY25-MOIN_IPK_DIR)$(OPTWARE_PREFIX)bin/py25-moin-createinstance.sh
	$(MAKE) $(PY25-MOIN_IPK_DIR)/CONTROL/control
	$(MAKE) $(PY-MOIN-COMMON_IPK_DIR)/CONTROL/control
	install -d $(PY-MOIN-COMMON_IPK_DIR)$(OPTWARE_PREFIX)
	mv $(PY25-MOIN_IPK_DIR)$(OPTWARE_PREFIX)share $(PY-MOIN-COMMON_IPK_DIR)$(OPTWARE_PREFIX)
#	chmod o+r $(PY-MOIN-COMMON_IPK_DIR)$(OPTWARE_PREFIX)share/moin/data/*-log
	for f in wikiserver.py wikiserverconfig.py wikiserverlogging.conf; \
	do install $(PY-MOIN_BUILD_DIR)/2.5/$${f} $(PY-MOIN-COMMON_IPK_DIR)$(OPTWARE_PREFIX)share/moin/$${f}; done
...
#
py-moin-ipk: $(PY26-MOIN_IPK) $(PY25-MOIN_IPK) $(PY-MOIN-COMMON_IPK)
...
py-moin-dirclean:
	...
	rm -rf $(PY26-MOIN_IPK_DIR) $(PY26-MOIN_IPK)
