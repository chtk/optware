...
#HTTPING_CONFFILES=$(OPTWARE_PREFIX)etc/httping.conf $(OPTWARE_PREFIX)etc/init.d/SXXhttping
...
$(HTTPING_BUILD_DIR)/.configured: $(DL_DIR)/$(HTTPING_SOURCE) $(HTTPING_PATCHES) make/httping.mk
	...
	sed -i -e 's:/usr/:$(OPTWARE_PREFIX)/:g' $(@D)/Makefile
#	(cd $(@D); \
...
#
# Binaries should be installed into $(HTTPING_IPK_DIR)$(OPTWARE_PREFIX)sbin or $(HTTPING_IPK_DIR)$(OPTWARE_PREFIX)bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(HTTPING_IPK_DIR)$(OPTWARE_PREFIX){lib,include}
# Configuration files should be installed in $(HTTPING_IPK_DIR)$(OPTWARE_PREFIX)etc/httping/...
# Documentation files should be installed in $(HTTPING_IPK_DIR)$(OPTWARE_PREFIX)doc/httping/...
# Daemon startup scripts should be installed in $(HTTPING_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/S??httping
#
...
$(HTTPING_IPK): $(HTTPING_BUILD_DIR)/.built
	...
	$(MAKE) -C $(HTTPING_BUILD_DIR) DESTDIR=$(HTTPING_IPK_DIR) install \
		STRIP=: PREFIX=$(OPTWARE_PREFIX)
	$(STRIP_COMMAND) $(HTTPING_IPK_DIR)$(OPTWARE_PREFIX)/bin/*
