...
#PICOCOM_CONFFILES=$(OPTWARE_PREFIX)etc/picocom.conf $(OPTWARE_PREFIX)etc/init.d/SXXpicocom
...
#
PICOCOM_CPPFLAGS=-DVERSION_STR=\\\"$(PICOCOM_VERSION)\\\" -DUUCP_LOCK_DIR=\\\"$(OPTWARE_PREFIX)var/lock\\\"
...
#
# Binaries should be installed into $(PICOCOM_IPK_DIR)$(OPTWARE_PREFIX)sbin or $(PICOCOM_IPK_DIR)$(OPTWARE_PREFIX)bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(PICOCOM_IPK_DIR)$(OPTWARE_PREFIX){lib,include}
# Configuration files should be installed in $(PICOCOM_IPK_DIR)$(OPTWARE_PREFIX)etc/picocom/...
# Documentation files should be installed in $(PICOCOM_IPK_DIR)$(OPTWARE_PREFIX)doc/picocom/...
# Daemon startup scripts should be installed in $(PICOCOM_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/S??picocom
#
...
#	$(MAKE) -C $(PICOCOM_BUILD_DIR) DESTDIR=$(PICOCOM_IPK_DIR) install-strip
	install -d $(PICOCOM_IPK_DIR)$(OPTWARE_PREFIX)bin $(PICOCOM_IPK_DIR)$(OPTWARE_PREFIX)share/doc/picocom $(PICOCOM_IPK_DIR)$(OPTWARE_PREFIX)share/man/man8
	install -m 755 $(PICOCOM_BUILD_DIR)/picocom $(PICOCOM_IPK_DIR)$(OPTWARE_PREFIX)bin/
	$(STRIP_COMMAND) $(PICOCOM_IPK_DIR)$(OPTWARE_PREFIX)bin/picocom
	install -m 755 $(PICOCOM_BUILD_DIR)/pc* $(PICOCOM_IPK_DIR)$(OPTWARE_PREFIX)bin/
	install -m 644 $(PICOCOM_BUILD_DIR)/picocom.8 $(PICOCOM_IPK_DIR)$(OPTWARE_PREFIX)share/man/man8/
	install -m 644 $(PICOCOM_BUILD_DIR)/picocom.8.html $(PICOCOM_BUILD_DIR)/picocom.8.ps \
		$(PICOCOM_IPK_DIR)$(OPTWARE_PREFIX)share/doc/picocom/
#	install -m 644 $(PICOCOM_SOURCE_DIR)/picocom.conf $(PICOCOM_IPK_DIR)$(OPTWARE_PREFIX)etc/picocom.conf
#	install -d $(PICOCOM_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d
#	install -m 755 $(PICOCOM_SOURCE_DIR)/rc.picocom $(PICOCOM_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/SXXpicocom
#	sed -i -e '/^#!/aOPTWARE_TARGET=${OPTWARE_TARGET}' $(XINETD_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/SXXpicocom
