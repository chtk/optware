...
#TTF_BITSTREAM_VERA_CONFFILES=$(OPTWARE_PREFIX)etc/ttf_bitstream_vera.conf $(OPTWARE_PREFIX)etc/init.d/SXXttf_bitstream_vera
...
#
# Binaries should be installed into $(TTF_BITSTREAM_VERA_IPK_DIR)$(OPTWARE_PREFIX)sbin or $(TTF_BITSTREAM_VERA_IPK_DIR)$(OPTWARE_PREFIX)bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(TTF_BITSTREAM_VERA_IPK_DIR)$(OPTWARE_PREFIX){lib,include}
# Configuration files should be installed in $(TTF_BITSTREAM_VERA_IPK_DIR)$(OPTWARE_PREFIX)etc/ttf_bitstream_vera/...
# Documentation files should be installed in $(TTF_BITSTREAM_VERA_IPK_DIR)$(OPTWARE_PREFIX)doc/ttf_bitstream_vera/...
# Daemon startup scripts should be installed in $(TTF_BITSTREAM_VERA_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/S??ttf_bitstream_vera
#
...
$(TTF_BITSTREAM_VERA_IPK): $(TTF_BITSTREAM_VERA_BUILD_DIR)/.configured
	...
	install -d $(TTF_BITSTREAM_VERA_IPK_DIR)$(OPTWARE_PREFIX)share/fonts/bitstream-vera
	install -m 644 $(TTF_BITSTREAM_VERA_BUILD_DIR)/*.ttf $(TTF_BITSTREAM_VERA_IPK_DIR)$(OPTWARE_PREFIX)share/fonts/bitstream-vera
