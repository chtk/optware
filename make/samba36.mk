...
SAMBA36_CONFFILES=$(OPTWARE_PREFIX)/etc/init.d/S08samba
SAMBA36-SWAT_CONFFILES=$(OPTWARE_PREFIX)/etc/xinetd.d/swat
...
SAMBA36_INST_DIR=$(OPTWARE_PREFIX)
...
#
# Binaries should be installed into $(SAMBA36_IPK_DIR)$(OPTWARE_PREFIX)sbin or $(SAMBA36_IPK_DIR)$(OPTWARE_PREFIX)bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(SAMBA36_IPK_DIR)$(OPTWARE_PREFIX){lib,include}
# Configuration files should be installed in $(SAMBA36_IPK_DIR)$(OPTWARE_PREFIX)etc/samba/...
# Documentation files should be installed in $(SAMBA36_IPK_DIR)$(OPTWARE_PREFIX)doc/samba/...
# Daemon startup scripts should be installed in $(SAMBA36_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/S??samba
#
...
$(SAMBA36_IPK) $(SAMBA36-DEV_IPK) $(SAMBA36-SWAT_IPK): $(SAMBA36_BUILD_DIR)/.built
	...
	$(STRIP_COMMAND) `ls $(SAMBA36_IPK_DIR)$(OPTWARE_PREFIX)/sbin/* | egrep -v 'mount.smbfs'`
	$(STRIP_COMMAND) `ls $(SAMBA36_IPK_DIR)$(OPTWARE_PREFIX)/bin/* | egrep -v 'findsmb|smbtar'`
	cd $(SAMBA36_BUILD_DIR)/source3/bin/; for f in lib*.so.[01]; \
		do cp -a $$f $(SAMBA36_IPK_DIR)$(OPTWARE_PREFIX)/lib/$$f; done
	$(STRIP_COMMAND) `find $(SAMBA36_IPK_DIR)$(OPTWARE_PREFIX)/lib -name '*.so'`
	install -d $(SAMBA36_IPK_DIR)$(OPTWARE_PREFIX)/etc/init.d
	install -m 755 $(SAMBA36_SOURCE_DIR)/rc.samba $(SAMBA36_IPK_DIR)$(OPTWARE_PREFIX)/etc/init.d/S08samba
	...
	# samba3-dev
	install -d $(SAMBA36-DEV_IPK_DIR)$(OPTWARE_PREFIX)
	mv $(SAMBA36_IPK_DIR)$(OPTWARE_PREFIX)/include $(SAMBA36-DEV_IPK_DIR)$(OPTWARE_PREFIX)
	# samba3-swat
	install -d $(SAMBA36-SWAT_IPK_DIR)$(OPTWARE_PREFIX)/share $(SAMBA36-SWAT_IPK_DIR)$(OPTWARE_PREFIX)/sbin
	mv $(SAMBA36_IPK_DIR)$(OPTWARE_PREFIX)/share/swat $(SAMBA36-SWAT_IPK_DIR)$(OPTWARE_PREFIX)/share/
	mv $(SAMBA36_IPK_DIR)$(OPTWARE_PREFIX)/sbin/swat $(SAMBA36-SWAT_IPK_DIR)$(OPTWARE_PREFIX)/sbin/
	install -d $(SAMBA36-SWAT_IPK_DIR)$(OPTWARE_PREFIX)/etc/xinetd.d
	install -m 755 $(SAMBA36_SOURCE_DIR)/swat $(SAMBA36-SWAT_IPK_DIR)$(OPTWARE_PREFIX)/etc/xinetd.d/swat
	# building ipk's
	sed -i -e "s,/opt/,$(OPTWARE_PREFIX)/,g" \
		$(subst $(OPTWARE_PREFIX),$(SAMBA36_IPK_DIR)$(OPTWARE_PREFIX),$(SAMBA36_CONFFILES)) \
		$(SAMBA36_IPK_DIR)/CONTROL/postinst
	...
	sed -i -e "s,/opt/,$(OPTWARE_PREFIX)/,g" \
		$(subst $(OPTWARE_PREFIX),$(SAMBA36-SWAT_IPK_DIR)$(OPTWARE_PREFIX),$(SAMBA36-SWAT_CONFFILES))
