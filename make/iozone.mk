...
#IOZONE_CONFFILES=$(OPTWARE_PREFIX)etc/iozone.conf $(OPTWARE_PREFIX)etc/init.d/SXXiozone
...
#
# Binaries should be installed into $(IOZONE_IPK_DIR)$(OPTWARE_PREFIX)sbin or $(IOZONE_IPK_DIR)$(OPTWARE_PREFIX)bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(IOZONE_IPK_DIR)$(OPTWARE_PREFIX){lib,include}
# Configuration files should be installed in $(IOZONE_IPK_DIR)$(OPTWARE_PREFIX)etc/iozone/...
# Documentation files should be installed in $(IOZONE_IPK_DIR)$(OPTWARE_PREFIX)doc/iozone/...
# Daemon startup scripts should be installed in $(IOZONE_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/S??iozone
#
# You may need to patch your application to make it use these locations.
#
$(IOZONE_IPK): $(IOZONE_BUILD_DIR)/.built
	rm -rf $(IOZONE_IPK_DIR) $(BUILD_DIR)/iozone_*_$(TARGET_ARCH).ipk
	install -d $(IOZONE_IPK_DIR)$(OPTWARE_PREFIX)bin/
	$(STRIP_COMMAND) $(IOZONE_BUILD_DIR)/src/current/iozone
	install -m 755 $(IOZONE_BUILD_DIR)/src/current/iozone $(IOZONE_IPK_DIR)$(OPTWARE_PREFIX)bin
	install -d $(IOZONE_IPK_DIR)$(OPTWARE_PREFIX)lib/iozone/
	install -m 755 $(IOZONE_BUILD_DIR)/src/current/Generate_Graphs $(IOZONE_IPK_DIR)$(OPTWARE_PREFIX)lib/iozone
	install -m 755 $(IOZONE_BUILD_DIR)/src/current/gengnuplot.sh $(IOZONE_IPK_DIR)$(OPTWARE_PREFIX)lib/iozone
	install -m 755 $(IOZONE_BUILD_DIR)/src/current/gnu3d.dem $(IOZONE_IPK_DIR)$(OPTWARE_PREFIX)lib/iozone
	install -d $(IOZONE_IPK_DIR)$(OPTWARE_PREFIX)share/doc/iozone
	install -m 644 $(IOZONE_BUILD_DIR)/docs/IOzone_msword_98.pdf $(IOZONE_IPK_DIR)$(OPTWARE_PREFIX)share/doc/iozone
	install -m 644 $(IOZONE_BUILD_DIR)/docs/Run_rules.doc $(IOZONE_IPK_DIR)$(OPTWARE_PREFIX)share/doc/iozone
	install -m 644 $(IOZONE_BUILD_DIR)/docs/Iozone_ps.gz $(IOZONE_IPK_DIR)$(OPTWARE_PREFIX)share/doc/iozone
	install -d $(IOZONE_IPK_DIR)$(OPTWARE_PREFIX)man1/
	install -m 644 $(IOZONE_BUILD_DIR)/docs/iozone.1 $(IOZONE_IPK_DIR)$(OPTWARE_PREFIX)man1
