...
#KERNEL-IMAGE_DESCRIPTION=Synology e500 (Marvell mv5281) kernel
KERNEL-MODULE_DESCRIPTION=Synology e500 (Marvell mv5281) kernel module
KERNEL-MODULES_DESCRIPTION=Synology e500 (Marvell mv5281) kernel modules
...
#KERNEL-MODULES_CONFFILES=$(OPTWARE_PREFIX)etc/kernel-modules.conf $(OPTWARE_PREFIX)etc/init.d/SXXkernel-modules
...
KERNEL-MODULES_SOURCE_DIR=$(SOURCE_DIR)/kernel-modules/syno-e500
...
KERNEL-MODULES-FLAGS = ARCH=ppc ROOTDIR=$(KERNEL-MODULES_BUILD_DIR) CROSS_COMPILE=$(TARGET_CROSS)
KERNEL-MODULES_CONFIG_METHOD=oldconfig

$(KERNEL-MODULES_BUILD_DIR)/.configured: \
$(DL_DIR)/$(SYNO-GPL_SOURCE) $(KERNEL-MODULES_PATCHES) \
$(KERNEL-MODULES_SOURCE_DIR)/defconfig make/syno-e500-kernel-modules.mk
#	$(MAKE) u-boot-mkimage
...
$(KERNEL-MODULES_IPK_DIR)/CONTROL/control:
	...
	for m in $(KERNEL-MODULES); do \
	  ...
	  rm -f $(KERNEL-MODULE_IPKS_DIR)/$$n/CONTROL/control; \
	    ...
	    for i in `grep "/$$m.ko:" $(KERNEL-MODULES_IPK_DIR)$(OPTWARE_PREFIX)lib/modules/$(KERNEL_VERSION)/modules.dep|cut -d ":" -f 2`; do \
...
#	( cd $(BUILD_DIR); $(IPKG_BUILD) $(KERNEL-IMAGE_IPK_DIR) )
	...
	mkdir -p $(KERNEL-MODULES_IPK_DIR)$(OPTWARE_PREFIX)lib/modules
	$(MAKE) -C $(KERNEL-MODULES_BUILD_DIR) $(KERNEL-MODULES-FLAGS) \
		INSTALL_MOD_PATH=$(KERNEL-MODULES_IPK_DIR)$(OPTWARE_PREFIX)modules_install
	for m in $(KERNEL-MODULES); do \
	  m=`basename $$m .ko`; \
	  n=`echo $$m | sed -e 's/_/-/g' | tr '[A-Z]' '[a-z]'`; \
	...
	rm -f $(KERNEL-MODULES_IPK_DIR)$(OPTWARE_PREFIX)lib/modules/$(KERNEL_VERSION)/build
	rm -f $(KERNEL-MODULES_IPK_DIR)$(OPTWARE_PREFIX)lib/modules/$(KERNEL_VERSION)/source
	rm -rf $(KERNEL-MODULES_IPK_DIR)$(OPTWARE_PREFIX)lib/modules/$(KERNEL_VERSION)/kernel
...
syno-e500-kernel-modules-ipk: $(KERNEL-MODULES_BUILD_DIR)/.ipkdone
