...
#        when upgrading.
XINETD_CONFFILES=$(OPTWARE_PREFIX)etc/xinetd.conf

ifeq ($(OPTWARE_TARGET),nslu2)
XINETD_CONFFILES+=$(OPTWARE_PREFIX)etc/xinetd.d/telnetd $(OPTWARE_PREFIX)etc/xinetd.d/ftp-sensor
endif
...
#
# $(STAGING_DIR)$(OPTWARE_PREFIX)lib/libxinetd.so.$(XINETD_VERSION): $(XINETD_BUILD_DIR)/.built


# xinetd-stage: $(STAGING_DIR)$(OPTWARE_PREFIX)lib/libxinetd.so.$(XINETD_VERSION)
...
#
# Binaries should be installed into $(XINETD_IPK_DIR)$(OPTWARE_PREFIX)sbin or $(XINETD_IPK_DIR)$(OPTWARE_PREFIX)bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(XINETD_IPK_DIR)$(OPTWARE_PREFIX){lib,include}
# Configuration files should be installed in $(XINETD_IPK_DIR)$(OPTWARE_PREFIX)etc/xinetd/...
# Documentation files should be installed in $(XINETD_IPK_DIR)$(OPTWARE_PREFIX)doc/xinetd/...
# Daemon startup scripts should be installed in $(XINETD_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/S??xinetd
#
...
$(XINETD_IPK): $(XINETD_BUILD_DIR)/.built
	...
	$(MAKE) -C $(XINETD_BUILD_DIR) DAEMONDIR=$(XINETD_IPK_DIR)$(OPTWARE_PREFIX)sbin \
		MANDIR=$(XINETD_IPK_DIR)$(OPTWARE_PREFIX)man install
	# Strip executables
	$(STRIP_COMMAND) $(XINETD_IPK_DIR)$(OPTWARE_PREFIX)sbin/xinetd $(XINETD_IPK_DIR)$(OPTWARE_PREFIX)sbin/itox
	# Install reload utility
	install -m 700 $(XINETD_SOURCE_DIR)/xinetd.reload  $(XINETD_IPK_DIR)$(OPTWARE_PREFIX)sbin
	# Install config file and create the xinetd.d catalog
	install -d $(XINETD_IPK_DIR)$(OPTWARE_PREFIX)etc/xinetd.d
	install -m 755 $(XINETD_SOURCE_DIR)/xinetd.conf $(XINETD_IPK_DIR)$(OPTWARE_PREFIX)etc
ifeq ($(OPTWARE_TARGET),nslu2)
	# Drop in the telnet and ftp-sensor config
	install -m 644 $(XINETD_SOURCE_DIR)/telnetd $(XINETD_IPK_DIR)$(OPTWARE_PREFIX)etc/xinetd.d
	install -m 644 $(XINETD_BUILD_DIR)/contrib/xinetd.d/ftp-sensor $(XINETD_IPK_DIR)$(OPTWARE_PREFIX)etc/xinetd.d
endif
	# Install daemon startup file
	install -d $(XINETD_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d
	install -m 755 $(XINETD_SOURCE_DIR)/rc.xinetd $(XINETD_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/S10xinetd
	sed -i -e '/^#!/aOPTWARE_TARGET=${OPTWARE_TARGET}' $(XINETD_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/S10xinetd
