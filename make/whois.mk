...
ifeq (uclibc, $(LIBC_STYLE))
	...
	sed -i -e 's|$$(BASEDIR)/usr/share|$$(BASEDIR)$(OPTWARE_PREFIX)share|' $(@D)/po/Makefile
	touch $@
...
#
# Binaries should be installed into $(WHOIS_IPK_DIR)$(OPTWARE_PREFIX)sbin or $(WHOIS_IPK_DIR)$(OPTWARE_PREFIX)bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(WHOIS_IPK_DIR)$(OPTWARE_PREFIX){lib,include}
# Configuration files should be installed in $(WHOIS_IPK_DIR)$(OPTWARE_PREFIX)etc/whois/...
# Documentation files should be installed in $(WHOIS_IPK_DIR)$(OPTWARE_PREFIX)doc/whois/...
# Daemon startup scripts should be installed in $(WHOIS_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/S??whois
#
# You may need to patch your application to make it use these locations.
#
$(WHOIS_IPK): $(WHOIS_BUILD_DIR)/.built
	rm -rf $(WHOIS_IPK_DIR) $(BUILD_DIR)/whois_*_$(TARGET_ARCH).ipk
	install -d $(WHOIS_IPK_DIR)$(OPTWARE_PREFIX)bin/
	install -d $(WHOIS_IPK_DIR)$(OPTWARE_PREFIX)share/man/man1
	$(MAKE) -C $(WHOIS_BUILD_DIR) BASEDIR=$(WHOIS_IPK_DIR) prefix=$(OPTWARE_PREFIX)install
	$(STRIP_COMMAND) $(WHOIS_IPK_DIR)$(OPTWARE_PREFIX)bin/*
#	install -d $(WHOIS_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d
