...
#PYTHON24_CONFFILES=$(OPTWARE_PREFIX)etc/python.conf $(OPTWARE_PREFIX)etc/init.d/SXXpython
...
$(PYTHON24_BUILD_DIR)/.staged: $(PYTHON24_BUILD_DIR)/.built
	...
	install $(@D)/buildpython/python $(STAGING_DIR)$(OPTWARE_PREFIX)bin/
	touch $@
...
#
# Binaries should be installed into $(PYTHON24_IPK_DIR)$(OPTWARE_PREFIX)sbin or $(PYTHON24_IPK_DIR)$(OPTWARE_PREFIX)bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(PYTHON24_IPK_DIR)$(OPTWARE_PREFIX){lib,include}
# Configuration files should be installed in $(PYTHON24_IPK_DIR)$(OPTWARE_PREFIX)etc/python/...
# Documentation files should be installed in $(PYTHON24_IPK_DIR)$(OPTWARE_PREFIX)doc/python/...
# Daemon startup scripts should be installed in $(PYTHON24_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/S??python
#
...
$(PYTHON24_IPK): $(PYTHON24_BUILD_DIR)/.built
	...
	$(STRIP_COMMAND) $(PYTHON24_IPK_DIR)$(OPTWARE_PREFIX)bin/python$(PYTHON24_VERSION_MAJOR)
	$(STRIP_COMMAND) $(PYTHON24_IPK_DIR)$(OPTWARE_PREFIX)lib/python$(PYTHON24_VERSION_MAJOR)/lib-dynload/*.so
	chmod 755 $(PYTHON24_IPK_DIR)$(OPTWARE_PREFIX)lib/libpython$(PYTHON24_VERSION_MAJOR).so.1.0
	$(STRIP_COMMAND) $(PYTHON24_IPK_DIR)$(OPTWARE_PREFIX)lib/libpython$(PYTHON24_VERSION_MAJOR).so.1.0
	chmod 555 $(PYTHON24_IPK_DIR)$(OPTWARE_PREFIX)lib/libpython$(PYTHON24_VERSION_MAJOR).so.1.0
	(cd $(PYTHON24_IPK_DIR)$(OPTWARE_PREFIX)bin; \
		mv idle idle$(PYTHON24_VERSION_MAJOR); \
		mv pydoc pydoc$(PYTHON24_VERSION_MAJOR); \
		mv smtpd.py smtpd$(PYTHON24_VERSION_MAJOR).py; \
	)
	rm $(PYTHON24_IPK_DIR)$(OPTWARE_PREFIX)bin/python
	install -d $(PYTHON24_IPK_DIR)$(OPTWARE_PREFIX)local/bin
	install -d $(PYTHON24_IPK_DIR)$(OPTWARE_PREFIX)local/lib/python$(PYTHON24_VERSION_MAJOR)/site-packages
	sed -i -e 's|$(TARGET_CROSS)|$(OPTWARE_PREFIX)bin/|g' \
	       ...
	       -e '/^RUNSHARED=/s|=.*|=|' \
	       $(PYTHON24_IPK_DIR)$(OPTWARE_PREFIX)lib/python2.4/config/Makefile
