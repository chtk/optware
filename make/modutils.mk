...
MODUTILS_CONFFILES=$(OPTWARE_PREFIX)etc/init.d/S01modutils
...
$(MODUTILS_BUILD_DIR)/.built: $(MODUTILS_BUILD_DIR)/.configured
	...
	$(MAKE) -C $(MODUTILS_BUILD_DIR) CFLAGS="$(STAGING_CPPFLAGS) $(MODUTILS_CPPFLAGS) -DMODDIR=\\\"$(OPTWARE_PREFIX)lib/modules\\\""
...
#
# Binaries should be installed into $(MODUTILS_IPK_DIR)$(OPTWARE_PREFIX)sbin or $(MODUTILS_IPK_DIR)$(OPTWARE_PREFIX)bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(MODUTILS_IPK_DIR)$(OPTWARE_PREFIX){lib,include}
# Configuration files should be installed in $(MODUTILS_IPK_DIR)$(OPTWARE_PREFIX)etc/modutils/...
# Documentation files should be installed in $(MODUTILS_IPK_DIR)$(OPTWARE_PREFIX)doc/modutils/...
# Daemon startup scripts should be installed in $(MODUTILS_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/S??modutils
#
...
$(MODUTILS_IPK): $(MODUTILS_BUILD_DIR)/.built
	...
	# Remove man pages
	rm -rf $(MODUTILS_IPK_DIR)$(OPTWARE_PREFIX)man
	# Strip binaries
	$(STRIP_COMMAND) $(MODUTILS_IPK_DIR)$(OPTWARE_PREFIX)sbin/depmod
	$(STRIP_COMMAND) $(MODUTILS_IPK_DIR)$(OPTWARE_PREFIX)sbin/genksyms
	$(STRIP_COMMAND) $(MODUTILS_IPK_DIR)$(OPTWARE_PREFIX)sbin/insmod
	$(STRIP_COMMAND) $(MODUTILS_IPK_DIR)$(OPTWARE_PREFIX)sbin/modinfo
	install -d $(MODUTILS_IPK_DIR)$(OPTWARE_PREFIX)etc/
	install -d $(MODUTILS_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d
	install -d $(MODUTILS_IPK_DIR)$(OPTWARE_PREFIX)lib/
	install -m 755 $(MODUTILS_SOURCE_DIR)/rc.modutils $(MODUTILS_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/S01modutils
