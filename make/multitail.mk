...
MULTITAIL_CONFFILES=$(OPTWARE_PREFIX)etc/multitail.conf
...
$(MULTITAIL_BUILD_DIR)/.built: $(MULTITAIL_BUILD_DIR)/.configured
	...
	$(MAKE) -C $(@D) -f makefile.cross-arm-linux \
		...
		CPPFLAGS="$(STAGING_CPPFLAGS) $(MULTITAIL_CPPFLAGS)" \
		LDFLAGS="$(STAGING_LDFLAGS) $(MULTITAIL_LDFLAGS)" \
		CONFIG_FILE=$(OPTWARE_PREFIX)etc/multitail.conf
	touch $@
...
#
# Binaries should be installed into $(MULTITAIL_IPK_DIR)$(OPTWARE_PREFIX)sbin or $(MULTITAIL_IPK_DIR)$(OPTWARE_PREFIX)bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(MULTITAIL_IPK_DIR)$(OPTWARE_PREFIX){lib,include}
# Configuration files should be installed in $(MULTITAIL_IPK_DIR)$(OPTWARE_PREFIX)etc/multitail/...
# Documentation files should be installed in $(MULTITAIL_IPK_DIR)$(OPTWARE_PREFIX)doc/multitail/...
# Daemon startup scripts should be installed in $(MULTITAIL_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/S??multitail
#
# You may need to patch your application to make it use these locations.
#
$(MULTITAIL_IPK): $(MULTITAIL_BUILD_DIR)/.built
	rm -rf $(MULTITAIL_IPK_DIR) $(BUILD_DIR)/multitail_*_$(TARGET_ARCH).ipk
	install -d $(MULTITAIL_IPK_DIR)$(OPTWARE_PREFIX)bin
	install -m 755 $(MULTITAIL_BUILD_DIR)/multitail $(MULTITAIL_IPK_DIR)$(OPTWARE_PREFIX)bin/
	$(STRIP_COMMAND) $(MULTITAIL_IPK_DIR)$(OPTWARE_PREFIX)bin/multitail
	install -d $(MULTITAIL_IPK_DIR)$(OPTWARE_PREFIX)etc
	install -m 644 $(MULTITAIL_BUILD_DIR)/multitail.conf $(MULTITAIL_IPK_DIR)$(OPTWARE_PREFIX)etc/
	install -d $(MULTITAIL_IPK_DIR)$(OPTWARE_PREFIX)man/man1
	install -m 644 $(MULTITAIL_BUILD_DIR)/multitail.1 $(MULTITAIL_IPK_DIR)$(OPTWARE_PREFIX)man/man1/
