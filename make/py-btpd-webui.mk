...
#PY-BTPD-WEBUI_CONFFILES=$(OPTWARE_PREFIX)etc/py-btpd-webui.conf $(OPTWARE_PREFIX)etc/init.d/SXXpy-btpd-webui
...
$(PY-BTPD-WEBUI_BUILD_DIR)/.configured: $(DL_DIR)/$(PY-BTPD-WEBUI_SOURCE) $(PY-BTPD-WEBUI_PATCHES) make/py-btpd-webui.mk
	...
	sed -i -e "s|^TWISTD=.*|TWISTD=$(OPTWARE_PREFIX)bin/twistd|" -e "s|^BTPDWEBUI=.*|BTPDWEBUI=$(OPTWARE_PREFIX)bin/btpd-webui-server|" $(@D)/scripts/btpd-webui
	(cd $(@D); \
	    ( \
		echo "[build_ext]"; \
...
#
# Binaries should be installed into $(PY-BTPD-WEBUI_IPK_DIR)$(OPTWARE_PREFIX)sbin or $(PY-BTPD-WEBUI_IPK_DIR)$(OPTWARE_PREFIX)bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(PY-BTPD-WEBUI_IPK_DIR)$(OPTWARE_PREFIX){lib,include}
# Configuration files should be installed in $(PY-BTPD-WEBUI_IPK_DIR)$(OPTWARE_PREFIX)etc/py-btpd-webui/...
# Documentation files should be installed in $(PY-BTPD-WEBUI_IPK_DIR)$(OPTWARE_PREFIX)doc/py-btpd-webui/...
# Daemon startup scripts should be installed in $(PY-BTPD-WEBUI_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/S??py-btpd-webui
#
...
$(PY-BTPD-WEBUI_IPK): $(PY-BTPD-WEBUI_BUILD_DIR)/.built
	...
	mkdir -p $(PY-BTPD-WEBUI_IPK_DIR)$(OPTWARE_PREFIX)doc/py-btpd-webui
	cp -f $(PY-BTPD-WEBUI_BUILD_DIR)/README $(PY-BTPD-WEBUI_IPK_DIR)$(OPTWARE_PREFIX)doc/py-btpd-webui/
