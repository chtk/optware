...
#PY-4SUITE_CONFFILES=$(OPTWARE_PREFIX)etc/py-4suite.conf $(OPTWARE_PREFIX)etc/init.d/SXXpy-4suite
...
#
# Binaries should be installed into $(PY-4SUITE_IPK_DIR)$(OPTWARE_PREFIX)sbin or $(PY-4SUITE_IPK_DIR)$(OPTWARE_PREFIX)bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(PY-4SUITE_IPK_DIR)$(OPTWARE_PREFIX){lib,include}
# Configuration files should be installed in $(PY-4SUITE_IPK_DIR)$(OPTWARE_PREFIX)etc/py-4suite/...
# Documentation files should be installed in $(PY-4SUITE_IPK_DIR)$(OPTWARE_PREFIX)doc/py-4suite/...
# Daemon startup scripts should be installed in $(PY-4SUITE_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/S??py-4suite
#
...
$(PY24-4SUITE_IPK): $(PY-4SUITE_BUILD_DIR)/.built
	...
	--root=$(PY24-4SUITE_IPK_DIR) --prefix=$(OPTWARE_PREFIX)--without-docs)
	$(STRIP_COMMAND) `find $(PY24-4SUITE_IPK_DIR)$(OPTWARE_PREFIX)lib/ -name '*.so'`
	sed -i -e '1s|#!/usr/bin/env python|#!$(OPTWARE_PREFIX)bin/python2.4|' $(PY24-4SUITE_IPK_DIR)$(OPTWARE_PREFIX)bin/*
...
$(PY25-4SUITE_IPK): $(PY-4SUITE_BUILD_DIR)/.built
	...
	--root=$(PY25-4SUITE_IPK_DIR) --prefix=$(OPTWARE_PREFIX)--without-docs)
	$(STRIP_COMMAND) `find $(PY25-4SUITE_IPK_DIR)$(OPTWARE_PREFIX)lib/ -name '*.so'`
	sed -i -e '1s|#!/usr/bin/env python|#!$(OPTWARE_PREFIX)bin/python2.5|' $(PY25-4SUITE_IPK_DIR)$(OPTWARE_PREFIX)bin/*
