...
#MLDONKEY_CONFFILES=$(OPTWARE_PREFIX)etc/mldonkey.conf $(OPTWARE_PREFIX)etc/init.d/SXXmldonkey
...
$(MLDONKEY_BUILD_DIR)/.configured: $(DL_DIR)/$(MLDONKEY_SOURCE) $(MLDONKEY_PATCHES) make/mldonkey.mk
	...
	sed -i -e 's|3\.11\.[0-9]|3\.11\.[0-9]*|' $(@D)/config/configure
	(cd $(@D); \
...
#
# Binaries should be installed into $(MLDONKEY_IPK_DIR)$(OPTWARE_PREFIX)sbin or $(MLDONKEY_IPK_DIR)$(OPTWARE_PREFIX)bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(MLDONKEY_IPK_DIR)$(OPTWARE_PREFIX){lib,include}
# Configuration files should be installed in $(MLDONKEY_IPK_DIR)$(OPTWARE_PREFIX)etc/mldonkey/...
# Documentation files should be installed in $(MLDONKEY_IPK_DIR)$(OPTWARE_PREFIX)doc/mldonkey/...
# Daemon startup scripts should be installed in $(MLDONKEY_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/S??mldonkey
#
...
#	$(MAKE) -C $(MLDONKEY_BUILD_DIR) DESTDIR=$(MLDONKEY_IPK_DIR) install
	install -d $(MLDONKEY_IPK_DIR)$(OPTWARE_PREFIX)bin
	for f in mlnet copysources get_range make_torrent mld_hash subconv; \
		do install $(MLDONKEY_BUILD_DIR)/$${f}.byte $(MLDONKEY_IPK_DIR)$(OPTWARE_PREFIX)bin/; done
	for l in mlslsk mldonkey mlgnut mldc mlbt; \
		do ln -s mlnet.byte $(MLDONKEY_IPK_DIR)$(OPTWARE_PREFIX)bin/$${l}.byte; done
