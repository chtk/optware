...
#PY-MERCURIAL_CONFFILES=$(OPTWARE_PREFIX)etc/py-mercurial.conf $(OPTWARE_PREFIX)etc/init.d/SXXpy-mercurial
...
#
# Binaries should be installed into $(PY-MERCURIAL_IPK_DIR)$(OPTWARE_PREFIX)sbin or $(PY-MERCURIAL_IPK_DIR)$(OPTWARE_PREFIX)bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(PY-MERCURIAL_IPK_DIR)$(OPTWARE_PREFIX){lib,include}
# Configuration files should be installed in $(PY-MERCURIAL_IPK_DIR)$(OPTWARE_PREFIX)etc/py-mercurial/...
# Documentation files should be installed in $(PY-MERCURIAL_IPK_DIR)$(OPTWARE_PREFIX)doc/py-mercurial/...
# Daemon startup scripts should be installed in $(PY-MERCURIAL_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/S??py-mercurial
#
...
$(PY25-MERCURIAL_IPK) $(PY26-MERCURIAL_IPK) $(PY27-MERCURIAL_IPK): $(PY-MERCURIAL_BUILD_DIR)/.built
	...
	(cd $(PY25-MERCURIAL_IPK_DIR)$(OPTWARE_PREFIX)lib/python2.5/site-packages; \
		find . -name '*.so' -exec chmod +w {} \; ; \
		find . -name '*.so' -exec $(STRIP_COMMAND) {} \; ; \
		find . -name '*.so' -exec chmod -w {} \; ; \
	)
	for f in $(PY25-MERCURIAL_IPK_DIR)$(OPTWARE_PREFIX)*bin/*; \
	...
	(cd $(PY26-MERCURIAL_IPK_DIR)$(OPTWARE_PREFIX)lib/python2.6/site-packages; \
		find . -name '*.so' -exec chmod +w {} \; ; \
		find . -name '*.so' -exec $(STRIP_COMMAND) {} \; ; \
		find . -name '*.so' -exec chmod -w {} \; ; \
	)
	...
	(cd $(PY27-MERCURIAL_IPK_DIR)$(OPTWARE_PREFIX)lib/python2.7/site-packages; \
		find . -name '*.so' -exec chmod +w {} \; ; \
		find . -name '*.so' -exec $(STRIP_COMMAND) {} \; ; \
		find . -name '*.so' -exec chmod -w {} \; ; \
	)
	for f in $(PY27-MERCURIAL_IPK_DIR)$(OPTWARE_PREFIX)*bin/*; \
