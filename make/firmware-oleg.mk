...
FIRMWARE_OLEG_CONFFILES=$(OPTWARE_PREFIX)etc/firmware-oleg.conf $(OPTWARE_PREFIX)etc/init.d/SXXfirmware-oleg
...
FIRMWARE_OLEG_TOOLCHAIN=$(FIRMWARE_OLEG_BUILD_DIR)/toolchain$(OPTWARE_PREFIX)brcm/hndtools-mipsel-uclibc
FIRMWARE_OLEG_TOOLPATH=$(FIRMWARE_OLEG_TOOLCHAIN)/bin:$(OPTWARE_PREFIX)brcm/hndtools-mipsel-linux/bin:${PATH}
...
ifeq ($(MAKE_VERSION), 3.81)
	...
	if ! test -d $(OPTWARE_PREFIX)brcm/hndtools-mipsel-uclibc/bin ; \
		then echo "Required wl500g toolchain missing!"; \
		echo "Use mv $(FIRMWARE_OLEG_BUILD_DIR)$(OPTWARE_PREFIX)brcm /opt"; \
		exit 1; \
	fi
...
#
# Binaries should be installed into $(FIRMWARE_OLEG_IPK_DIR)$(OPTWARE_PREFIX)sbin or $(FIRMWARE_OLEG_IPK_DIR)$(OPTWARE_PREFIX)bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(FIRMWARE_OLEG_IPK_DIR)$(OPTWARE_PREFIX){lib,include}
# Configuration files should be installed in $(FIRMWARE_OLEG_IPK_DIR)$(OPTWARE_PREFIX)etc/firmware-oleg/...
# Documentation files should be installed in $(FIRMWARE_OLEG_IPK_DIR)$(OPTWARE_PREFIX)doc/firmware-oleg/...
# Daemon startup scripts should be installed in $(FIRMWARE_OLEG_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/S??firmware-oleg
#
...
$(FIRMWARE_OLEG_IPK): $(FIRMWARE_OLEG_BUILD_DIR)/.built
	...
	install -d $(FIRMWARE_OLEG_IPK_DIR)$(OPTWARE_PREFIX)share/firmware
	install -m 644 $(FIRMWARE_OLEG_BUILD_DIR)/src/gateway/mipsel-uclibc/*.trx \
		$(FIRMWARE_OLEG_IPK_DIR)$(OPTWARE_PREFIX)share/firmware
#	$(MAKE) -C $(FIRMWARE_OLEG_BUILD_DIR)/src/gateway DESTDIR=$(FIRMWARE_OLEG_IPK_DIR)
#	install -d $(FIRMWARE_OLEG_IPK_DIR)$(OPTWARE_PREFIX)etc/
#	install -m 644 $(FIRMWARE_OLEG_SOURCE_DIR)/firmware-oleg.conf $(FIRMWARE_OLEG_IPK_DIR)$(OPTWARE_PREFIX)etc/firmware-oleg.conf
#	install -d $(FIRMWARE_OLEG_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d
#	install -m 755 $(FIRMWARE_OLEG_SOURCE_DIR)/rc.firmware-oleg $(FIRMWARE_OLEG_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/SXXfirmware-oleg
#	sed -i -e '/^#!/aOPTWARE_TARGET=${OPTWARE_TARGET}' $(FIRMWARE_OLEG_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/SXXfirmware-oleg
