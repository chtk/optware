...
DENYHOSTS_CONFFILES=$(OPTWARE_PREFIX)/etc/denyhosts.cfg $(OPTWARE_PREFIX)/etc/init.d/S01denyhosts
...
#
# Binaries should be installed into $(DENYHOSTS_IPK_DIR)$(OPTWARE_PREFIX)sbin or $(DENYHOSTS_IPK_DIR)$(OPTWARE_PREFIX)bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(DENYHOSTS_IPK_DIR)$(OPTWARE_PREFIX){lib,include}
# Configuration files should be installed in $(DENYHOSTS_IPK_DIR)$(OPTWARE_PREFIX)etc/denyhosts/...
# Documentation files should be installed in $(DENYHOSTS_IPK_DIR)$(OPTWARE_PREFIX)doc/denyhosts/...
# Daemon startup scripts should be installed in $(DENYHOSTS_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/S??denyhosts
#
...
$(DENYHOSTS_IPK): $(DENYHOSTS_BUILD_DIR)/.built
	...
	install -d $(DENYHOSTS_IPK_DIR)$(OPTWARE_PREFIX)/etc/
	install -m 644 $(DENYHOSTS_SOURCE_DIR)/denyhosts.cfg $(DENYHOSTS_IPK_DIR)$(OPTWARE_PREFIX)/etc/denyhosts.cfg
	install -d $(DENYHOSTS_IPK_DIR)$(OPTWARE_PREFIX)/etc/init.d
	install -m 755 $(DENYHOSTS_SOURCE_DIR)/rc.denyhosts $(DENYHOSTS_IPK_DIR)$(OPTWARE_PREFIX)/etc/init.d/S01denyhosts
	install -dv $(DENYHOSTS_IPK_DIR)$(OPTWARE_PREFIX)/share/denyhosts
	cp -r $(DENYHOSTS_BUILD_DIR)/* $(DENYHOSTS_IPK_DIR)$(OPTWARE_PREFIX)/share/denyhosts
	...
	sed -i -e "s,/opt/,$(OPTWARE_PREFIX)/,g" \
		$(subst $(OPTWARE_PREFIX),$(DENYHOSTS_IPK_DIR)$(OPTWARE_PREFIX), $(DENYHOSTS_CONFFILES)) \
		$(DENYHOSTS_IPK_DIR)/CONTROL/postinst
