...
$(LIBPCAP_BUILD_DIR)/.staged: $(LIBPCAP_BUILD_DIR)/.built
	rm -f $@
	rm -f $(STAGING_DIR)$(OPTWARE_PREFIX)share/man/man3/pcap_*.3pcap
	$(MAKE) -C $(@D) DESTDIR=$(STAGING_DIR) install
	rm -f $(STAGING_LIB_DIR)$(OPTWARE_PREFIX)lib/libpcap.a
	touch $@
...
#
# Binaries should be installed into $(LIBPCAP_IPK_DIR)$(OPTWARE_PREFIX)sbin or $(LIBPCAP_IPK_DIR)$(OPTWARE_PREFIX)bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(LIBPCAP_IPK_DIR)$(OPTWARE_PREFIX){lib,include}
# Configuration files should be installed in $(LIBPCAP_IPK_DIR)$(OPTWARE_PREFIX)etc/libpcap/...
# Documentation files should be installed in $(LIBPCAP_IPK_DIR)$(OPTWARE_PREFIX)doc/libpcap/...
# Daemon startup scripts should be installed in $(LIBPCAP_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/S??libpcap
#
...
$(LIBPCAP_IPK) $(LIBPCAP-DEV_IPK): $(LIBPCAP_BUILD_DIR)/.built
	...
	install -d $(LIBPCAP_IPK_DIR)$(OPTWARE_PREFIX)bin
	$(MAKE) -C $(LIBPCAP_BUILD_DIR) DESTDIR=$(LIBPCAP_IPK_DIR) install
	$(STRIP_COMMAND) $(LIBPCAP_IPK_DIR)$(OPTWARE_PREFIX)lib/libpcap.so.[0-9]*.[0-9]*.[0-9]*
	rm -f $(LIBPCAP_IPK_DIR)$(OPTWARE_PREFIX)lib/libpcap.a
	$(MAKE) $(LIBPCAP_IPK_DIR)/CONTROL/control
	$(MAKE) $(LIBPCAP-DEV_IPK_DIR)/CONTROL/control
	install -d $(LIBPCAP-DEV_IPK_DIR)$(OPTWARE_PREFIX)
	mv $(LIBPCAP_IPK_DIR)$(OPTWARE_PREFIX)bin $(LIBPCAP-DEV_IPK_DIR)$(OPTWARE_PREFIX)
	mv $(LIBPCAP_IPK_DIR)$(OPTWARE_PREFIX)include $(LIBPCAP-DEV_IPK_DIR)$(OPTWARE_PREFIX)
	mv $(LIBPCAP_IPK_DIR)$(OPTWARE_PREFIX)share $(LIBPCAP-DEV_IPK_DIR)$(OPTWARE_PREFIX)
