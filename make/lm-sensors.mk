...
LM_SENSORS_CONFFILES=$(OPTWARE_PREFIX)etc/sensors3.conf
...
#
# Binaries should be installed into $(LM_SENSORS_IPK_DIR)$(OPTWARE_PREFIX)sbin or $(LM_SENSORS_IPK_DIR)$(OPTWARE_PREFIX)bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(LM_SENSORS_IPK_DIR)$(OPTWARE_PREFIX){lib,include}
# Configuration files should be installed in $(LM_SENSORS_IPK_DIR)$(OPTWARE_PREFIX)etc/lm-sensors/...
# Documentation files should be installed in $(LM_SENSORS_IPK_DIR)$(OPTWARE_PREFIX)doc/lm-sensors/...
# Daemon startup scripts should be installed in $(LM_SENSORS_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/S??lm-sensors
#
...
$(LM_SENSORS_IPK): $(LM_SENSORS_BUILD_DIR)/.built
	...
	$(MAKE) -C $(LM_SENSORS_BUILD_DIR) install \
		...
		ETCDIR=$(OPTWARE_PREFIX)etc \
		;
	rm -f $(LM_SENSORS_IPK_DIR)$(OPTWARE_PREFIX)lib/libsensors.a
	$(STRIP_COMMAND) \
		$(LM_SENSORS_IPK_DIR)$(OPTWARE_PREFIX)bin/sensors \
		$(LM_SENSORS_IPK_DIR)$(OPTWARE_PREFIX)sbin/isa* \
		$(LM_SENSORS_IPK_DIR)$(OPTWARE_PREFIX)lib/libsensors.so
