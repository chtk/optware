...
#MAN_PAGES_CONFFILES=$(OPTWARE_PREFIX)etc/man-pages.conf $(OPTWARE_PREFIX)etc/init.d/SXXman-pages
...
$(MAN_PAGES_BUILD_DIR)/.built: $(MAN_PAGES_BUILD_DIR)/.configured
	rm -f $@
	$(MAKE) -C $(@D) MANDIR=$(OPTWARE_PREFIX)man gz
	touch $@
...
#
# Binaries should be installed into $(MAN_PAGES_IPK_DIR)$(OPTWARE_PREFIX)sbin or $(MAN_PAGES_IPK_DIR)$(OPTWARE_PREFIX)bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(MAN_PAGES_IPK_DIR)$(OPTWARE_PREFIX){lib,include}
# Configuration files should be installed in $(MAN_PAGES_IPK_DIR)$(OPTWARE_PREFIX)etc/man-pages/...
# Documentation files should be installed in $(MAN_PAGES_IPK_DIR)$(OPTWARE_PREFIX)doc/man-pages/...
# Daemon startup scripts should be installed in $(MAN_PAGES_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/S??man-pages
#
# You may need to patch your application to make it use these locations.
#
$(MAN_PAGES_IPK): $(MAN_PAGES_BUILD_DIR)/.built
	rm -rf $(MAN_PAGES_IPK_DIR) $(BUILD_DIR)/man-pages_*_$(TARGET_ARCH).ipk
	$(MAKE) -C $(MAN_PAGES_BUILD_DIR) MANDIR=$(MAN_PAGES_IPK_DIR)$(OPTWARE_PREFIX)man install
#	install -d $(MAN_PAGES_IPK_DIR)$(OPTWARE_PREFIX)etc/
#	install -m 755 $(MAN_PAGES_SOURCE_DIR)/man-pages.conf $(MAN_PAGES_IPK_DIR)$(OPTWARE_PREFIX)etc/man-pages.conf
#	install -d $(MAN_PAGES_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d
#	install -m 755 $(MAN_PAGES_SOURCE_DIR)/rc.man-pages $(MAN_PAGES_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/SXXman-pages
