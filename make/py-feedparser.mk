...
#PY-FEEDPARSER_CONFFILES=$(OPTWARE_PREFIX)etc/py-feedparser.conf $(OPTWARE_PREFIX)etc/init.d/SXXpy-feedparser
...
#	cat $(PY-FEEDPARSER_PATCHES) | patch -d $(BUILD_DIR)/$(PY-FEEDPARSER_DIR) -p1
	...
	(echo "[build_scripts]"; \
         echo "executable=$(OPTWARE_PREFIX)bin/python2.4") >> $(@D)/2.4/setup.cfg
	# 2.5
	rm -rf $(BUILD_DIR)/$(PY-FEEDPARSER_DIR)
	$(PY-FEEDPARSER_UNZIP) $(DL_DIR)/$(PY-FEEDPARSER_SOURCE) | tar -C $(BUILD_DIR) -xvf -
#	cat $(PY-FEEDPARSER_PATCHES) | patch -d $(BUILD_DIR)/$(PY-FEEDPARSER_DIR) -p1
	...
	(echo "[build_scripts]"; \
         echo "executable=$(OPTWARE_PREFIX)bin/python2.5") >> $(@D)/2.4/setup.cfg
	touch $@
...
#
# Binaries should be installed into $(PY-FEEDPARSER_IPK_DIR)$(OPTWARE_PREFIX)sbin or $(PY-FEEDPARSER_IPK_DIR)$(OPTWARE_PREFIX)bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(PY-FEEDPARSER_IPK_DIR)$(OPTWARE_PREFIX){lib,include}
# Configuration files should be installed in $(PY-FEEDPARSER_IPK_DIR)$(OPTWARE_PREFIX)etc/py-feedparser/...
# Documentation files should be installed in $(PY-FEEDPARSER_IPK_DIR)$(OPTWARE_PREFIX)doc/py-feedparser/...
# Daemon startup scripts should be installed in $(PY-FEEDPARSER_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/S??py-feedparser
#
...
#	for f in $(PY24-FEEDPARSER_IPK_DIR)$(OPTWARE_PREFIX)bin/*; \
...
#	cd $(PY25-FEEDPARSER_IPK_DIR)$(OPTWARE_PREFIX)share/feedparser
