...
#CATDOC_CONFFILES=$(OPTWARE_PREFIX)etc/catdoc.conf $(OPTWARE_PREFIX)etc/init.d/SXXcatdoc
...
#
# Binaries should be installed into $(CATDOC_IPK_DIR)$(OPTWARE_PREFIX)sbin or $(CATDOC_IPK_DIR)$(OPTWARE_PREFIX)bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(CATDOC_IPK_DIR)$(OPTWARE_PREFIX){lib,include}
# Configuration files should be installed in $(CATDOC_IPK_DIR)$(OPTWARE_PREFIX)etc/catdoc/...
# Documentation files should be installed in $(CATDOC_IPK_DIR)$(OPTWARE_PREFIX)doc/catdoc/...
# Daemon startup scripts should be installed in $(CATDOC_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/S??catdoc
#
# You may need to patch your application to make it use these locations.
#
$(CATDOC_IPK): $(CATDOC_BUILD_DIR)/.built
	rm -rf $(CATDOC_IPK_DIR) $(BUILD_DIR)/catdoc_*_$(TARGET_ARCH).ipk
	$(MAKE) -C $(CATDOC_BUILD_DIR) prefix=$(CATDOC_IPK_DIR)$(OPTWARE_PREFIX)install
	rm -f $(CATDOC_IPK_DIR)$(OPTWARE_PREFIX)bin/wordview
	$(STRIP_COMMAND) $(CATDOC_IPK_DIR)$(OPTWARE_PREFIX)bin/*
#	install -d $(CATDOC_IPK_DIR)$(OPTWARE_PREFIX)etc/
#	install -m 644 $(CATDOC_SOURCE_DIR)/catdoc.conf $(CATDOC_IPK_DIR)$(OPTWARE_PREFIX)etc/catdoc.conf
#	install -d $(CATDOC_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d
#	install -m 755 $(CATDOC_SOURCE_DIR)/rc.catdoc $(CATDOC_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/SXXcatdoc
#	sed -i -e '/^#!/aOPTWARE_TARGET=${OPTWARE_TARGET}' $(XINETD_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/SXXcatdoc
