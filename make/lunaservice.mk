...
#LUNASERVICE_CONFFILES=$(OPTWARE_PREFIX)etc/lunaservice.conf $(OPTWARE_PREFIX)etc/init.d/SXXlunaservice
...
$(LUNASERVICE_BUILD_DIR)/.staged: $(LUNASERVICE_BUILD_DIR)/.built
	rm -f $@
	mkdir -p $(STAGING_DIR)$(OPTWARE_PREFIX)include $(STAGING_DIR)$(OPTWARE_PREFIX)lib
	install -m 644 $(LUNASERVICE_BUILD_DIR)/lunaservice.h $(STAGING_DIR)$(OPTWARE_PREFIX)include/
	install -m 644 $(LUNASERVICE_BUILD_DIR)/lunaservice-errors.h $(STAGING_DIR)$(OPTWARE_PREFIX)include/
	install -m 644 $(LUNASERVICE_BUILD_DIR)/liblunaservice.so $(STAGING_DIR)$(OPTWARE_PREFIX)lib/
	touch $@
...
#
# Binaries should be installed into $(LUNASERVICE_IPK_DIR)$(OPTWARE_PREFIX)sbin or $(LUNASERVICE_IPK_DIR)$(OPTWARE_PREFIX)bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(LUNASERVICE_IPK_DIR)$(OPTWARE_PREFIX){lib,include}
# Configuration files should be installed in $(LUNASERVICE_IPK_DIR)$(OPTWARE_PREFIX)etc/lunaservice/...
# Documentation files should be installed in $(LUNASERVICE_IPK_DIR)$(OPTWARE_PREFIX)doc/lunaservice/...
# Daemon startup scripts should be installed in $(LUNASERVICE_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/S??lunaservice
#
...
$(LUNASERVICE_IPK): $(LUNASERVICE_BUILD_DIR)/.built
	...
	mkdir -p $(LUNASERVICE_IPK_DIR)$(OPTWARE_PREFIX)include $(LUNASERVICE_IPK_DIR)$(OPTWARE_PREFIX)lib
	install -m 644 $(LUNASERVICE_BUILD_DIR)/lunaservice.h $(LUNASERVICE_IPK_DIR)$(OPTWARE_PREFIX)include/
	install -m 644 $(LUNASERVICE_BUILD_DIR)/lunaservice-errors.h $(LUNASERVICE_IPK_DIR)$(OPTWARE_PREFIX)include/
	install -m 644 $(LUNASERVICE_BUILD_DIR)/liblunaservice.so $(LUNASERVICE_IPK_DIR)$(OPTWARE_PREFIX)lib/
