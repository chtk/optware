...
#PYTHON27_CONFFILES=$(OPTWARE_PREFIX)etc/python.conf $(OPTWARE_PREFIX)etc/init.d/SXXpython
...
#
# Binaries should be installed into $(PYTHON27_IPK_DIR)$(OPTWARE_PREFIX)sbin or $(PYTHON27_IPK_DIR)$(OPTWARE_PREFIX)bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(PYTHON27_IPK_DIR)$(OPTWARE_PREFIX){lib,include}
# Configuration files should be installed in $(PYTHON27_IPK_DIR)$(OPTWARE_PREFIX)etc/python/...
# Documentation files should be installed in $(PYTHON27_IPK_DIR)$(OPTWARE_PREFIX)doc/python/...
# Daemon startup scripts should be installed in $(PYTHON27_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/S??python
#
...
$(PYTHON27_IPK): $(PYTHON27_BUILD_DIR)/.built
	...
	$(STRIP_COMMAND) $(PYTHON27_IPK_DIR)$(OPTWARE_PREFIX)bin/python$(PYTHON27_VERSION_MAJOR)
	$(STRIP_COMMAND) $(PYTHON27_IPK_DIR)$(OPTWARE_PREFIX)lib/python$(PYTHON27_VERSION_MAJOR)/lib-dynload/*.so
	chmod 755 $(PYTHON27_IPK_DIR)$(OPTWARE_PREFIX)lib/libpython$(PYTHON27_VERSION_MAJOR).so.1.0
	$(STRIP_COMMAND) $(PYTHON27_IPK_DIR)$(OPTWARE_PREFIX)lib/libpython$(PYTHON27_VERSION_MAJOR).so.1.0
	chmod 555 $(PYTHON27_IPK_DIR)$(OPTWARE_PREFIX)lib/libpython$(PYTHON27_VERSION_MAJOR).so.1.0
	rm $(PYTHON27_IPK_DIR)$(OPTWARE_PREFIX)bin/python $(PYTHON27_IPK_DIR)$(OPTWARE_PREFIX)bin/python-config
#	cd $(PYTHON27_IPK_DIR)$(OPTWARE_PREFIX)bin; ln -s python$(PYTHON27_VERSION_MAJOR) python
	for f in bin/pydoc bin/idle bin/smtpd.py; \
	    do mv $(PYTHON27_IPK_DIR)$(OPTWARE_PREFIX)$$f $(PYTHON27_IPK_DIR)$(OPTWARE_PREFIX)`echo $$f | sed -e 's/\(\.\|$$\)/-2.7\1/'`; done
	install -d $(PYTHON27_IPK_DIR)$(OPTWARE_PREFIX)local/bin
	install -d $(PYTHON27_IPK_DIR)$(OPTWARE_PREFIX)local/lib/python$(PYTHON27_VERSION_MAJOR)/site-packages
	sed -i -e 's|$(TARGET_CROSS)|$(OPTWARE_PREFIX)bin/|g' \
	       ...
	       -e '/^RUNSHARED=/s|=.*|=|' \
	       $(PYTHON27_IPK_DIR)$(OPTWARE_PREFIX)lib/python2.7/config/Makefile
#ifeq ($(OPTWARE_WRITE_OUTSIDE_OPT_ALLOWED),true)
#	install -d $(PYTHON27_IPK_DIR)/usr/bin
#	ln -s $(OPTWARE_PREFIX)bin/python $(PYTHON27_IPK_DIR)/usr/bin/python
#endif
