...
#LIBGMP_CONFFILES=$(OPTWARE_PREFIX)etc/libgmp.conf $(OPTWARE_PREFIX)etc/init.d/SXXlibgmp
...
$(LIBGMP_HOST_BUILD_DIR)/.staged: host/.configured $(DL_DIR)/$(LIBGMP_SOURCE) make/libgmp.mk
	...
	(cd $(@D); \
	    CPPFLAGS="$(LIBGMP_M32)" \
	    ./configure \
		--prefix=$(OPTWARE_PREFIX)$(LIBGMP_HOST32) \
		--disable-nls \
		--disable-shared; \
...
#
# Binaries should be installed into $(LIBGMP_IPK_DIR)$(OPTWARE_PREFIX)sbin or $(LIBGMP_IPK_DIR)$(OPTWARE_PREFIX)bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(LIBGMP_IPK_DIR)$(OPTWARE_PREFIX){lib,include}
# Configuration files should be installed in $(LIBGMP_IPK_DIR)$(OPTWARE_PREFIX)etc/libgmp/...
# Documentation files should be installed in $(LIBGMP_IPK_DIR)$(OPTWARE_PREFIX)doc/libgmp/...
# Daemon startup scripts should be installed in $(LIBGMP_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/S??libgmp
#
...
$(LIBGMP_IPK): $(LIBGMP_BUILD_DIR)/.built
	...
	$(STRIP_COMMAND) $(LIBGMP_IPK_DIR)$(OPTWARE_PREFIX)lib/libgmp.so.[0-9].[0-9].[0-9]
#	install -d $(LIBGMP_IPK_DIR)$(OPTWARE_PREFIX)etc/
#	install -m 644 $(LIBGMP_SOURCE_DIR)/libgmp.conf $(LIBGMP_IPK_DIR)$(OPTWARE_PREFIX)etc/libgmp.conf
#	install -d $(LIBGMP_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d
#	install -m 755 $(LIBGMP_SOURCE_DIR)/rc.libgmp $(LIBGMP_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/SXXlibgmp
