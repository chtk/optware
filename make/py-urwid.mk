...
#PY-URWID_CONFFILES=$(OPTWARE_PREFIX)etc/py-urwid.conf $(OPTWARE_PREFIX)etc/init.d/SXXpy-urwid
...
PY26-URWID_IPK_DIR=$(BUILD_DIR)/py26-urwid-$(PY-URWID_VERSION)-ipk
PY26-URWID_IPK=$(BUILD_DIR)/py26-urwid_$(PY-URWID_VERSION)-$(PY-URWID_IPK_VERSION)_$(TARGET_ARCH).ipk
...
#
$(PY-URWID_BUILD_DIR)/.configured: $(DL_DIR)/$(PY-URWID_SOURCE) $(PY-URWID_PATCHES) make/py-urwid.mk
...
$(PY26-URWID_IPK_DIR)/CONTROL/control:
	...
	@echo "Source: $(PY-URWID_SITE)/$(PY-URWID_SOURCE)" >>$@
	@echo "Description: $(PY-URWID_DESCRIPTION)" >>$@
	@echo "Depends: py-urwid-common, $(PY26-URWID_DEPENDS)" >>$@
	@echo "Conflicts: $(PY-URWID_CONFLICTS)" >>$@

#
# This builds the IPK file.
#
# Binaries should be installed into $(PY-URWID_IPK_DIR)$(OPTWARE_PREFIX)sbin or $(PY-URWID_IPK_DIR)$(OPTWARE_PREFIX)bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(PY-URWID_IPK_DIR)$(OPTWARE_PREFIX){lib,include}
# Configuration files should be installed in $(PY-URWID_IPK_DIR)$(OPTWARE_PREFIX)etc/py-urwid/...
# Documentation files should be installed in $(PY-URWID_IPK_DIR)$(OPTWARE_PREFIX)doc/py-urwid/...
# Daemon startup scripts should be installed in $(PY-URWID_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/S??py-urwid
#
...
$(PY25-URWID_IPK): $(PY-URWID_BUILD_DIR)/.built
	...
	$(STRIP_COMMAND) `find $(PY25-URWID_IPK_DIR)$(OPTWARE_PREFIX)lib/python2.5/site-packages -name '*.so'`
	$(MAKE) $(PY25-URWID_IPK_DIR)/CONTROL/control
	cd $(BUILD_DIR); $(IPKG_BUILD) $(PY25-URWID_IPK_DIR)

$(PY26-URWID_IPK): $(PY-URWID_BUILD_DIR)/.built
	rm -rf $(PY26-URWID_IPK_DIR) $(BUILD_DIR)/py26-urwid_*_$(TARGET_ARCH).ipk
	(cd $(PY-URWID_BUILD_DIR)/2.6; \
	    PYTHONPATH=$(STAGING_LIB_DIR)/python2.6/site-packages \
	    $(HOST_STAGING_PREFIX)/bin/python2.6 setup.py install \
	    --root=$(PY26-URWID_IPK_DIR) --prefix=/opt; \
	)
	$(STRIP_COMMAND) `find $(PY26-URWID_IPK_DIR)$(OPTWARE_PREFIX)lib/python2.6/site-packages -name '*.so'`
	$(MAKE) $(PY26-URWID_IPK_DIR)/CONTROL/control
	cd $(BUILD_DIR); $(IPKG_BUILD) $(PY26-URWID_IPK_DIR)
#
	rm -rf $(PY-URWID-COMMON_IPK_DIR) $(BUILD_DIR)/py-urwid-common_*_$(TARGET_ARCH).ipk
	install -d $(PY-URWID-COMMON_IPK_DIR)$(OPTWARE_PREFIX)share/doc/py-urwid
	echo "http://excess.org/urwid/" > $(PY-URWID-COMMON_IPK_DIR)$(OPTWARE_PREFIX)share/doc/py-urwid/url.txt
	install -m 644 $(PY-URWID_BUILD_DIR)/2.6/*.html $(PY-URWID-COMMON_IPK_DIR)$(OPTWARE_PREFIX)share/doc/py-urwid
	install -d $(PY-URWID-COMMON_IPK_DIR)$(OPTWARE_PREFIX)share/doc/py-urwid/examples
	install -m 644 $(PY-URWID_BUILD_DIR)/2.6/*.py $(PY-URWID-COMMON_IPK_DIR)$(OPTWARE_PREFIX)share/doc/py-urwid/examples
	rm -f $(PY-URWID-COMMON_IPK_DIR)$(OPTWARE_PREFIX)share/doc/py-urwid/examples/setup.py
...
#
py-urwid-ipk: $(PY25-URWID_IPK) $(PY26-URWID_IPK)
...
py-urwid-dirclean:
	...
	rm -rf $(PY26-URWID_IPK_DIR) $(PY26-URWID_IPK)

#
# Some sanity check for the package.
#
py-urwid-check: $(PY25-URWID_IPK) $(PY26-URWID_IPK)
