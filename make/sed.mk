...
#SED_CONFFILES=$(OPTWARE_PREFIX)etc/sed.conf $(OPTWARE_PREFIX)etc/init.d/SXXsed
...
#
# Binaries should be installed into $(SED_IPK_DIR)$(OPTWARE_PREFIX)sbin or $(SED_IPK_DIR)$(OPTWARE_PREFIX)bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(SED_IPK_DIR)$(OPTWARE_PREFIX){lib,include}
# Configuration files should be installed in $(SED_IPK_DIR)$(OPTWARE_PREFIX)etc/sed/...
# Documentation files should be installed in $(SED_IPK_DIR)$(OPTWARE_PREFIX)doc/sed/...
# Daemon startup scripts should be installed in $(SED_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/S??sed
#
...
$(SED_IPK): $(SED_BUILD_DIR)/.built
	...
	mv $(SED_IPK_DIR)$(OPTWARE_PREFIX)bin/sed $(SED_IPK_DIR)$(OPTWARE_PREFIX)bin/gnu-sed
	$(MAKE) $(SED_IPK_DIR)/CONTROL/control
	(echo "#!/bin/sh"; \
	 echo "update-alternatives --install $(OPTWARE_PREFIX)bin/sed sed $(OPTWARE_PREFIX)bin/gnu-sed 80"; \
	) > $(SED_IPK_DIR)/CONTROL/postinst
	(echo "#!/bin/sh"; \
	 echo "update-alternatives --remove sed $(OPTWARE_PREFIX)bin/gnu-sed"; \
