...
#EMACS_CONFFILES=$(OPTWARE_PREFIX)etc/emacs.conf $(OPTWARE_PREFIX)etc/init.d/SXXemacs
...
$(EMACS_BUILD_DIR)/.configured: $(DL_DIR)/$(EMACS_SOURCE) $(EMACS_PATCHES)
	...
	sed -i -e 's%LIBES =%LIBES = -Wl,-rpath-link=$(STAGING_LIB_DIR) -Wl,-rpath=$(OPTWARE_PREFIX)lib%' $(EMACS_BUILD_DIR)/src/Makefile
...
$(EMACS_IPK): $(EMACS_BUILD_DIR)/.built
	...
	$(MAKE) -C $(EMACS_BUILD_DIR) prefix=$(EMACS_IPK_DIR)$(OPTWARE_PREFIX)install TARGET_LIBDIR=$(TARGET_LIBDIR)
	rm -f $(EMACS_IPK_DIR)$(OPTWARE_PREFIX)bin/emacs
	for F in \
		$(EMACS_IPK_DIR)$(OPTWARE_PREFIX)bin/* \
		$(EMACS_IPK_DIR)$(OPTWARE_PREFIX)libexec/emacs/$(EMACS_VERSION)/*/* ; \
		do $(STRIP_COMMAND) $$F || : ; \
	done
	ln -s $(OPTWARE_PREFIX)bin/emacs-$(EMACS_VERSION) $(EMACS_IPK_DIR)$(OPTWARE_PREFIX)bin/emacs
	...
	install -d $(EMACS_LISP_IPK_DIR)$(OPTWARE_PREFIX)share/emacs/$(EMACS_VERSION)
	mv $(EMACS_IPK_DIR)$(OPTWARE_PREFIX)share/emacs/$(EMACS_VERSION)/lisp $(EMACS_LISP_IPK_DIR)$(OPTWARE_PREFIX)share/emacs/$(EMACS_VERSION)
