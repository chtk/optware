...
# ERL-ESCRIPT_CONFFILES=$(OPTWARE_PREFIX)etc/escript.conf $(OPTWARE_PREFIX)etc/escript-cert.pem $(OPTWARE_PREFIX)etc/escript-key.pem
...
$(ERL-ESCRIPT_BUILD_DIR)/.configured: $(DL_DIR)/$(ERL-ESCRIPT_SOURCE) $(ERL-ESCRIPT_PATCHES) make/erl-escript.mk
	...
	sed -i -e 's|$$CWD|$(OPTWARE_PREFIX)lib/erlang/lib/$(ERL-ESCRIPT_DIR)|' $(ERL-ESCRIPT_BUILD_DIR)/mk_escript.sh
...
#
# Binaries should be installed into $(ERL-ESCRIPT_IPK_DIR)$(OPTWARE_PREFIX)sbin or $(ERL-ESCRIPT_IPK_DIR)$(OPTWARE_PREFIX)bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(ERL-ESCRIPT_IPK_DIR)$(OPTWARE_PREFIX){lib,include}
# Configuration files should be installed in $(ERL-ESCRIPT_IPK_DIR)$(OPTWARE_PREFIX)etc/erl-escript/...
# Documentation files should be installed in $(ERL-ESCRIPT_IPK_DIR)$(OPTWARE_PREFIX)doc/erl-escript/...
# Daemon startup scripts should be installed in $(ERL-ESCRIPT_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/S??erl-escript
#
...
#	$(MAKE) -C $(ERL-ESCRIPT_BUILD_DIR) DESTDIR=$(ERL-ESCRIPT_IPK_DIR) install
	install -d $(ERL-ESCRIPT_IPK_DIR)$(OPTWARE_PREFIX)lib/erlang/lib/$(ERL-ESCRIPT_DIR)
	(cd $(ERL-ESCRIPT_BUILD_DIR); \
	install -m 755 escript mk_escript.sh factorial fibi fibc \
		$(ERL-ESCRIPT_IPK_DIR)$(OPTWARE_PREFIX)lib/erlang/lib/$(ERL-ESCRIPT_DIR); \
	install -m 644 escript.{erl,beam} \
		Makefile history escript.html \
		$(ERL-ESCRIPT_IPK_DIR)$(OPTWARE_PREFIX)lib/erlang/lib/$(ERL-ESCRIPT_DIR); )
	install -d $(ERL-ESCRIPT_IPK_DIR)$(OPTWARE_PREFIX)bin
	(cd $(ERL-ESCRIPT_IPK_DIR)$(OPTWARE_PREFIX)bin; \
		ln -s $(OPTWARE_PREFIX)lib/erlang/lib/$(ERL-ESCRIPT_DIR)/escript .; )
