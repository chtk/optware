...
#TORSOCKS_CONFFILES=$(OPTWARE_PREFIX)etc/torsocks.conf
...
#	$(MAKE) <bar>-stage <baz>-stage
	...
	(cd $(@D); \
		...
		--with-conf=$(OPTWARE_PREFIX)etc/torsocks.conf \
		--disable-nls \
	)
...
#
# Binaries should be installed into $(TORSOCKS_IPK_DIR)$(OPTWARE_PREFIX)sbin or $(TORSOCKS_IPK_DIR)$(OPTWARE_PREFIX)bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(TORSOCKS_IPK_DIR)$(OPTWARE_PREFIX){lib,include}
# Configuration files should be installed in $(TORSOCKS_IPK_DIR)$(OPTWARE_PREFIX)etc/torsocks/...
# Documentation files should be installed in $(TORSOCKS_IPK_DIR)$(OPTWARE_PREFIX)doc/torsocks/...
# Daemon startup scripts should be installed in $(TORSOCKS_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/S??torsocks
#
...
$(TORSOCKS_IPK): $(TORSOCKS_BUILD_DIR)/.built
	...
	sed -i -e 's:/usr/:$(OPTWARE_PREFIX):g' $(TORSOCKS_IPK_DIR)$(OPTWARE_PREFIX)bin/torsocks
	rm -f $(TORSOCKS_IPK_DIR)$(OPTWARE_PREFIX)lib/torsocks/libtorsocks*.a
	$(STRIP_COMMAND) $(TORSOCKS_IPK_DIR)$(OPTWARE_PREFIX)lib/torsocks/libtorsocks.so.[0-9].[0-9].[0-9]
#	install -d $(TORSOCKS_IPK_DIR)$(OPTWARE_PREFIX)etc/
#	mv $(TORSOCKS_IPK_DIR)/lib $(TORSOCKS_IPK_DIR)$(OPTWARE_PREFIX)
#	$(STRIP_COMMAND) $(TORSOCKS_IPK_DIR)$(OPTWARE_PREFIX)lib/libtorsocks.so.1.8
	#install -m 644 $(TORSOCKS_SOURCE_DIR)/torsocks.conf $(TORSOCKS_IPK_DIR)$(OPTWARE_PREFIX)etc/torsocks.conf
