...
ifneq ($(QEMU_LIBC_I386_CROSSTOOL_VERSION), $(CROSSTOOL-NATIVE_VERSION))
...
$(QEMU_LIBC_I386_IPK_DIR)/CONTROL/postinst:
	...
	@rm -f $@
	@echo "#!/bin/sh" >>$@
	@echo "mkdir -p $(OPTWARE_PREFIX)$(QEMU_LIBC_I386_TARGET)/etc" >>$@
	@echo "test -e $(OPTWARE_PREFIX)$(QEMU_LIBC_I386_TARGET)/etc/ld.so.cache || $(OPTWARE_PREFIX)bin/qemu-i386 $(OPTWARE_PREFIX)$(QEMU_LIBC_I386_TARGET)/sbin/ldconfig -C $(OPTWARE_PREFIX)$(QEMU_LIBC_I386_TARGET)/etc/ld.so.cache" >>$@
...
$(QEMU_LIBC_I386_IPK): $(QEMU_LIBC_I386_BUILD_DIR)/.built
	...
	mkdir -p $(QEMU_LIBC_I386_IPK_DIR)$(OPTWARE_PREFIX)$(QEMU_LIBC_I386_TARGET)/lib
	cp -a $(QEMU_LIBC_I386_PREFIX)/$(QEMU_LIBC_I386_TARGET)/lib/*.so.* \
		$(QEMU_LIBC_I386_PREFIX)/$(QEMU_LIBC_I386_TARGET)/lib/*.so \
		$(QEMU_LIBC_I386_IPK_DIR)$(OPTWARE_PREFIX)$(QEMU_LIBC_I386_TARGET)/lib
	rm -rf $(QEMU_LIBC_I386_IPK_DIR)$(OPTWARE_PREFIX)$(QEMU_LIBC_I386_TARGET)/lib/*.dir
	( cd $(QEMU_LIBC_I386_IPK_DIR)$(OPTWARE_PREFIX)$(QEMU_LIBC_I386_TARGET)/lib ; \
		for F in *.so *.so.* ; \
		do if test -x $$F ; then \
	...
	mkdir -p $(QEMU_LIBC_I386_IPK_DIR)$(OPTWARE_PREFIX)$(QEMU_LIBC_I386_TARGET)/sbin
	$(QEMU_LIBC_I386_PREFIX)/bin/$(QEMU_LIBC_I386_TARGET)-strip \
		$(QEMU_LIBC_I386_PREFIX)/$(QEMU_LIBC_I386_TARGET)/sbin/ldconfig \
		-o $(QEMU_LIBC_I386_IPK_DIR)$(OPTWARE_PREFIX)$(QEMU_LIBC_I386_TARGET)/sbin/ldconfig
	mkdir -p $(QEMU_LIBC_I386_IPK_DIR)$(OPTWARE_PREFIX)lib/gnemul
	ln -s ../../$(QEMU_LIBC_I386_TARGET) \
		$(QEMU_LIBC_I386_IPK_DIR)$(OPTWARE_PREFIX)lib/gnemul/qemu-i386
