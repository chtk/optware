...
#PY-SCGI_CONFFILES=$(OPTWARE_PREFIX)etc/py-scgi.conf $(OPTWARE_PREFIX)etc/init.d/SXXpy-scgi
...
PY26-SCGI_IPK_DIR=$(BUILD_DIR)/py26-scgi-$(PY-SCGI_VERSION)-ipk
PY26-SCGI_IPK=$(BUILD_DIR)/py26-scgi_$(PY-SCGI_VERSION)-$(PY-SCGI_IPK_VERSION)_$(TARGET_ARCH).ipk
...
$(PY26-SCGI_IPK_DIR)/CONTROL/control:
	...
	@echo "Source: $(PY-SCGI_SITE)/$(PY-SCGI_SOURCE)" >>$@
	@echo "Description: $(PY-SCGI_DESCRIPTION)" >>$@
	@echo "Depends: $(PY26-SCGI_DEPENDS)" >>$@
	@echo "Conflicts: $(PY-SCGI_CONFLICTS)" >>$@

#
# This builds the IPK file.
#
# Binaries should be installed into $(PY-SCGI_IPK_DIR)$(OPTWARE_PREFIX)sbin or $(PY-SCGI_IPK_DIR)$(OPTWARE_PREFIX)bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(PY-SCGI_IPK_DIR)$(OPTWARE_PREFIX){lib,include}
# Configuration files should be installed in $(PY-SCGI_IPK_DIR)$(OPTWARE_PREFIX)etc/py-scgi/...
# Documentation files should be installed in $(PY-SCGI_IPK_DIR)$(OPTWARE_PREFIX)doc/py-scgi/...
# Daemon startup scripts should be installed in $(PY-SCGI_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/S??py-scgi
#
...
$(PY25-SCGI_IPK): $(PY-SCGI_BUILD_DIR)/.built
	...
	$(STRIP_COMMAND) $(PY25-SCGI_IPK_DIR)$(OPTWARE_PREFIX)lib/python2.5/site-packages/scgi/*.so
	$(MAKE) $(PY25-SCGI_IPK_DIR)/CONTROL/control
	cd $(BUILD_DIR); $(IPKG_BUILD) $(PY25-SCGI_IPK_DIR)

$(PY26-SCGI_IPK): $(PY-SCGI_BUILD_DIR)/.built
	rm -rf $(PY26-SCGI_IPK_DIR) $(BUILD_DIR)/py26-scgi_*_$(TARGET_ARCH).ipk
	(cd $(PY-SCGI_BUILD_DIR)/2.6; \
	 CC='$(TARGET_CC)' LDSHARED='$(TARGET_CC) -shared' \
	    $(HOST_STAGING_PREFIX)/bin/python2.6 setup.py install \
	    --root=$(PY26-SCGI_IPK_DIR) --prefix=/opt; \
	)
	$(STRIP_COMMAND) $(PY26-SCGI_IPK_DIR)$(OPTWARE_PREFIX)lib/python2.6/site-packages/scgi/*.so
	$(MAKE) $(PY26-SCGI_IPK_DIR)/CONTROL/control
	cd $(BUILD_DIR); $(IPKG_BUILD) $(PY26-SCGI_IPK_DIR)

#
# This is called from the top level makefile to create the IPK file.
#
py-scgi-ipk: $(PY25-SCGI_IPK) $(PY26-SCGI_IPK)
...
py-scgi-dirclean:
	...
	rm -rf $(PY26-SCGI_IPK_DIR) $(PY26-SCGI_IPK)

#
# Some sanity check for the package.
#
py-scgi-check: $(PY25-SCGI_IPK) $(PY26-SCGI_IPK)
