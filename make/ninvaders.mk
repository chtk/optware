...
NINVADERS_CONFFILES=$(OPTWARE_PREFIX)etc/ninvaders.conf $(OPTWARE_PREFIX)etc/init.d/SXXninvaders
...
#
NINVADERS_CPPFLAGS=-I$(STAGING_INCLUDE_DIR)/$(NCURSES_FOR_OPTWARE_TARGET)
NINVADERS_LDFLAGS=
...
#
# Binaries should be installed into $(NINVADERS_IPK_DIR)$(OPTWARE_PREFIX)sbin or $(NINVADERS_IPK_DIR)$(OPTWARE_PREFIX)bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(NINVADERS_IPK_DIR)$(OPTWARE_PREFIX){lib,include}
# Configuration files should be installed in $(NINVADERS_IPK_DIR)$(OPTWARE_PREFIX)etc/ninvaders/...
# Documentation files should be installed in $(NINVADERS_IPK_DIR)$(OPTWARE_PREFIX)doc/ninvaders/...
# Daemon startup scripts should be installed in $(NINVADERS_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/S??ninvaders
#
# You may need to patch your application to make it use these locations.
#
$(NINVADERS_IPK): $(NINVADERS_BUILD_DIR)/.built
	rm -rf $(NINVADERS_IPK_DIR) $(BUILD_DIR)/ninvaders_*_$(TARGET_ARCH).ipk
	install -d $(NINVADERS_IPK_DIR)$(OPTWARE_PREFIX)bin/
	install -m 755 $(NINVADERS_BUILD_DIR)/nInvaders $(NINVADERS_IPK_DIR)$(OPTWARE_PREFIX)bin/
	$(STRIP_COMMAND) $(NINVADERS_IPK_DIR)$(OPTWARE_PREFIX)bin/nInvaders
