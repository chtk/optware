...
#DAVTOOLS_CONFFILES=$(OPTWARE_PREFIX)etc/davtools.conf $(OPTWARE_PREFIX)etc/init.d/SXXdavtools
...
#
# Binaries should be installed into $(DAVTOOLS_IPK_DIR)$(OPTWARE_PREFIX)sbin or $(DAVTOOLS_IPK_DIR)$(OPTWARE_PREFIX)bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(DAVTOOLS_IPK_DIR)$(OPTWARE_PREFIX){lib,include}
# Configuration files should be installed in $(DAVTOOLS_IPK_DIR)$(OPTWARE_PREFIX)etc/davtools/...
# Documentation files should be installed in $(DAVTOOLS_IPK_DIR)$(OPTWARE_PREFIX)doc/davtools/...
# Daemon startup scripts should be installed in $(DAVTOOLS_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/S??davtools
#
...
#	$(MAKE) -C $(DAVTOOLS_BUILD_DIR) DESTDIR=$(DAVTOOLS_IPK_DIR) install-strip
	install -d $(DAVTOOLS_IPK_DIR)$(OPTWARE_PREFIX)bin
	install -m 755 $(DAVTOOLS_BUILD_DIR)/src/cdavl/cdavl $(DAVTOOLS_IPK_DIR)$(OPTWARE_PREFIX)bin
	$(STRIP_COMMAND) $(DAVTOOLS_IPK_DIR)$(OPTWARE_PREFIX)bin/cdavl
	install -d $(DAVTOOLS_IPK_DIR)$(OPTWARE_PREFIX)man/man8
	install -m 644 $(DAVTOOLS_BUILD_DIR)/doc/cdavl.8 $(DAVTOOLS_IPK_DIR)$(OPTWARE_PREFIX)man/man8/
#	install -m 755 $(DAVTOOLS_SOURCE_DIR)/rc.davtools $(DAVTOOLS_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/SXXdavtools
#	sed -i -e '/^#!/aOPTWARE_TARGET=${OPTWARE_TARGET}' $(DAVTOOLS_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/SXXdavtools
