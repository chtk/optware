...
#SHARUTILS_CONFFILES=$(OPTWARE_PREFIX)etc/sharutils.conf $(OPTWARE_PREFIX)etc/init.d/SXXsharutils
...
#
# Binaries should be installed into $(SHARUTILS_IPK_DIR)$(OPTWARE_PREFIX)sbin or $(SHARUTILS_IPK_DIR)$(OPTWARE_PREFIX)bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(SHARUTILS_IPK_DIR)$(OPTWARE_PREFIX){lib,include}
# Configuration files should be installed in $(SHARUTILS_IPK_DIR)$(OPTWARE_PREFIX)etc/sharutils/...
# Documentation files should be installed in $(SHARUTILS_IPK_DIR)$(OPTWARE_PREFIX)doc/sharutils/...
# Daemon startup scripts should be installed in $(SHARUTILS_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/S??sharutils
#
...
$(SHARUTILS_IPK): $(SHARUTILS_BUILD_DIR)/.built
	...
	mv $(SHARUTILS_IPK_DIR)$(OPTWARE_PREFIX)bin/uudecode $(SHARUTILS_IPK_DIR)$(OPTWARE_PREFIX)bin/sharutils-uudecode
	mv $(SHARUTILS_IPK_DIR)$(OPTWARE_PREFIX)bin/uuencode $(SHARUTILS_IPK_DIR)$(OPTWARE_PREFIX)bin/sharutils-uuencode
	$(MAKE) $(SHARUTILS_IPK_DIR)/CONTROL/control
	(echo "#!/bin/sh"; \
	 echo "update-alternatives --install $(OPTWARE_PREFIX)bin/uudecode uudecode $(OPTWARE_PREFIX)bin/sharutils-uudecode 70"; \
	 echo "update-alternatives --install $(OPTWARE_PREFIX)bin/uuencode uuencode $(OPTWARE_PREFIX)bin/sharutils-uuencode 70"; \
	) > $(SHARUTILS_IPK_DIR)/CONTROL/postinst
	(echo "#!/bin/sh"; \
	 echo "update-alternatives --remove uudecode $(OPTWARE_PREFIX)bin/sharutils-uudecode"; \
	 echo "update-alternatives --remove uuencode $(OPTWARE_PREFIX)bin/sharutils-uuencode"; \
