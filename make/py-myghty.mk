...
#
PY-MYGHTY_SITE=http://pypi.python.org/packages/source/M/Myghty
PY-MYGHTY_VERSION=1.2
...
PY26-MYGHTY_DEPENDS=py25-paste, py25-pastedeploy, py25-pastescript, py25-routes
PY-MYGHTY_CONFLICTS=
...
#PY-MYGHTY_CONFFILES=$(OPTWARE_PREFIX)etc/py-myghty.conf $(OPTWARE_PREFIX)etc/init.d/SXXpy-myghty
...
PY26-MYGHTY_IPK_DIR=$(BUILD_DIR)/py26-myghty-$(PY-MYGHTY_VERSION)-ipk
PY26-MYGHTY_IPK=$(BUILD_DIR)/py26-myghty_$(PY-MYGHTY_VERSION)-$(PY-MYGHTY_IPK_VERSION)_$(TARGET_ARCH).ipk
...
#
$(PY-MYGHTY_BUILD_DIR)/.configured: $(DL_DIR)/$(PY-MYGHTY_SOURCE) $(PY-MYGHTY_PATCHES) make/py-myghty.mk
...
#	cat $(PY-MYGHTY_PATCHES) | patch -d $(BUILD_DIR)/$(PY-MYGHTY_DIR) -p1
	mv $(BUILD_DIR)/$(PY-MYGHTY_DIR) $(@D)/2.5
	(cd $(@D)/2.5; \
	    (echo "[build_scripts]"; \
	...
	$(PY-MYGHTY_UNZIP) $(DL_DIR)/$(PY-MYGHTY_SOURCE) | tar -C $(BUILD_DIR) -xvf -
#	cat $(PY-MYGHTY_PATCHES) | patch -d $(BUILD_DIR)/$(PY-MYGHTY_DIR) -p1
	mv $(BUILD_DIR)/$(PY-MYGHTY_DIR) $(@D)/2.6
	(cd $(@D)/2.6; \
	    (echo "[build_scripts]"; \
...
$(PY26-MYGHTY_IPK_DIR)/CONTROL/control:
	@install -d $(@D)
	@rm -f $@
	@echo "Package: py26-myghty" >>$@
	@echo "Architecture: $(TARGET_ARCH)" >>$@
	@echo "Priority: $(PY-MYGHTY_PRIORITY)" >>$@
	@echo "Section: $(PY-MYGHTY_SECTION)" >>$@
	@echo "Version: $(PY-MYGHTY_VERSION)-$(PY-MYGHTY_IPK_VERSION)" >>$@
	...
	@echo "Source: $(PY-MYGHTY_SITE)/$(PY-MYGHTY_SOURCE)" >>$@
	@echo "Description: $(PY-MYGHTY_DESCRIPTION)" >>$@
	@echo "Depends: $(PY26-MYGHTY_DEPENDS)" >>$@
	@echo "Conflicts: $(PY-MYGHTY_CONFLICTS)" >>$@

#
# This builds the IPK file.
#
# Binaries should be installed into $(PY-MYGHTY_IPK_DIR)$(OPTWARE_PREFIX)sbin or $(PY-MYGHTY_IPK_DIR)$(OPTWARE_PREFIX)bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(PY-MYGHTY_IPK_DIR)$(OPTWARE_PREFIX){lib,include}
# Configuration files should be installed in $(PY-MYGHTY_IPK_DIR)$(OPTWARE_PREFIX)etc/py-myghty/...
# Documentation files should be installed in $(PY-MYGHTY_IPK_DIR)$(OPTWARE_PREFIX)doc/py-myghty/...
# Daemon startup scripts should be installed in $(PY-MYGHTY_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/S??py-myghty
#
...
$(PY26-MYGHTY_IPK): $(PY-MYGHTY_BUILD_DIR)/.built
	rm -rf $(PY26-MYGHTY_IPK_DIR) $(BUILD_DIR)/py26-myghty_*_$(TARGET_ARCH).ipk
	(cd $(PY-MYGHTY_BUILD_DIR)/2.6; \
	    PYTHONPATH=$(STAGING_LIB_DIR)/python2.6/site-packages \
	    $(HOST_STAGING_PREFIX)/bin/python2.6 setup.py install \
	    --root=$(PY26-MYGHTY_IPK_DIR) --prefix=/opt)
	$(MAKE) $(PY26-MYGHTY_IPK_DIR)/CONTROL/control
#	echo $(PY-MYGHTY_CONFFILES) | sed -e 's/ /\n/g' > $(PY26-MYGHTY_IPK_DIR)/CONTROL/conffiles
	cd $(BUILD_DIR); $(IPKG_BUILD) $(PY26-MYGHTY_IPK_DIR)

#
# This is called from the top level makefile to create the IPK file.
#
py-myghty-ipk: $(PY25-MYGHTY_IPK) $(PY26-MYGHTY_IPK)
...
py-myghty-dirclean:
	...
	rm -rf $(PY26-MYGHTY_IPK_DIR) $(PY25-MYGHTY_IPK)

#
# Some sanity check for the package.
#
py-myghty-check: $(PY25-MYGHTY_IPK) $(PY26-MYGHTY_IPK)
