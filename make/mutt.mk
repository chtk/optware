...
#MUTT_CONFFILES=$(OPTWARE_PREFIX)etc/mutt.conf $(OPTWARE_PREFIX)etc/init.d/SXXmutt
...
#	cat $(MUTT_PATCHES) | patch -d $(BUILD_DIR)/$(MUTT_DIR) -p1
	...
	(cd $(@D); \
		...
		--disable-nls \
		--with-mailpath=$(OPTWARE_PREFIX)var/spool/mail \
		--enable-imap \
		--with-ssl \
		--with-sasl2 \
		--with-bdb \
...
#
# Binaries should be installed into $(MUTT_IPK_DIR)$(OPTWARE_PREFIX)sbin or $(MUTT_IPK_DIR)$(OPTWARE_PREFIX)bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(MUTT_IPK_DIR)$(OPTWARE_PREFIX){lib,include}
# Configuration files should be installed in $(MUTT_IPK_DIR)$(OPTWARE_PREFIX)etc/mutt/...
# Documentation files should be installed in $(MUTT_IPK_DIR)$(OPTWARE_PREFIX)doc/mutt/...
# Daemon startup scripts should be installed in $(MUTT_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/S??mutt
#
...
$(MUTT_IPK): $(MUTT_BUILD_DIR)/.built
	...
	$(STRIP_COMMAND) $(MUTT_IPK_DIR)$(OPTWARE_PREFIX)bin/mutt $(MUTT_IPK_DIR)$(OPTWARE_PREFIX)bin/pgpewrap $(MUTT_IPK_DIR)$(OPTWARE_PREFIX)bin/pgpring
#	install -d $(MUTT_IPK_DIR)$(OPTWARE_PREFIX)etc/
#	install -m 755 $(MUTT_SOURCE_DIR)/mutt.conf $(MUTT_IPK_DIR)$(OPTWARE_PREFIX)etc/mutt.conf
#	install -d $(MUTT_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d
#	install -m 755 $(MUTT_SOURCE_DIR)/rc.mutt $(MUTT_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/SXXmutt
