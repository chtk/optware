...
#PICOLISP_CONFFILES=$(OPTWARE_PREFIX)etc/picolisp.conf $(OPTWARE_PREFIX)etc/init.d/SXXpicolisp
...
#
# Binaries should be installed into $(PICOLISP_IPK_DIR)$(OPTWARE_PREFIX)sbin or $(PICOLISP_IPK_DIR)$(OPTWARE_PREFIX)bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(PICOLISP_IPK_DIR)$(OPTWARE_PREFIX){lib,include}
# Configuration files should be installed in $(PICOLISP_IPK_DIR)$(OPTWARE_PREFIX)etc/picolisp/...
# Documentation files should be installed in $(PICOLISP_IPK_DIR)$(OPTWARE_PREFIX)doc/picolisp/...
# Daemon startup scripts should be installed in $(PICOLISP_IPK_DIR)$(OPTWARE_PREFIX)etc/init.d/S??picolisp
#
...
#	$(MAKE) -C $(PICOLISP_BUILD_DIR) DESTDIR=$(PICOLISP_IPK_DIR) install-strip
	install -d $(PICOLISP_IPK_DIR)$(OPTWARE_PREFIX)lib/picolisp
	cp -a $(PICOLISP_BUILD_DIR)/* $(PICOLISP_IPK_DIR)$(OPTWARE_PREFIX)lib/picolisp/
	rm -rf $(PICOLISP_IPK_DIR)$(OPTWARE_PREFIX)lib/picolisp/cygwin
	rm -rf $(PICOLISP_IPK_DIR)$(OPTWARE_PREFIX)lib/picolisp/src
	$(MAKE) $(PICOLISP_IPK_DIR)/CONTROL/control
	echo "touch $(OPTWARE_PREFIX)lib/picolisp/.picoHistory" > $(PICOLISP_IPK_DIR)/CONTROL/postinst
	echo "chmod 666 $(OPTWARE_PREFIX)lib/picolisp/.picoHistory" >> $(PICOLISP_IPK_DIR)/CONTROL/postinst
